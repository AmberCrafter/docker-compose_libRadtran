...
char* get_crystal_filename (char* datapath, char* crystal_name);

double get_crystal_geometry (atmosphere_struct* atmos,
                             double**           p1,
                             double**           p2,
                             double**           p3,
                             double**           p1_col,
                             ...
                             double             fraction_col,
                             int*               deg_of_freedom,
                             double             fraction_col_orient,
                             double             fraction_pla_orient,
                             double             dist_col,
                             double             dist_pla,
                             double*            crystal_axis_tilt,
                             const int          um,
                             int*               vm);

void raytracing_crystal (double   refre,
                         double** p1,
                         double** p2,
                         double** p3,
                         double*  dx,
...
void random_crystal_rotation (double**  p1,
                              double**  p2,
                              double**  p3,
                              double**  p4,
                              double**  p5,
...
void solve_equation_system (double* co11,
                            double* co12,
                            double* co13,
                            double* co21,
                            double* co22,
...
char* get_crystal_filename (char* datapath, char* crystal_name) {
  char  crystal_ending[] = ".crystal_points";
  char  path[]           = "ic/crystal_geometry/";
  char* filename         = malloc (strlen (datapath) + strlen (path) + strlen (crystal_name) + strlen (crystal_ending) + 1);
  //fprintf(stderr, "crystal_type: %s\n", crystal_name);
  strcpy (filename, datapath);
  strcat (filename, path);
  strcat (filename, crystal_name);
  strcat (filename, crystal_ending);
  return filename;
}

void new_direction_raytracing (atmosphere_struct* atmos,
                               char*              datapath,
                               double             mu,
                               double             phi,
                               direction*         dir,
  ...
  for (i = 0; i < un; i++)
    p1[i] = malloc (vn * sizeof (double));
  p2 = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    p2[i] = malloc (vn * sizeof (double));
  p3 = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    p3[i] = malloc (vn * sizeof (double));
  ...
  if (first_call) {
    // initilize crystal coordinates for columns and plates
    p1_col = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p1_col[i] = malloc (vn * sizeof (double));
    p2_col = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p2_col[i] = malloc (vn * sizeof (double));
    p3_col = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p3_col[i] = malloc (vn * sizeof (double));

    p1_pla = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p1_pla[i] = malloc (vn * sizeof (double));
    p2_pla = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p2_pla[i] = malloc (vn * sizeof (double));
    p3_pla = malloc (un * sizeof (double*));
    for (i = 0; i < un; i++)
      p3_pla[i] = malloc (vn * sizeof (double));

    for (iprop = 0; iprop < atmos->n_raytracing_prop; iprop++) {
      if (strstr (atmos->raytracing_prop[iprop].name, "columns")) {
        // generate crystal filename according to raytracing_prop.name
        char* filename = get_crystal_filename (datapath, atmos->raytracing_prop[iprop].name);
        // read in crystal points
        read_3c_file (filename, &first, &second, &third, &value);
        l = 0;
        for (i = 0; i < um; i++) {
          for (j = 0; j < vm[i]; j++) {
            p1_col[i][j] = first[l];
      ...
      else if (strstr (atmos->raytracing_prop[iprop].name, "plates")) {
        // generate crystal filename according to raytracing_prop.name
        char* filename = get_crystal_filename (datapath, atmos->raytracing_prop[iprop].name);
        // read in crystal points
        read_3c_file (filename, &first, &second, &third, &value);
        ...
        fprintf (stderr,
                 "ERROR crystal name is '%s', but has to be either 'plates*' or 'columns*'!\n",
                 atmos->raytracing_prop[iprop].name);
    }
    first_call = 0;
  }
  ...
  dist = get_crystal_geometry (atmos,
                               p1,
                               p2,
                               p3,
                               p1_col,
...
double get_crystal_geometry (atmosphere_struct* atmos,
  ...
  double rnd  = 0;
  double dist = 0;
  double cumulative_fractions[atmos->n_raytracing_prop];
  double dummy = 0;
  //char name[10];
  // habit mixture
  ...
  for (i = 0; i < atmos->n_raytracing_prop; i++) {
    dummy = 0;
    for (j = 0; j <= i; j++) {
      dummy += atmos->raytracing_prop[j].fraction;
    }
    cumulative_fractions[i] = dummy;
  }

  if (cumulative_fractions[atmos->n_raytracing_prop - 1] > 1) {
    fprintf (stderr,
             "ERROR crystal fraction sum up to %f, should be 1!\nPlease correct the input parametes in ic_raytracing_file!\n",
             cumulative_fractions[atmos->n_raytracing_prop - 1]);
  }

  rnd = uvspec_random();
  for (iprop = 0; iprop < atmos->n_raytracing_prop; iprop++) {
    // get crystal properties from ith component
    if (rnd < cumulative_fractions[iprop]) {
      //fprintf(stderr, "%s, orientation_fraction=%.2f, angdist_width=%.2f\n", atmos->raytracing_prop[iprop].name, atmos->raytracing_prop[iprop].oriented_fraction, atmos->raytracing_prop[iprop].angdist_width);
      if (strstr (atmos->raytracing_prop[iprop].name, "columns")) {
        //strcpy(name, "columns");
        // hexagonal columns
        for (i = 0; i < um; i++) {
        ...
        rnd                 = uvspec_random();
        fraction_col_orient = atmos->raytracing_prop[iprop].oriented_fraction;
        if (rnd < fraction_col_orient) {
          (*crystal_axis_tilt) = M_PI / 2.;
          if (atmos->raytracing_prop[iprop].orientation_dof == 1)
            (*deg_of_freedom) = 1;  //Parry-oriented columns
          else
            (*deg_of_freedom) = 2;  //oriented columns
          dist = atmos->raytracing_prop[iprop].angdist_width;
        } else {
        ...
        rnd                 = uvspec_random();
        fraction_pla_orient = atmos->raytracing_prop[iprop].oriented_fraction;
  ...
  //fprintf(stderr, "%d, %s, orientation_fraction=%.2f, angdist_width=%.2f, dof=%d\n", iprop, name, atmos->raytracing_prop[iprop].oriented_fraction, dist, (*deg_of_freedom));
  return dist;
}

/** \brief calculate a new photon direction, based on raytracing through crystal
 *
 * \param [in] refre real part of refractive index
 * \param p1 [in/out] x values of crystal corners
 * \param p2 [in/out] y values of crystal corners
 * \param p3 [in/out] z values of crystal corners
 * \param dx [in/out] unit vector of incident direction
 * \param u [in/out] helper unit vector, perpendicular to dx and v
 * \param v [in/out] helper unit vector, perpendicular to dx and u, (== dx x v)
 * \param deg_of_freedom [in] integer representing the number of Euler angles of free rotation [1...3]
 * \param dist [in] orientation distortion parameter in degrees
 * \param crystal_axis_tilt [in] default orientation of crystal c-axis 0 for plates, pi/2 for columns
 *
 * \note p1, p2 and p3 must describe a hexagon with base and top faces first,
 *       followed by side faces. (Base & Top: 6 corners each, side faces: 4 corners each)
 *       8 crystal faces in total.
 * \note p1, p2 and p3 will be overwritten by this function
 */
...
void raytracing_crystal (double   refre,
  ...
  for (i = 0; i < un; i++)
    p4[i] = malloc (vn * sizeof (double));
  p5 = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    p5[i] = malloc (vn * sizeof (double));
  p6 = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    p6[i] = malloc (vn * sizeof (double));
  ...
  for (i = 0; i < un; i++)
    kano1[i] = malloc (vn * sizeof (double));
  double** kano2 = NULL;
  kano2          = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    kano2[i] = malloc (vn * sizeof (double));
  double** kano3 = NULL;
  kano3          = malloc (un * sizeof (double*));
  for (i = 0; i < un; i++)
    kano3[i] = malloc (vn * sizeof (double));
  ...
  int    utr = 0, utrold = 0;
  double phi = 0, cos_phi = 0, sin_phi = 0, radius = 0, crystal_radius = 0;
  ...
  utrold = 999;
  utr    = -999;  // number of crystal plane

  while (utr == -999) {
    random_crystal_rotation (p1,
                             p2,
                             p3,
                             p4,
                             p5,
    ...
    // search intersection with crystal plane
    phi            = uvspec_random() * 2. * M_PI;
    cos_phi        = cos (phi);
    sin_phi        = sin (phi);
    radius         = sqrt (uvspec_random());  // interval [0,1]
    crystal_radius = sqrt (lq_max) / 2.;

    xold = sp1 - dx[0] * crystal_radius + (cos_phi * u[0] + sin_phi * v[0]) * crystal_radius * radius;
    yold = sp2 - dx[1] * crystal_radius + (cos_phi * u[1] + sin_phi * v[1]) * crystal_radius * radius;
    zold = sp3 - dx[2] * crystal_radius + (cos_phi * u[2] + sin_phi * v[2]) * crystal_radius * radius;
  ...
  // calculate angles of incidence (alpha) and transmission (beta) using Snell's law
  ...
  if (rnd < R) {
    ...
    // compare scattering angles of external reflection
    assert (fabs (acos (scalar_product (ev3, vk9)) - (M_PI - 2 * alpha)) < 1e-6);
    //assert(fabs(acos(ev3[0]*vk9[0] +ev3[1]*vk9[1] +ev3[2]*vk9[2]) - (M_PI - 2*alpha))<1e-6);
    ...
    if (alpha != 0) {
      ...
      assert (fabs (scalar_product (vk7, vk12)) < 1e-6);
      assert (fabs (scalar_product (vk7, dx)) < 1e-6);
      assert (fabs (vk7[0] * fn1[utr] + vk7[1] * fn2[utr] + vk7[2] * fn3[utr]) < 1e-6);

      if (scalar_product (vk7, ev1) < 0)
        for (k = 0; k < 3; k++)
          vk7[k] = -vk7[k];

      // test if vectors include angle < 180deg
      assert (scalar_product (vk7, ev1) > 0);

      cross_product (vk12, vk7, cross12);

      if (scalar_product (cross12, ev2) < 0)
        for (k = 0; k < 3; k++)
          cross12[k] = -cross12[k];

      // test if vectors are perpendicular
      assert (scalar_product (cross12, vk7) < 1e-5);
      assert (scalar_product (cross12, ev2) > 0);
    }
    // compare scattering angles of 1st transmission
    assert (fabs (acos (scalar_product (ev3, vk12)) - (alpha - beta)) < 1e-6);
    ...
    while (rnd < R) {
      ...
      if (rnd < R) {
        ...
        // compare scattering angles of internal reflection
        assert (fabs (acos (scalar_product (ev3, vk9)) - (M_PI - 2. * beta)) < 1e-6);
    ...
    if (scalar_product (vk7, vk7_check) < 0)
      for (k = 0; k < 3; k++)
        vk7[k] = -vk7[k];

    //fprintf(stderr, "asser %f\n", (vk7[0]*vk7_check[0]+vk7[1]*vk7_check[1]+vk7[2]*vk7_check[2]));
    assert (scalar_product (vk7, vk7_check) >= 0);
    ...
    // compare scattering angles of 2nd transmission
    assert (fabs (acos (scalar_product (ev3, vk12)) - (alpha - beta)) < 1e-6);
...
double refraction (double* ref1, double* ref2, double* ref3, double* ref6, double theta_i, double* krel, double* k2) {
  // This function was translated from A. Macke's fortran90 code (Macke et al. 1996, tools.tropos.de)
...
void solve_equation_system (double* co11,
  ...
  *x3 = 0;
  *vf = 99;
  d1  = (*co11) * (*co22) * (*co33) + (*co21) * (*co32) * (*co13) + (*co31) * (*co12) * (*co23) - (*co31) * (*co22) * (*co13) -
       (*co11) * (*co32) * (*co23) - (*co21) * (*co12) * (*co33);
  if (d1 == 0)
    (*vf) = 2;
  else {
    d2 = (*co14) * (*co22) * (*co33) + (*co24) * (*co32) * (*co13) + (*co34) * (*co12) * (*co23) - (*co34) * (*co22) * (*co13) -
         (*co14) * (*co32) * (*co23) - (*co24) * (*co12) * (*co33);
    d3 = (*co11) * (*co24) * (*co33) + (*co21) * (*co34) * (*co13) + (*co31) * (*co14) * (*co23) - (*co31) * (*co24) * (*co13) -
         (*co11) * (*co34) * (*co23) - (*co21) * (*co14) * (*co33);
    d4 = (*co11) * (*co22) * (*co34) + (*co21) * (*co32) * (*co14) + (*co31) * (*co12) * (*co24) - (*co31) * (*co22) * (*co14) -
         (*co11) * (*co32) * (*co24) - (*co21) * (*co12) * (*co34);
    *x1 = d2 / d1;
    *x2 = d3 / d1;
    *x3 = d4 / d1;
    *vf = 0;
  ...
  *utr = -999;
  ...
  while (m < (h - 1) && (*utr) != u && lq[m] < (*lq_max)) {  //  determines wether xs is within the boundary of the surface
    ...
    while (inside == 1 && v != vmu) {
      v      = v + 1;
      scalpr = kano1[ui][v - 1] * (xs1[ui] - p1[ui][v - 1]) + kano2[ui][v - 1] * (xs2[ui] - p2[ui][v - 1]) +
               kano3[ui][v - 1] * (xs3[ui] - p3[ui][v - 1]);
      if (scalpr > 0.)
        inside = 0;
    }
    if (inside == 1)
...
void random_crystal_rotation (double**  p1,
                              ...
                              double    dist,
                              double    crystal_axis_tilt) {
  // Parts of this function were inspired by A. Macke's fortran90 code (Macke et al. 1996, tools.tropos.de)
  ...
  if ((deg_of_freedom) == 3) {
    ...
    beta_euler  = acos (1. - 2. * uvspec_random());  // as in Macke rt.f90
  ...
  //orientation, singly oriented columns
    ...
    gamma_euler = 2. * M_PI * uvspec_random();
  } else if ((deg_of_freedom) == 1) {
    alpha_euler = 2. * M_PI * uvspec_random();
    beta        = uvspec_random_gauss (dist);
    beta_euler  = M_PI / 2. + beta / 180. * M_PI;
    gamma       = uvspec_random_gauss (dist);
    gamma_euler = 30. / 180. * M_PI + gamma / 180. * M_PI;
  } else
  ...
  for (i = 0; i < um; i++) {
    for (j = 0; j < vm[i]; j++) {
      if (x_max < p1[i][j])
        x_max = p1[i][j];  // use maxval, minval
      if (y_max < p2[i][j])
        y_max = p2[i][j];
      if (z_max < p3[i][j])
        z_max = p3[i][j];
