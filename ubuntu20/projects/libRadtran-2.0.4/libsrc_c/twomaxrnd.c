...
static int buildMatrixA (int      nlev,
                         double   Ag,
                         double*  ar_a11_c,
                         double*  ar_a11_f,
                         double*  ar_a12_c,
...
static int buildVectorBsol (int     nlev,
                            double  Ag,
                            double  mu0,
                            double* ar_a13_c,
                            double* ar_a13_f,
...
static int buildVectorBthe (int     nlev,
                            double  Ag,
                            double  Bg,
...
static int twostream_maxrand (double*  dtau_c,
                              double*  omega0_c,
                              double*  g_c,  //"_c" for cloudy region
                              double*  dtau_f,
                              double*  omega0_f,
                              double*  g_f,  //"_f" for cloud-free region
                              double*  cf,
                              int      nlev,
                              double   S0,
                              double   mu0,
...
int twomaxrnd (float* dtau_org,
  ...
  status = twostream_maxrand (dtau_org_d,
                              omega0_org_d,
                              g_org_d,
                              dtau_clr_org_d,
                              omega0_clr_org_d,
...
static int twostream_maxrand (double*  dtau_org_c,
                              double*  omega0_org_c,
                              double*  g_org_c,  //"_c" for cloudy region
                              double*  dtau_org_f,
                              double*  omega0_org_f,
  ...
  *Lavg = calloc (nlev, sizeof (double));
  ...
  for (ilyr = 0; ilyr < nlyr; ilyr++) {
    ...
    if (delta) {
      delta_scale_hg (dtau_org_c[ilyr], omega0_org_c[ilyr], g_org_c[ilyr], &dtau_c, &omega0_c, &g_c);
    } else {
  ...
  // Build matrix AA:
  iStatus = buildMatrixA (nlev, Ag, ar_a11_c, ar_a11_f, ar_a12_c, ar_a12_f, ar_p1, ar_p2, ar_p3, ar_p4, AA);
  if (iStatus != 0) {
  ...
  double **matr, **mlu;
  long*    indx;
  int      i = 0, nn = NFLUX * nlev, m1 = 5, m2 = 5;  // n=2*input.nlyr+2;
  ...
  for (ilyr = 0; ilyr < nlyr; ilyr++) {
    ...
    if (ilyr == 0) {
      ...
      if (cf[ilyr - 1] == 1.0)
        ...
        ar_p1[ilyr] = (1.0 - max_p1) / (1.0 - cf[ilyr - 1]);

      if (cf[ilyr - 1] == 0.0)
        ar_p3[ilyr] = 1.0;
      else
        ar_p3[ilyr] = min_p3 / cf[ilyr - 1];
  ...
  for (ilyr = 0; ilyr < nlyr; ilyr++) {
    ...
    if (ilyr == (nlyr - 1)) {
      ...
      if (cf[ilyr + 1] == 1.0)
        ...
        ar_p2[ilyr] = (1.0 - max_p2) / (1.0 - cf[ilyr + 1]);

      if (cf[ilyr + 1] == 0.0)
        ar_p4[ilyr] = 1.0;
      else
        ar_p4[ilyr] = min_p4 / cf[ilyr + 1];
    }  //e-if
  }    //e-for
...
void eddington_coeffc (double  dtau,
                       double  g,
                       double  omega0,
                       double  mu0,
                       double* a11,
...
void calcThermalComponents (int ilyr, double* B, double dtau, double omega0, double g, double* theComp1, double* theComp2) {
  double B0;
  double B1;
  double alpha1;
  double alpha2;
