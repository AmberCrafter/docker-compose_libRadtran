...
int write_netCDF_1D_float (int    ncid,
                           char*  filename,
                           char*  data_name,
                           int*   id_data,
                           char*  long_name,
  ...
  if (status != NC_NOERR) {
    ...
    if (status != NC_NOERR) {
      fprintf (stderr,
               "Error %d putting netCDF file %s into define mode, in %s (%s)\n",
               status,
               filename,
               function_name,
               file_name);
    ...
    if (status != NC_NOERR) {
      fprintf (stderr,
               "Error %d defining data '%s' in netCDF file %s, in %s (%s)\n",
               status,
               data_name,
               filename,
               function_name,
    ...
    for (iv = 0; iv < n; iv++) {
      ...
      if (status != NC_NOERR) {
        fprintf (stderr,
                 "Error %d writing %s in netCDF file %s, in %s (%s)\n",
                 status,
                 data_name,
                 filename,
                 function_name,
...
int write_netCDF_3D_float (int     ncid,
                           char*   filename,
                           char*   data_name,
                           char*   long_name,
                           char*   units,
  ...
  status = create_float_variable_or_get_id (ncid, filename, data_name, &id_data, ndim, id_dim_array, long_name, units);
  if (status != NC_NOERR) {
    fprintf (stderr,
             "Error %d creating float variable %s in netCDF file %s, in %s (%s)\n",
             status,
             long_name,
             filename,
             function_name,
...
int write_netCDF_4D_float (int     ncid,
                           char*   filename,
                           char*   data_name,
                           char*   long_name,
                           char*   units,
...
int write_netCDF_5D_float (int     ncid,
                           char*   filename,
                           char*   data_name,
                           char*   long_name,
                           char*   units,
...
int write_netCDF_6D_float (int      ncid,
                           char*    filename,
                           char*    data_name,
                           char*    long_name,
                           char*    units,
    ...
    for (iu = 0; iu < n_end[POSITION_UMU_6D]; iu++)
      for (iv = 0; iv < n_end[POSITION_LAMBDA_6D]; iv++) {
        ...
        if (isnan (data[lev][iu][iv])) {
          (*simulation_done) = FALSE;
          fprintf (stderr,
                   " *** Warning, unresonable result for '%s' (iv=%d, lev=%d, iu=%d)\n",
                   long_name,
                   (int)iv,
                   (int)lev,
                   (int)iu);
...
int write_netCDF_7D_float (int       ncid,
                           char*     filename,
                           char*     data_name,
                           char*     long_name,
                           char*     units,
      ...
      for (iu = 0; iu < n_end[POSITION_UMU_6D]; iu++)
        for (iv = 0; iv < n_end[POSITION_LAMBDA_6D]; iv++) {
          ...
          if (isnan (data[lev][ip][iu][iv])) {
            (*simulation_done) = FALSE;
            fprintf (stderr,
                     " *** Warning, unresonable result for '%s' (iv=%d, lev=%d, iu=%d, ip=%d)\n",
                     long_name,
                     (int)iv,
                     (int)lev,
                     (int)iu,
...
int create_float_variable_or_get_id (int   ncid,
                                     char* filename,
                                     char* data_name,
                                     int*  id_data,
                                     int   ndim,
  ...
  if (status != NC_NOERR) {
    ...
    if (status != NC_NOERR) {
      fprintf (stderr,
               "Error %d put attribute long_name to '%s' in netCDF file %s, in %s (%s)\n",
               status,
               data_name,
               filename,
               function_name,
    ...
    if (status != NC_NOERR) {
      fprintf (stderr,
               "Error %d put attribute units to '%s' in netCDF file %s, in %s (%s)\n",
               status,
               data_name,
               filename,
               function_name,
    ...
    if (status != NC_NOERR) {
      fprintf (stderr,
               "Error %d defining missing argument in netCDF file %s, in %s (%s)\n",
               status,
               filename,
               function_name,
               file_name);
...
int write_netCDF_3D_byte (int     ncid,
                          char*   filename,
                          char*   data_name,
                          char*   long_name,
                          char*   units,
...
int read_3d_float (int ncid, char* file, char* var_name, float**** float3d, long n1, long n2, long n3, char*** done, int verbose) {
  ...
  if (status != NC_NOERR) {
    fprintf (stderr,
             "Error, get id for '%s' field in the netCDF result file %s, in %s (%s) \n",
             var_name,
             file,
             function_name,
             file_name);
      ...
      for (c3 = 0; c3 < n3; c3++)
        for (c4 = 0; c4 < n4; c4++)
          for (j = 0; j < n5; j++)
            for (i = 0; i < n6; i++) {
              /* if (done[t][j][i]) { */
              (*float6d)[t][c2][c3][c4][j][i] =
                tmp_float6d[t * n2 * n3 * n4 * n5 * n6 + c2 * n3 * n4 * n5 * n6 + c3 * n4 * n5 * n6 + c4 * n5 * n6 + j * n6 + i];
        ...
        for (c4 = 0; c4 < n4; c4++)
          for (c5 = 0; c5 < n5; c5++)
            for (j = 0; j < n6; j++)
              for (i = 0; i < n7; i++) {
                /* if (done[t][j][i]) { */
                (*float7d)[t][c2][c3][c4][c5][j][i] =
                  tmp_float7d[(((((t * n2 + c2) * n3 + c3) * n4 + c4) * n5 + c5) * n6 + j) * n7 + i];
...
int read_3d_char (int ncid, char* file, char* var_name, char**** done, long ntime, long nlat, long nlon, int verbose) {
  ...
  if (status != NC_NOERR) {
    fprintf (stderr,
             "Error '%s' while reading '%s' from %s, in %s (%s)\n",
             nc_strerror (status),
             var_name,
             file,
             function_name,
...
int write_netcdf_int (int ncid, int value, char* var_name) {
  ...
  if ((retval = nc_def_var (ncid, var_name, NC_INT, 0, 0, &varid)))
    ERR (retval);
  ...
  if ((retval = nc_put_var_int (ncid, varid, &value)))
    ERR (retval);
...
int write_netcdf_float (int ncid, float value, char* var_name) {
  ...
  if ((retval = nc_redef (ncid)))
    ERR (retval);
  if ((retval = nc_def_var (ncid, var_name, NC_FLOAT, 0, 0, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_float (ncid, varid, &value)))
    ERR (retval);
...
int write_netcdf_1Dint (int ncid, int* value, int rows, char* var_name, char* row_name) {
  ...
  if ((retval = nc_redef (ncid)))
    ERR (retval);
  if ((nc_inq_dimid (ncid, row_name, &row_dimid) != NC_NOERR)) {
    if ((retval = nc_def_dim (ncid, row_name, rows, &row_dimid)))
      ERR (retval);
  }
  int dimid[1] = {row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_INT, 1, dimid, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_int (ncid, varid, &value[0])))
    ERR (retval);
...
int write_netcdf_1Dsize_t (int ncid, size_t* value, int rows, char* var_name, char* row_name) {
#if HAVE_NETCDF4
  if (ncid == 0)
    return -1;
  int                       retval, varid, row_dimid;
  const unsigned long long* data = (unsigned long long*)value;
  ...
  int dimid[1] = {row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_LONG, 1, dimid, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_ulonglong (ncid, varid, &(data[0]))))
    ERR (retval);
...
int write_netcdf_1Dfloat (int ncid, float* value, int rows, char* var_name, char* row_name) {
  ...
  int dimid[1] = {row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_FLOAT, 1, dimid, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_float (ncid, varid, &value[0])))
    ERR (retval);
...
int write_netcdf_1Ddouble (int ncid, double* value, int rows, char* var_name, char* row_name) {
#if HAVE_NETCDF4
  if (ncid == 0)
    return -1;
  int retval, varid, row_dimid;
  ...
  /********** The dimids array is used to pass the IDs of the dimensions of the variable. *********/
  int dimid[1] = {row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_DOUBLE, 1, dimid, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_double (ncid, varid, &value[0])))
    ERR (retval);
...
int write_netcdf_2Dint (int ncid, int** value, int rows, int columns, char* var_name, char* row_name, char* column_name) {
  ...
  if ((nc_inq_dimid (ncid, column_name, &column_dimid) != NC_NOERR)) {
    if ((retval = nc_def_dim (ncid, column_name, columns, &column_dimid)))
      ERR (retval);
  }
  int dimids[2] = {column_dimid, row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_INT, NDIMS, dimids, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_int (ncid, varid, &value_data[0][0])))
    ERR (retval);
...
int write_netcdf_2Dfloat (int ncid, float** value, int rows, int columns, char* var_name, char* row_name, char* column_name) {
  ...
  int dimids[2] = {column_dimid, row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_FLOAT, NDIMS, dimids, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_float (ncid, varid, &value_data[0][0])))
    ERR (retval);
...
int write_netcdf_2Ddouble (int ncid, double** value, int rows, int columns, char* var_name, char* row_name, char* column_name) {
  ...
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_double (ncid, varid, &value_data[0][0])))
    ERR (retval);
...
int write_netcdf_3Dfloat (int      ncid,
                          float*** value,
                          int      rows,
                          int      columns,
                          int      length,
  ...
  if ((nc_inq_dimid (ncid, length_name, &length_dimid) != NC_NOERR)) {
    if ((retval = nc_def_dim (ncid, length_name, length, &length_dimid)))
      ERR (retval);
  }
  ...
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_float (ncid, varid, &value_data[0][0][0])))
    ERR (retval);
...
int write_netcdf_3D_contiguous_float (int    ncid,
                                      float* value,
                                      int    rows,
                                      int    columns,
                                      int    length,
  ...
  int dimids[3] = {row_dimid, column_dimid, length_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_FLOAT, NDIMS, dimids, &varid)))
    ERR (retval);
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
...
int write_netcdf_3Dirr_row_float (int      ncid,
                                  float*** value,
                                  int      rows,
                                  int      columns,
                                  int*     length,
  ...
  length_max = 0;
  for (row = 0; row < rows; row++) {
    if (length_max < length[row])
      length_max = length[row];
  }
    ...
    for (column = 0; column < columns; column++) {
      for (il = 0; il < length_max; il++)
        if (il < length[row])
          ...
          value_data[row][column][il] = NOT_DEFINED_FLOAT;
    }
  }
  ...
  if ((nc_inq_dimid (ncid, length_name, &length_dimid) != NC_NOERR)) {
    if ((retval = nc_def_dim (ncid, length_name, length_max, &length_dimid)))
      ERR (retval);
  }
  int dimids[3] = {length_dimid, column_dimid, row_dimid};
  if ((retval = nc_def_var (ncid, var_name, NC_FLOAT, NDIMS, dimids, &varid)))
...
int write_netcdf_3Dirrfloat (int      ncid,
                             float*** value,
                             int      rows,
                             int      columns,
                             int**    length,
  ...
  for (row = 0; row < rows; row++) {
    for (column = 0; column < columns; column++) {
      if (length_max < length[row][column])
        length_max = length[row][column];
    }
  }

  if (length_max <= 0) {
    fprintf (stderr, "ERROR: Could write %s to netcdf_file\n", var_name);
    return 0;
  }
  /**** Copy ***value into a block of contiguous data values for netcdf ****/
  float value_data[rows][columns][length_max];
...
int write_netcdf_3Dirrdouble (int       ncid,
                              double*** value,
                              int       rows,
                              int       columns,
                              int**     length,
  ...
  if (length_max <= 0) {
    fprintf (stderr, "WARNING: Did not write %s to netcdf_file because it is empty\n", var_name);
    return 0;
  }
  ...
  if ((retval = nc_enddef (ncid)))
    ERR (retval);
  if ((retval = nc_put_var_double (ncid, varid, &value_data[0][0][0])))
    ERR (retval);
