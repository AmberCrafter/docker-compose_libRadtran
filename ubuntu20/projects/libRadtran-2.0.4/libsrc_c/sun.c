...
double eccentricity (int day) {
  ...
  E0 = 1.000110 + 0.034221 * cos (angle) + 0.001280 * sin (angle) + 0.000719 * cos (2 * angle) + 0.000077 * sin (2 * angle);
...
double declination (int day) {
  ...
  delta = 0.006918 - 0.399912 * cos (angle) + 0.070257 * sin (angle) - 0.006758 * cos (2 * angle) + 0.000907 * sin (2 * angle) -
...
int equation_of_time (int day) {
  ...
  et =
    (0.000075 + 0.001868 * cos (angle) - 0.032077 * sin (angle) - 0.014615 * cos (2 * angle) - 0.04089 * sin (2 * angle)) * 13750.8;
...
int LAT (int time_std, int day, double longitude, double long_std) {
  ...
  lat = time_std + (int)(240.0 * (long_std - longitude)) + equation_of_time (day);
...
double solar_azimuth (int time, int day, double latitude, double longitude, double long_std) {
  double theta = 0, delta = 0, phi = latitude * _PI_ / 180.0, psi = 0, cospsi = 0;
  int    lat = LAT (time, day, longitude, long_std);
...
int zenith2time (int day, double zenith_angle, double latitude, double longitude, double long_std, int* time1, int* time2) {
  double delta     = _PI_ / 180.0 * declination (day);
  double phi       = _PI_ / 180.0 * latitude;
  double theta     = _PI_ / 180.0 * zenith_angle;
  double cos_omega = (cos (theta) - sin (delta) * sin (phi)) / cos (delta) / cos (phi);
