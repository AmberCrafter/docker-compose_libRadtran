...
int setup_factors (/* INPUT */
  ...
  if (useplanck) {
    if (wvnm_low > 5000.0) { /* no thermal for lambda < 2mu */
      ...
      for (i = 0; i < nlyr + 1; i++)
        F77_FUNC (cplkavg, CPLKAVG) (&(wvnm_low), &(wvnm_upp), &(temper[i]), &(B_atm[i]));
      F77_FUNC (cplkavg, CPLKAVG) (&(wvnm_low), &(wvnm_upp), &(btemp), &(input->B_ground));
    }
  }
  ...
  for (i = 0; i < nlyr; i++) {
    ...
    g_d       = (g[i] - f[i]) / (1. - f[i]);
    omega_0_d = omega_0[i] * (1. - f[i]) / (1. - f[i] * omega_0[i]);
    ...
    if (input->solar) {
      ...
      input->a13[i] = +alpha_5 * (1.0 - input->a33[i] * input->a11[i]) - alpha_6 * input->a12[i];

      input->a23[i] = -alpha_5 * input->a33[i] * input->a12[i] + alpha_6 * (input->a33[i] - input->a11[i]);
    ...
    if (input->usrtau && iu >= 0) {
      while (input->ilyr_usr[iu] == i) {
        ...
        if (input->solar) {
          ...
          input->a23_usr[iu] =
            -alpha_5 * input->a33_usr[iu] * input->a21_usr[iu] + alpha_6 * (input->a33_usr[iu] - input->a22_usr[iu]);
        ...
        /* solar only */
        if (input->solar) {
          input->a13_usr[iu] = +alpha_5 * (1.0 - exp (-dusrtau1_d * mu_0_inv) * input->a11_usr[iu]) - alpha_6 * input->a12_usr[iu];
...
int multi_layer_solver (rodents_input_struct input, rodents_output_struct output) {
  double **matr, **mlu, *b;
  long*    indx;
  ...
  if (input.S_0 != 0.0) {
    solve_direct (input.a33, input.a33_unsc, input.nlyr, input.S_0, output.s_direct, output.s_direct_unsc);
  }
  ...
  bandec5d (matr, n, mlu, indx);
  banbks5d (matr, n, mlu, indx, b);
...
void solve_direct (double* a33, double* a33_unsc, int nlyr, double S_0, float* S, float* S_unsc) {
  int i = 0;
...
void cp_to_result (double* b, int nlyr, float* e_minus, float* e_plus) {
  int i = 0;
  for (i = 0; i < nlyr + 1; i++) {
    e_minus[i] = b[2 * i];
    e_plus[i]  = b[2 * i + 1];
...
int process_output (/* input */
  ...
  if (input.usrtau) {
    for (iu = 0; iu < input.nzout; iu++) {
      ...
      fldn[iu] = input.a21_usr[iu] * output.e_plus[i + 1] + input.a22_usr[iu] * output.e_minus[i] +
                 input.a23_usr[iu] * output.s_direct[i] + input.b2_usr[iu];

      flup[iu] = input.a11_usr[iu] * output.e_plus[i + 1] + input.a12_usr[iu] * output.e_minus[i] +
                 input.a13_usr[iu] * output.s_direct[i] + input.b1_usr[iu];
    ...
    for (iu = 0; iu < input.nlyr + 1; iu++) {
      fldn[iu]  = output.e_minus[iu] + (output.s_direct[iu] - output.s_direct_unsc[iu]) * input.mu_0;
