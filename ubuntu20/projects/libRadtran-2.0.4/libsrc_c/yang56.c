...
static float qep[3][12] = {{1.989, 1.979, 1.982, 1.992, 1.984, 2.032, 2.036, 2.029, 2.012, 2.006, 2.000, 1.998},
...
static float ssp[3][12] = {{0.5294, 0.5205, 0.5198, 0.5191, 0.5116, 0.523, 0.5384, 0.5475, 0.5562, 0.5633, 0.5556, 0.5678},
...
int yang56_old (float wavel, float reff, int nhabit, char* path, float* exti, float* ssai, float* gi, float* g2i, float* fi) {
  ...
  if (filere == 0) {
    ...
    for (j = 1; j < IC_HABIT_NN; j++)
      for (i = 1; i < index; i++)
        if (!fscanf (fp, "%s %f %f %f %f %f", habit_name, &auxd, &ext[j][0][i], &ext[j][1][i], &ext[j][2][i], &ext[j][3][i])) {
          fprintf (stderr, "Error reading file %s\n", filename);
          return -1;
        }
    ...
    for (j = 0; j < IC_HABIT_NN; j++)
      for (i = 0; i < index; i++)
        if (!fscanf (fp, "%s %f %f %f %f %f", habit_name, &auxd, &ssa[j][0][i], &ssa[j][1][i], &ssa[j][2][i], &ssa[j][3][i])) {
          fprintf (stderr, "Error reading file %s\n", filename);
          return -1;
        }
  ...
  if (wavel < wlp[0]) {
    ...
    auxd  = 1.0 / reff;
    auxd1 = ext[nhabit][0][ini] + ext[nhabit][1][ini] * auxd + ext[nhabit][2][ini] * auxd * auxd +
            ext[nhabit][3][ini] * auxd * auxd * auxd;
    auxd2 = ext[nhabit][0][fin] + ext[nhabit][1][fin] * auxd + ext[nhabit][2][fin] * auxd * auxd +
            ext[nhabit][3][fin] * auxd * auxd * auxd;
    *exti = auxd1 + (wavel - wl[ini]) * (auxd2 - auxd1) / (wl[fin] - wl[ini]);

    auxd  = reff;
    auxd1 = ssa[nhabit][0][ini] + ssa[nhabit][1][ini] * auxd + ssa[nhabit][2][ini] * (auxd * auxd) +
            ssa[nhabit][3][ini] * (auxd * auxd * auxd);
    auxd2 = ssa[nhabit][0][fin] + ssa[nhabit][1][fin] * auxd + ssa[nhabit][2][fin] * auxd * auxd +
            ssa[nhabit][3][fin] * auxd * auxd * auxd;
    *ssai = auxd1 + (wavel - wl[ini]) * (auxd2 - auxd1) / (wl[fin] - wl[ini]);

    auxd = reff;
    auxd1 =
      g[nhabit][0][ini] + g[nhabit][1][ini] * auxd + g[nhabit][2][ini] * (auxd * auxd) + g[nhabit][3][ini] * (auxd * auxd * auxd);
    auxd2 = g[nhabit][0][fin] + g[nhabit][1][fin] * auxd + g[nhabit][2][fin] * auxd * auxd + g[nhabit][3][fin] * auxd * auxd * auxd;
...
int yang56 (float  wavelength,
  ...
  const unsigned int lext                   = 16;  // length of extension string
  char               extstr[lext];
  ...
  if (!fileread[habit]) {
    ...
    /*------------------------*/
    if (newkey)
      sprintf (filename, "%.*sext.new.%s", FILENAME_MAX - 9 - lext, path, extstr);
    else
      sprintf (filename, "%.*sext.%s", FILENAME_MAX - 5 - lext, path, extstr);

    status = read_6c_file (filename, &wlmin[habit], &wlmax[habit], &a0[habit], &a1[habit], &a2[habit], &a3[habit], &nwvn);
    ...
    /*--------------------------*/
    if (newkey)
      sprintf (filename, "%.*sssa.new.%s", FILENAME_MAX - 10 - lext, path, extstr);
    else
      sprintf (filename, "%.*sssa.%s", FILENAME_MAX - 6 - lext, path, extstr);

    status = read_6c_file (filename, &wlmin[habit], &wlmax[habit], &b0[habit], &b1[habit], &b2[habit], &b3[habit], &nwvn);
    ...
    /*---------------------*/
    if (newkey)
      sprintf (filename, "%.*sg.new.%s", FILENAME_MAX - 7 - lext, path, extstr);
    else
      sprintf (filename, "%.*sg.%s", FILENAME_MAX - 3 - lext, path, extstr);
