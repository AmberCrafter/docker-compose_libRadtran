...
int crs_rayleigh_bodhaine (/* Input */
  ...
  for (iv = 0; iv < n_lambda; iv++) {
    ...
    n_300 =
      (8060.51 + (2480990 / (132.274 - pow (lambda_um, -2))) + (17455.7 / (39.32957 - pow (lambda_um, -2)))) * 1e-08; /* Eq. (18) */
...
int crs_rayleigh_penndorf (float* lambda, int n_lambda, float mixing_ratio_co2, float depol_user, float** depol, float** crs) {
  double from_nm_to_cm = 1.0e-7;
  double from_nm_to_um = 1.0e-3;
  double N_s           = 2.546899e+19;
  ...
  for (iv = 0; iv < n_lambda; iv++) {
    ...
    (*crs)[iv] = (ray_const * pow (n - 1, 2) / pow (lambda_cm, 4)) * ((6.0 + 3.0 * (*depol)[iv]) / (6.0 - 7.0 * (*depol)[iv]));
  }
