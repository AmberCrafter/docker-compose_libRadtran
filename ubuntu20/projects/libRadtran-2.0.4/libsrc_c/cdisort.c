...
void c_disort (disort_state* ds, disort_output* out) {
  static int self_tested = -1;
  int prntu0[2], corint, deltam, scat_yes, compare, lyrcut, needdeltam, iq, iu, j, kconv, l, lc, lev, lu, k, mazim, naz, ncol, ncos,
    ncut, nn;
  static int callnum = 1;
  int        ipvt[ds->nstr * ds->nlyr], layru[ds->ntau];
  double     angcos, azerr, azterm, bplanck, cosphi, delm0, sgn, tplanck, scat_type;
  double *array, *b, *bdr, *bem, *cband, *cc, *ch, *chtau, *cmu, *cwt, *dtaucpr, *emu, *eval, *evecc, *expbea, *flyr, *gc, *gl, *gu,
    *kk, *ll, *oprim, *phasa, *phast, *phasm, *phirad, *pkag, *rmu, *tauc, *taucpr, *u0c, *utaupr, *uum, *wk, *xba, *ylm0, *ylmc,
    *ylmu, *z, *zbeam, *zbeama, zbsa = 0, *zj, *zjg, *zju, *zgu, *zz, *zzg;
  disort_pair *   ab, *fl, *plk, *xr, *psi, *xb, *zbeamsp, *zbs, *zee, *zu;
  disort_triplet* zbu;
  const double    dither = 100. * DBL_EPSILON;
  ...
  if (self_tested == -1) {
    int           prntu0_test[2] = {FALSE, FALSE};
    disort_state  ds_test;
    disort_output out_test;
    /*
  ...
  if (ds->flag.onlyfl || ds->bc.fbeam == 0. || !scat_yes || !deltam)
    corint = FALSE;
  /* Turn on single scattering surface reflection correction for non-Lambertian BRDF*/
  /*  Arve: not yet implemented
      if ( !ds->flag.lamber ) 
      corint = FALSE; */

  prntu0[0] = ds->flag.prnt[2];
  prntu0[1] = ds->flag.prnt[2];
  ...
  if (ds->flag.ibcnd == SPECIAL_BC) {
    ...
    free (ll), free (oprim), free (taucpr), free (utaupr), free (wk);
   ...
   */
  fl = (disort_pair*)calloc (ds->ntau, sizeof (disort_pair));
  if (!fl)
    c_errmsg ("disort alloc error for fl", DS_ERROR);
  plk = (disort_pair*)calloc (ds->nlyr * ds->nstr, sizeof (disort_pair));
  if (!plk)
    c_errmsg ("disort alloc error for plk", DS_ERROR);
  ab = (disort_pair*)calloc ((ds->nstr / 2) * (ds->nstr / 2), sizeof (disort_pair));
  if (!ab)
    c_errmsg ("disort alloc error for ab", DS_ERROR);
  xr = (disort_pair*)calloc (ds->nlyr, sizeof (disort_pair));
  if (!xr)
    c_errmsg ("disort alloc error for xr", DS_ERROR);
  psi = (disort_pair*)calloc (ds->nstr, sizeof (disort_pair));
  if (!psi)
    c_errmsg ("disort alloc error for psi", DS_ERROR);
  xb = (disort_pair*)calloc (ds->nlyr * ds->nstr, sizeof (disort_pair));
  if (!xb)
    c_errmsg ("disort alloc error for xb", DS_ERROR);
  zbs = (disort_pair*)calloc (ds->nstr, sizeof (disort_pair));
  if (!zbs)
    c_errmsg ("disort alloc error for zbs", DS_ERROR);
  zbeamsp = (disort_pair*)calloc (ds->nlyr * ds->nstr, sizeof (disort_pair));
  if (!zbeamsp)
    c_errmsg ("disort alloc error for zbeamsp", DS_ERROR);
  zee = (disort_pair*)calloc (ds->nstr, sizeof (disort_pair));
  if (!zee)
    c_errmsg ("disort alloc error for zee", DS_ERROR);
  zu = (disort_pair*)calloc (ds->nlyr * ds->numu, sizeof (disort_pair));
  if (!zu)
    c_errmsg ("disort alloc error for zu", DS_ERROR);

  zbu = (disort_triplet*)calloc (ds->nlyr * ds->numu, sizeof (disort_triplet));
  if (!zbu)
    c_errmsg ("disort alloc error for zbu", DS_ERROR);
  ...
  /* scat_type: Flag to decide whether scattering is only Rayleigh
   *  or another type of scattering (>0 for cloud or aerosol, 
   *  =0 for Rayleigh scattering)
   *
   * Included on 2014-07-08 (CE)
   */
  scat_type = 0.0;
  for (lc = 1; lc <= ds->nlyr; lc++)
    for (k = 3; k <= ds->nstr; k++)
   ...
   */
  if (ds->bc.fbeam == 0. || fabs (1. - ds->bc.umu0) < 1.e-5 || ds->flag.onlyfl || (ds->numu == 1 && fabs (1. - UMU (1)) < 1.e-5) ||
  ...
  for (mazim = 0; mazim <= naz; mazim++) {
     ...
     */
    if (!lyrcut) {
      c_surface_bidir (ds, delm0, cmu, mazim, nn, bdr, emu, bem, rmu, callnum);
    }
    ...
    for (lc = 1; lc <= ncut; lc++) {
      ...
      if (ds->bc.fbeam > 0.) {
        if (ds->flag.spher == TRUE) {
          ...
          c_set_coefficients_beam_source (ds,
                                          ch,
                                          chtau,
                                          cmu,
                                          delm0,
          ...
          if (ds->flag.usrang == TRUE) {
            ...
            c_interp_coefficients_beam_source (ds,
                                               chtau,
                                               delm0,
                                               ds->bc.fbeam,
                                               gl,
          ...
          c_upbeam_pseudo_spherical (ds, lc, array, cc, cmu, ipvt, nn, wk, xb, xba, zbs, &zbsa, zbeamsp, zbeama);
        } else {
  ...
  if (corint) {
    ...
    /* Arve : not yet implmented
       if (!ds->flag.lamber && !lyrcut && ds->bc.fbeam > 0.) {
       fprintf(stderr,"Using single scattering beam reflection correction\n");*/
  ...
  free (z), free (zbu), free (zbeam), free (zbeamsp), free (zbeama), free (zbs), free (zj), free (zjg), free (zju), free (zgu),
    free (zz), free (zzg), free (zee), free (zu);
...
/*
  ...
  LOCAL VARIABLES:
   ...
   Calls- c_dref, c_bidir_reflectivity_hapke, c_bidir_reflectivity_rossli,
...
double c_bidir_reflectivity (double       wvnmlo,
  ...
  double ans, rmu, flxalb;

  static double badmu, swvnmlo, swvnmhi, srho0, sk, stheta, ssigma, st1, st2, sscale, siso, svol, sgeo;
  static int    shot;
  ...
  switch (brdf_type) {
  case BRDF_HAPKE:
    ans = c_bidir_reflectivity_hapke (wvnmlo, wvnmhi, mu, mup, dphi, brdf->hapke->b0, brdf->hapke->h, brdf->hapke->w);
    break;
  case BRDF_RPV:
    if (swvnmlo != wvnmlo || swvnmhi != wvnmhi || srho0 != brdf->rpv->rho0 || sk != brdf->rpv->k || stheta != brdf->rpv->theta ||
        ssigma != brdf->rpv->sigma || st1 != brdf->rpv->t1 || st2 != brdf->rpv->t2 || sscale != brdf->rpv->scale) {
  ...
  case BRDF_CAM:
     ...
     */
    ans = ocean_brdf (wvnmlo, wvnmhi, mu, mup, dphi, brdf->cam->u10, brdf->cam->pcl, brdf->cam->xsal, callnum);
    ...
    if (ans != ans) {
      if (!quiet)
        fprintf (stderr,
                 "NaN returned from ocean_brdf: %e %e %e %e %e %e %e %e\n",
                 wvnmlo,
                 wvnmhi,
                 mu,
                 mup,
  ...
  case BRDF_ROSSLI:
    ...
    if (siso != brdf->rossli->iso || svol != brdf->rossli->vol || sgeo != brdf->rossli->geo || shot != brdf->rossli->hotspot) {

      siso = brdf->rossli->iso;
      svol = brdf->rossli->vol;
      sgeo = brdf->rossli->geo;
      shot = brdf->rossli->hotspot;

      badmu = 0.01; /* badmu=0 is not sensible, because functions diverge at mu=0 */

      for (irmu = 100; irmu >= 1; irmu--) {
        ...
        if (flxalb < 0.0 || flxalb > 1.0) {
          ...
          if (badmu > 1.0)
            badmu = 1.0;
          if (quiet < 2)
            fprintf (stderr, "Using %f as limiting mu in RossLi \n", badmu);
          break;
        }
      }
    }
...
#if 1
    ans = c_bidir_reflectivity_rossli (brdf->rossli, mu, mup, dphi);
#else
    // former version
#if HAVE_BRDF
    ans = ambrals_brdf (brdf->rossli->iso, brdf->rossli->vol, brdf->rossli->geo, mu, mup, dphi * 180. / M_PI);
#else
    ...
    if (ans != ans) {
      if (!quiet)
        fprintf (stderr,
                 "NaN returned from brdf_rossli: %e %e %e %e %e %e %e %e\n",
                 wvnmlo,
                 wvnmhi,
                 mu,
                 mup,
    ...
    break;
  default:
    fprintf (stderr, "bidir_reflectivity--surface BDRF model %d not known", brdf_type);
...
double
c_bidir_reflectivity_hapke (double wvnmlo, double wvnmhi, double mu, double mup, double dphi, double b0, double hh, double w) {
  double b, ctheta, Xgamm, h0, h, p, thetah, brdf;
  ...
  brdf = .25 * w * ((1. + b) * p + h0 * h - 1.0) / (mu + mup);
...
/*============================= c_bidir_reflectivity_rossli() ============*/

/*    
 * Ross-Li BRDF:
 * 
 * Lin et al., Improved discrete ordinate solutions in the presence of an
                  anisotropically reflecting lower boundary: Upgrades
                  of the DISORT computational tool, JQSRT, 
                 http://dx.doi.org/10.1016/j.jqsrt.2015.02.014
  ...
  INPUT:
    ...
    dphi   : Relative azimuth to incident vector; (pi - dphi), sun-view relative
             ...
             at phi = 0Difference of azimuth angles of incidence and reflection
             (radians)

   iso     : BRDF parameter, isotropic scattering kernel
   vol     : BRDF parameter, volume scattering kernel
   geo     : BRDF parameter, geometry scattering kernel
...
double c_bidir_reflectivity_rossli (rossli_brdf_spec* brdf, double mu, double mup, double dphi) {
  double alpha0, ratio_hb, ratio_br, sin_i, sin_r, tan_i, tan_r, cos_alpha, sin_alpha, alpha, c, f_vol, tan_i1, tan_r1, sin_i1,
    sin_r1, cos_i1, cos_r1, cos_alpha1, g_sq, cos_t, t, f_geo, ans;
  mu  = fabs (mu);
  mup = fabs (mup);
  ...
  ratio_br  = 1;
  sin_i     = sqrt (1. - mu * mu);
  sin_r     = sqrt (1. - mup * mup);
  tan_i     = sin_i / mu;
  tan_r     = sin_r / mup;
  cos_alpha = mu * mup - sin_i * sin_r * cos (dphi);
  ...
  f_vol = 4. / (3. * M_PI) * (1. / (mu + mup)) * ((M_PI / 2. - alpha) * cos_alpha + sin_alpha) * c - 1. / 3.;
  //this formula is consistent with ambralsfor.c:
  //f_vol = (1./(mu+mup))*((M_PI/2. - alpha)*cos_alpha+sin_alpha)*c - M_PI/4.;
  ...
  cos_alpha1 = cos_i1 * cos_r1 - sin_i1 * sin_r1 * cos (dphi);

  g_sq = tan_i1 * tan_i1 + tan_r1 * tan_r1 + 2. * tan_i1 * tan_r1 * cos (dphi);

  cos_t = ratio_hb * (cos_i1 * cos_r1) / (cos_i1 + cos_r1) * sqrt (g_sq + pow ((tan_i1 * tan_r1 * sin (dphi)), 2));
  ...
  f_geo =
    (cos_i1 + cos_r1) / (M_PI * cos_i1 * cos_r1) * (t - sin (t) * cos (t) - M_PI) + (1. + cos_alpha1) / (2. * cos_i1 * cos_r1);

  /* Compute BRDF */

  ans = M_PI * (brdf->iso + brdf->geo * f_geo + brdf->vol * f_vol);
  //this formula is consistent with ambralsfor.c:
  //ans = brdf->iso + brdf->geo*f_geo + brdf->vol*f_vol;
...
/*============================= end of c_bidir_reflectivity_rossli() =====*/
...
double c_bidir_reflectivity_rpv (rpv_brdf_spec* brdf, double mu1, double mu2, double phi, double badmu) {
  double m, f, h, cosphi, sin1, sin2, cosg, tan1, tan2, capg, hspot, t, g;
  double ans;
  ...
  /* Hot spot */
  hspot = brdf->rho0 *
          (pow (2.0 * mu1 * mu1 * mu1, brdf->k - 1.0) * (1.0 - brdf->theta) / (1.0 + brdf->theta) / (1.0 + brdf->theta) *
             (2.0 - brdf->rho0) +
           brdf->sigma / mu1) *
          (brdf->t1 * exp (M_PI * brdf->t2) + 1.0);
  ...
  g      = acos (cosg);
  f      = (1.0 - brdf->theta * brdf->theta) / pow (1.0 + 2.0 * brdf->theta * cosg + brdf->theta * brdf->theta, 1.5);
...
void c_getmom (int iphas, double gg, int nmom, double* pmom) {
  const double cldmom[299] = {
    2.544,  3.883,  4.568,  5.235,  5.887,  6.457,  7.177,  7.859,  8.494,  9.286,  9.856,  10.615, 11.229, 11.851, 12.503, 13.058,
    13.626, 14.209, 14.660, 15.231, 15.641, 16.126, 16.539, 16.934, 17.325, 17.673, 17.999, 18.329, 18.588, 18.885, 19.103, 19.345,
    19.537, 19.721, 19.884, 20.024, 20.145, 20.251, 20.330, 20.401, 20.444, 20.477, 20.489, 20.483, 20.467, 20.427, 20.382, 20.310,
    20.236, 20.136, 20.036, 19.909, 19.785, 19.632, 19.486, 19.311, 19.145, 18.949, 18.764, 18.551, 18.348, 18.119, 17.901, 17.659,
    17.428, 17.174, 16.931, 16.668, 16.415, 16.144, 15.883, 15.606, 15.338, 15.058, 14.784, 14.501, 14.225, 13.941, 13.662, 13.378,
    13.098, 12.816, 12.536, 12.257, 11.978, 11.703, 11.427, 11.156, 10.884, 10.618, 10.350, 10.090, 9.827,  9.574,  9.318,  9.072,
    8.822,  8.584,  8.340,  8.110,  7.874,  7.652,  7.424,  7.211,  6.990,  6.785,  6.573,  6.377,  6.173,  5.986,  5.790,  5.612,
    5.424,  5.255,  5.075,  4.915,  4.744,  4.592,  4.429,  4.285,  4.130,  3.994,  3.847,  3.719,  3.580,  3.459,  3.327,  3.214,
    3.090,  2.983,  2.866,  2.766,  2.656,  2.562,  2.459,  2.372,  2.274,  2.193,  2.102,  2.025,  1.940,  1.869,  1.790,  1.723,
    1.649,  1.588,  1.518,  1.461,  1.397,  1.344,  1.284,  1.235,  1.179,  1.134,  1.082,  1.040,  0.992,  0.954,  0.909,  0.873,
    0.832,  0.799,  0.762,  0.731,  0.696,  0.668,  0.636,  0.610,  0.581,  0.557,  0.530,  0.508,  0.483,  0.463,  0.440,  0.422,
    0.401,  0.384,  0.364,  0.349,  0.331,  0.317,  0.301,  0.288,  0.273,  0.262,  0.248,  0.238,  0.225,  0.215,  0.204,  0.195,
    0.185,  0.177,  0.167,  0.160,  0.151,  0.145,  0.137,  0.131,  0.124,  0.118,  0.112,  0.107,  0.101,  0.097,  0.091,  0.087,
    0.082,  0.079,  0.074,  0.071,  0.067,  0.064,  0.060,  0.057,  0.054,  0.052,  0.049,  0.047,  0.044,  0.042,  0.039,  0.038,
    0.035,  0.034,  0.032,  0.030,  0.029,  0.027,  0.026,  0.024,  0.023,  0.022,  0.021,  0.020,  0.018,  0.018,  0.017,  0.016,
    0.015,  0.014,  0.013,  0.013,  0.012,  0.011,  0.011,  0.010,  0.009,  0.009,  0.008,  0.008,  0.008,  0.007,  0.007,  0.006,
    0.006,  0.006,  0.005,  0.005,  0.005,  0.005,  0.004,  0.004,  0.004,  0.004,  0.003,  0.003,  0.003,  0.003,  0.003,  0.003,
    0.002,  0.002,  0.002,  0.002,  0.002,  0.002,  0.002,  0.002,  0.002,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,
    0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001,  0.001};
  const double hazelm[82] = {
    2.41260, 3.23047, 3.37296, 3.23150, 2.89350, 2.49594, 2.11361, 1.74812, 1.44692, 1.17714, 0.96643, 0.78237, 0.64114, 0.51966,
    0.42563, 0.34688, 0.28351, 0.23317, 0.18963, 0.15788, 0.12739, 0.10762, 0.08597, 0.07381, 0.05828, 0.05089, 0.03971, 0.03524,
    0.02720, 0.02451, 0.01874, 0.01711, 0.01298, 0.01198, 0.00904, 0.00841, 0.00634, 0.00592, 0.00446, 0.00418, 0.00316, 0.00296,
    0.00225, 0.00210, 0.00160, 0.00150, 0.00115, 0.00107, 0.00082, 0.00077, 0.00059, 0.00055, 0.00043, 0.00040, 0.00031, 0.00029,
    0.00023, 0.00021, 0.00017, 0.00015, 0.00012, 0.00011, 0.00009, 0.00008, 0.00006, 0.00006, 0.00005, 0.00004, 0.00004, 0.00003,
    0.00003, 0.00002, 0.00002, 0.00002, 0.00001, 0.00001, 0.00001, 0.00001, 0.00001, 0.00001, 0.00001, 0.00001};
  register int k;
...
void c_asymmetric_matrix (double* aa, double* evec, double* eval, int m, int ia, int ievec, int* ier, double* wk) {
  const double c1 = .4375, c2 = .5, c3 = .75, c4 = .95, c5 = 16., c6 = 256.;
  int          noconv, notlas, i, ii, in, j, k, ka, kkk, l, lb = 0, lll, n, n1, n2;
  double       col, discri, f, g, h, p = 0, q = 0, r = 0, repl, rnorm, row, s, scale, sgn, t, tol, uu, vv, w, x, y, z;
...
void c_intensity_components (disort_state* ds,
                             ...
                             disort_pair*  plk,
                             double*       uum) {
  register int    iq, jq, lu, lyu;
  register double zint;
...
void c_fluxes (disort_state*  ds,
               ...
               double*        u0c) {
  register int iq, jq, lu, lyu;
  double       ang1, ang2, dirint, fact = 0, fdntot, fnet, plsorc, zint;
  ...
  for (lu = 1; lu <= ds->ntau; lu++) {
    ...
    if (lyrcut && lyu > ncut) {
      ...
      if (ds->flag.prnt[1]) {
        fprintf (stdout,
                 "%10.4f%7d%12.3e%12.3e%12.3e%12.3e%12.3e%12.3e%12.3e%14.3e\n",
                 UTAU (lu),
                 lyu,
                 RFLDIR (lu),
                 RFLDN (lu),
    ...
    for (iq = 1; iq <= nn; iq++) {
      ...
      if (ds->bc.fbeam > 0.) {
        if (ds->flag.spher == TRUE) {
          U0C (iq, lu) += exp (-ZBEAMA (lyu) * UTAUPR (lu)) * (ZBEAM0 (iq, lyu) + ZBEAM1 (iq, lyu) * UTAUPR (lu));
        } else {
  ...
  if (prntu0) {
    ...
    for (lu = 1; lu <= ds->ntau; lu++) {
      ...
      for (iq = 1; iq <= nn; iq++) {
        ...
        fprintf (stdout,
                 "%16.4f%13.5f%14.3e%16.4f%13.5f%14.3e\n",
                 ang1,
                 CMU (2 * nn - iq + 1),
                 U0C (iq, lu),
                 ang2,
...
void c_intensity_correction (disort_state*  ds,
                             ...
                             double*        taucpr,
                             double*        utaupr) {
  register int iu, jp, k, lc, ltau, lu;
  double       ctheta, dtheta, duims, pl, plm1, plm2, theta0 = 0, thetap = 0, ussndm, ussp;
    ...
    for (jp = 1; jp <= ds->nphi; jp++) {
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
        if (!lyrcut || LAYRU (lu) < ncut) {
          ussndm =
            c_single_scat (dither, LAYRU (lu), ncut, phast, ds->ssalb, taucpr, UMU (iu), ds->bc.umu0, UTAUPR (lu), ds->bc.fbeam);
...
double c_secondary_scat (disort_state* ds, int iu, int lu, double ctheta, double* flyr, int layru, double* tauc) {
  ...
  double          dtau, fbar, gbar, pl, plm1, plm2, pspike, stau, umu0p, wbar;
  register double tmp;
  ...
  if (need_secondary_scattering == TRUE) {
	 ...
	 functions (phas2) */
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
        ...
        for (lyr = 1; lyr <= LAYRU (lu) - 1; lyr++)
          PHAS2 (it, lu) += (DSPHASE (it, lyr) - PHASR (lyr)) * SSALB (lyr) * DTAUC (lyr);

        lyr = LAYRU (lu);
        PHAS2 (it, lu) += (DSPHASE (it, lyr) - PHASR (lyr)) * SSALB (lyr) * (UTAU (lu) - TAUC (lyr - 1));
      }
    ...
    for (lu = 1; lu <= ds->ntau; lu++) {
      ...
      for (it = 2; it <= ds->nphase; it++)
        f_phas2 += (ds->MUP (it) - ds->MUP (it - 1)) * 0.5 * (PHAS2 (it, lu) + PHAS2 (it - 1, lu));
    ...
    prep_double_scat_integr (ds->nphase, ds->ntau, nf, ds->mu_phase, phas2, mu_eq, neg_phas, norm_phas);
    ...
    for (jp = 1; jp <= ds->nphi; jp++) {
      ...
      for (lc = 1; lc <= ncut; lc++)
        PHASA (lc) = DSPHASE (it, lc) +
                     (ctheta - ds->MUP (it)) / (ds->MUP (it + 1) - ds->MUP (it)) * (DSPHASE (it + 1, lc) - DSPHASE (it, lc));
      /*
       * Initialize Legendre poly. recurrence
       */
      plm1 = 1.;
      ...
      if (UMU (iu) < 0. && fabs (theta0 - thetap) <= dtheta) {
        ...
        for (lu = ltau; lu <= ds->ntau; lu++) {
          if (!lyrcut || LAYRU (lu) < ncut) {
            duims =
              c_new_secondary_scat (ds, iu, lu, it, ctheta, flyr, LAYRU (lu), tauc, nf, phas2, mu_eq, neg_phas, NORM_PHAS (lu));
            UU (iu, lu, jp) -= duims;
          }
        }
      }
...
void prep_double_scat_integr (int     nphase,
  ...
  for (lu = 1; lu <= ntau; lu++) {
    ...
    for (it = 2; it <= nphase; it++)
      F_PHAS2_ABS (it) =
        F_PHAS2_ABS (it - 1) + (MUP (it) - MUP (it - 1)) * 0.5 * (fabs (PHAS2 (it, lu)) + fabs (PHAS2 (it - 1, lu)));
    ...
    for (i = 2; i <= nf - 1; i++) {
      ...
      MU_EQ (i, lu) =
        MUP (it) + (f_phas2 - F_PHAS2_ABS (it)) / (F_PHAS2_ABS (it + 1) - F_PHAS2_ABS (it)) * (MUP (it + 1) - MUP (it));
      ...
      if (PHAS2 (it, lu) > 0.0 && PHAS2 (it + 1, lu) > 0.0)
        ...
        if (PHAS2 (it, lu) < 0.0 && PHAS2 (it + 1, lu) < 0.0)
          ...
          if (PHAS2 (it, lu) +
                (f_phas2 - F_PHAS2_ABS (it)) / (F_PHAS2_ABS (it + 1) - F_PHAS2_ABS (it)) * (PHAS2 (it + 1, lu) - PHAS2 (it, lu)) >
              0.0)
            NEG_PHAS (i, lu) = FALSE;
          else
            NEG_PHAS (i, lu) = TRUE;
...
double c_new_secondary_scat (disort_state* ds,
  ...
  register int lyr;
  const double tiny = 1.e-4;
  double       dtau, fbar, pspike, stau, umu0p, wbar;
  int          nphase = ds->nphase;
  ...
  pspike1 = PHAS2 (it, lu) + (ctheta - ds->MUP (it)) / (ds->MUP (it + 1) - ds->MUP (it)) * (PHAS2 (it + 1, lu) - PHAS2 (it, lu));

  pspike2 = calc_phase_squared (ds->nphase, lu, ctheta, nf, ds->mu_phase, phas2, mu_eq, neg_phas, norm_phas);
  ...
  for (j = 1; j <= nf; j++) {
    ...
    if (ctheta == 1.0 || MU_EQ (j, lu) == 1.0) {
      ...
      phint = M_PI * (PHAS2 (it, lu) +
                      (MU_EQ (j, lu) * ctheta - MUP (it)) / (MUP (it + 1) - MUP (it)) * (PHAS2 (it + 1, lu) - PHAS2 (it, lu)));
      if (ctheta == 1.0)
        phint /= 2.0;
    } else {
      phint = 0.0;
      ...
      if (mumin < mumax) {
        ...
        for (k = imin + 1; k <= imax; k++) {
          ...
          phint += (Dp - D) * sqrt (1.0 - ctheta * ctheta - MU_EQ (j, lu) * MU_EQ (j, lu) + 2.0 * ctheta * MU_EQ (j, lu) * MUP (k) -
                                    MUP (k) * MUP (k)) +
                   ((Dp - D) * ctheta * MU_EQ (j, lu) + Cp - C) * asin ((ctheta * MU_EQ (j, lu) - MUP (k)) / (smueq * stheta));
        ...
        if (cutting == TRUE)
          phint += -D * sqrt (1.0 - ctheta * ctheta + 2.0 * MU_EQ (j, lu) * MU_EQ (j, lu) * (ctheta - 1.0)) -
                   (D * ctheta * MU_EQ (j, lu) + C) * asin ((ctheta - 1.0) * MU_EQ (j, lu) / (smueq * stheta));
        else
          phint += (D * ctheta * MU_EQ (j, lu) + C) * M_PI / 2.0;
      }
    }
...
void c_disort_set (disort_state* ds,
                   ...
                   double*       taucpr,
                   double*       utaupr) {
  register int iq, iu, k, lc, lu;
  const double abscut = 10.;
  double       abstau, chtau_tmp, f, taup, zenang;
   ...
   * ch for beam source.
   */
  if ((ds->flag.ibcnd == GENERAL_BC && ds->bc.fbeam > 0.) || (ds->flag.ibcnd == GENERAL_BC && ds->flag.general_source)) {
    ...
    if (ds->flag.spher == TRUE && ds->bc.umu0 < 0.) {
      EXPBEA (0) = exp (-c_chapman (1, 0., tauc, ds->nlyr, ds->zd, ds->dtauc, zenang, ds->radius));
    }
    if (ds->flag.spher == TRUE) {
      for (lc = 1; lc <= *ncut; lc++) {
        taup = TAUCPR (lc - 1) + DTAUCPR (lc) / 2.;
        /* Need Chapman function at top (0.0) and middle (0.5) of layer */
        CHTAU (lc)  = c_chapman (lc, 0., taucpr, ds->nlyr, ds->zd, dtaucpr, zenang, ds->radius);
        chtau_tmp   = c_chapman (lc, 0.5, taucpr, ds->nlyr, ds->zd, dtaucpr, zenang, ds->radius);
...
void c_set_matrix (disort_state* ds,
                   ...
                   double*       taucpr,
                   double*       wk) {
  int          mi = ds->nstr / 2, mi9m2 = 9 * mi - 2, nnlyri = ds->nstr * ds->nlyr, nn = ds->nstr / 2;
  register int iq, irow, jcol, jq, k, lc, lda, ncd, nncol, nshift;
  double       expa, sum;
...
void c_solve_eigen (disort_state* ds,
                    ...
                    double*       wk) {
  int          ier;
  register int iq, jq, kq, l;
  double       alpha, beta, gpmigm, gpplgm, sum;
...
void c_solve0 (disort_state* ds,
               ...
               double*       zzg,
               disort_pair*  plk) {
  register int ipnt, iq, it, jq, lc, ncd;
  double       rcond, sum, diff;
  ...
  if (mazim > 0 && (ds->bc.fbeam > 0. || ds->flag.general_source)) {
    ...
    if (lyrcut == TRUE || ds->flag.lamber == TRUE) {
      ...
      for (iq = 1; iq <= nn; iq++) {
        ...
        if (ds->flag.spher == TRUE) {
          B (ncol - nn + iq) =
            -exp (-ZBEAMA (ncut) * TAUCPR (ncut)) * (ZBEAM0 (iq + nn, ncut) + ZBEAM1 (iq + nn, ncut) * TAUCPR (ncut));
        } else {
      ...
      for (iq = 1; iq <= nn; iq++) {
        ...
        if (ds->flag.spher == TRUE) {
          c_errmsg ("solve0--BDR not implemented for pseudo-spherical geometry", DS_WARNING);
        } else {
          sum = 0.;
          for (jq = 1; jq <= nn; jq++) {
        ...
        if (ds->bc.fbeam > 0.) {
          if (ds->flag.spher == TRUE) {
            ...
            B (ncol - nn + iq) += (BDR (iq, 0) * ds->bc.umu0 * ds->bc.fbeam / M_PI - ZZ (iq + nn, ncut)) * EXPBEA (ncut);
          }
        }
        if (ds->flag.general_source == TRUE) {
          B (ncol - nn + iq) += -ZZG (iq + nn, ncut);
    ...
    for (lc = 1; lc <= ncut - 1; lc++) {
      for (iq = 1; iq <= ds->nstr; iq++) {
        ...
        if (ds->flag.spher == TRUE) {
          B (++it) = exp (-ZBEAMA (lc + 1) * TAUCPR (lc)) * (ZBEAM0 (iq, lc + 1) + ZBEAM1 (iq, lc + 1) * TAUCPR (lc)) -
                     exp (-ZBEAMA (lc) * TAUCPR (lc)) * (ZBEAM0 (iq, lc) + ZBEAM1 (iq, lc) * TAUCPR (lc)) + diff;
        } else {
    ...
    if (ds->bc.fbeam == 0. && ds->flag.general_source == FALSE) {
      ...
      if (lyrcut == TRUE) {
        ...
        for (iq = 1; iq <= nn; iq++) {
          sum = 0.;
          for (jq = 1; jq <= nn; jq++) {
            sum += CWT (jq) * CMU (jq) * BDR (iq, jq) * (ZPLK0 (nn + 1 - jq, ncut) + ZPLK1 (nn + 1 - jq, ncut) * TAUCPR (ncut));
          }
          B (ncol - nn + iq) = 2. * sum + BEM (iq) * bplanck - ZPLK0 (iq + nn, ncut) - ZPLK1 (iq + nn, ncut) * TAUCPR (ncut);
        }
      }
      /*
      ...
      for (lc = 1; lc <= ncut - 1; lc++) {
        for (iq = 1; iq <= ds->nstr; iq++) {
          B (++it) = ZPLK0 (iq, lc + 1) - ZPLK0 (iq, lc) + (ZPLK1 (iq, lc + 1) - ZPLK1 (iq, lc)) * TAUCPR (lc);
        }
      }
    } else {
      if (ds->flag.spher == TRUE) {
      ...
      if (lyrcut) {
        if (ds->flag.spher == TRUE) {
          ...
          for (iq = 1; iq <= nn; iq++) {
            B (ncol - nn + iq) =
              -ZZ (iq + nn, ncut) * EXPBEA (ncut) - ZPLK0 (iq + nn, ncut) - ZPLK1 (iq + nn, ncut) * TAUCPR (ncut);
          }
        }
        if (ds->flag.general_source == TRUE) {
          for (iq = 1; iq <= nn; iq++)
        ...
        if (ds->flag.spher == TRUE) {
          for (iq = 1; iq <= nn; iq++) {
            ...
            for (jq = 1; jq <= nn; jq++) {
              sum +=
                CWT (jq) * CMU (jq) * BDR (iq, jq) *
                (exp (-ZBEAMA (ncut) * TAUCPR (ncut)) * (ZBEAM0 (nn + 1 - jq, ncut) + ZBEAM1 (nn + 1 - jq, ncut) * TAUCPR (ncut)) +
                 ZZG (nn + 1 - jq, ncut) + ZPLK0 (nn + 1 - jq, ncut) + ZPLK1 (nn + 1 - jq, ncut) * TAUCPR (ncut));
            }
            B (ncol - nn + iq) =
              2.0 * sum + (BDR (iq, 0) * ds->bc.umu0 * ds->bc.fbeam / M_PI) * EXPBEA (ncut) -
              exp (-ZBEAMA (ncut) * TAUCPR (ncut)) * (ZBEAM0 (iq + nn, ncut) + ZBEAM1 (iq + nn, ncut) * TAUCPR (ncut)) -
              ZZG (iq + nn, ncut) + BEM (iq) * bplanck - ZPLK0 (iq + nn, ncut) - ZPLK1 (iq + nn, ncut) * TAUCPR (ncut) +
              ds->bc.fluor;
          }
        } else {
          for (iq = 1; iq <= nn; iq++) {
            ...
            for (jq = 1; jq <= nn; jq++) {
              sum += CWT (jq) * CMU (jq) * BDR (iq, jq) *
                     (ZZ (nn + 1 - jq, ncut) * EXPBEA (ncut) + ZPLK0 (nn + 1 - jq, ncut) + ZZG (nn + 1 - jq, ncut) +
                      ZPLK1 (nn + 1 - jq, ncut) * TAUCPR (ncut));
            }
            B (ncol - nn + iq) = 2. * sum + (BDR (iq, 0) * ds->bc.umu0 * ds->bc.fbeam / M_PI - ZZ (iq + nn, ncut)) * EXPBEA (ncut) -
                                 ZZG (iq + nn, ncut) + BEM (iq) * bplanck - ZPLK0 (iq + nn, ncut) -
                                 ZPLK1 (iq + nn, ncut) * TAUCPR (ncut) + ds->bc.fluor;
          }
        }
      }
      it = nn;
      if (ds->flag.spher == TRUE) {
        for (lc = 1; lc <= ncut - 1; lc++) {
          for (iq = 1; iq <= ds->nstr; iq++) {
            B (++it) = exp (-ZBEAMA (lc + 1) * TAUCPR (lc)) * (ZBEAM0 (iq, lc + 1) + ZBEAM1 (iq, lc + 1) * TAUCPR (lc)) -
                       exp (-ZBEAMA (lc) * TAUCPR (lc)) * (ZBEAM0 (iq, lc) + ZBEAM1 (iq, lc) * TAUCPR (lc)) + ZZG (iq, lc + 1) -
                       ZZG (iq, lc) + ZPLK0 (iq, lc + 1) - ZPLK0 (iq, lc) + (ZPLK1 (iq, lc + 1) - ZPLK1 (iq, lc)) * TAUCPR (lc);
          }
        }
      } else {
        for (lc = 1; lc <= ncut - 1; lc++) {
          for (iq = 1; iq <= ds->nstr; iq++) {
            B (++it) = (ZZ (iq, lc + 1) - ZZ (iq, lc)) * EXPBEA (lc) + ZZG (iq, lc + 1) - ZZG (iq, lc) + ZPLK0 (iq, lc + 1) -
                       ZPLK0 (iq, lc) + (ZPLK1 (iq, lc + 1) - ZPLK1 (iq, lc)) * TAUCPR (lc);
          }
        }
      }
    }
...
void c_surface_bidir (disort_state* ds,
                      ...
                      double*       rmu,
                      int           callnum) {
  static int    pass1 = TRUE;
  register int  iq, iu, jg, jq, k;
  double        dref, sum;
  static double gmu[NMUG], gwt[NMUG];
  ...
  if (ds->flag.lamber && mazim == 0) {
    ...
    for (iq = 1; iq <= nn; iq++) {
      for (jq = 1; jq <= nn; jq++) {
        ...
        for (k = 1; k <= NMUG; k++) {
          sum += GWT (k) *
                 c_bidir_reflectivity (ds->wvnmlo,
                                       ds->wvnmhi,
                                       CMU (iq),
                                       CMU (jq),
                                       M_PI * GMU (k),
...
void c_interp_source (disort_state*   ds,
                      ...
                      double*         zgu,
                      disort_pair*    zu) {
  register int iq, iu, jq;
  double       fact, psum, psum0, psum1, sum, sum0, sum1;
...
void c_interp_coefficients_beam_source (disort_state*   ds,
                                        ...
                                        double*         ylmu) {
  register int iu, k;
  double       deltat, sum, q0a, q2a, q0, q2;
  ...
  for (iu = 1; iu <= numu; iu++) {
    ...
    ZB1U (iu, lc) = (1. / deltat) * (q2 * exp (XBA (lc) * TAUCPR (lc)) - q0 * exp (XBA (lc) * TAUCPR (lc - 1)));
...
void c_set_coefficients_beam_source (disort_state* ds,
  ...
  if (fabs (XBA (lc)) > big && TAUCPR (lc) > 1.)
    XBA (lc) = 0.0;

  if (fabs (XBA (lc) * TAUCPR (lc)) > log (big))
    XBA (lc) = 0.0;
  ...
  if (fabs (XBA (lc)) > 0.00001) {
    for (iq = 1; iq <= nstr / 2; iq++) {
      if (fabs ((fabs (XBA (lc)) - 1.0 / CMU (iq)) / XBA (lc)) < 0.05)
        XBA (lc) = XBA (lc) * 1.001;
    }
  }
...
void c_upbeam_pseudo_spherical (disort_state* ds,
  ...
  if (1. + rcond == 1.) {
    c_errmsg ("upbeam_pseudo_spherical--sgeco says matrix near singular", DS_WARNING);
  }
...
void c_user_intensities (disort_state*   ds,
                         ...
                         disort_pair*    plk,
                         double*         uum) {
  register int negumu, iq, iu, jq, lc, lu, lyrend, lyrstr, lyu;
  double       alfa, bnddfu, bnddir, bndint, denom, dfuint, dtau, dtau1, dtau2, exp0 = 0, exp1 = 0, exp2 = 0, expn, f0n, f1n, fact,
                                                                          genint, palint, plkint, sgn;
    ...
    for (iu = 1; iu <= ds->numu; iu++) {
      ...
      for (lc = lyrstr; lc <= lyrend; lc++) {
        ...
        if (ds->flag.planck && mazim == 0) {
           ...
           */
          f0n = sgn * (exp1 - exp2);
          f1n = sgn * ((TAUCPR (lc - 1) + UMU (iu)) * exp1 - (TAUCPR (lc) + UMU (iu)) * exp2);
        ...
        if (ds->bc.fbeam > 0.) {
          if (ds->flag.spher == TRUE) {
            ...
            palint +=
              (ZB0U (iu, lc) * denom * (exp (-ZBAU (iu, lc) * TAUCPR (lc - 1)) * exp1 - exp (-ZBAU (iu, lc) * TAUCPR (lc)) * exp2) +
               ZB1U (iu, lc) * denom *
                 ((TAUCPR (lc - 1) + sgn * denom * UMU (iu)) * exp (-ZBAU (iu, lc) * TAUCPR (lc - 1)) * exp1 -
                  (TAUCPR (lc) + sgn * denom * UMU (iu)) * exp (-ZBAU (iu, lc) * TAUCPR (lc)) * exp2));
          } else {
      ...
      if ((fabs (dtau1) >= 1.e-6 || !negumu) && (fabs (dtau2) >= 1.e-6 || negumu)) {
        ...
        if (ds->bc.fbeam > 0.) {
          if (ds->flag.spher == TRUE) {
            if (negumu) {
              ...
              palint += ZB0U (iu, lyu) * denom * (-exp (-alfa * UTAUPR (lu)) + expn * exp (-alfa * TAUCPR (lyu - 1))) +
                        ZB1U (iu, lyu) * denom *
                          (-(UTAUPR (lu) - UMU (iu) * denom) * exp (-alfa * UTAUPR (lu)) +
                           (TAUCPR (lyu - 1) - UMU (iu) * denom) * expn * exp (-alfa * TAUCPR (lyu - 1)));
            } else {
              expn  = exp2;
              alfa  = ZBAU (iu, lyu);
              denom = (1.0 / (alfa * UMU (iu) + 1.0));
              palint += ZB0U (iu, lyu) * denom * (exp (-alfa * UTAUPR (lu)) - exp (-alfa * TAUCPR (lyu)) * expn) +
                        ZB1U (iu, lyu) * denom *
                          ((UTAUPR (lu) + UMU (iu) * denom) * exp (-alfa * UTAUPR (lu)) -
        ...
        for (iq = 1; iq <= nn; iq++) {
          ...
          if (fabs (denom) < 1.e-7) {
            ...
            expn = (exp (-KK (iq, lyu) * dtau2) - exp (KK (iq, lyu) * dtau) * exp1) / denom;
          } else {
        ...
        for (iq = nn + 1; iq <= ds->nstr; iq++) {
          ...
          if (fabs (denom) < 1.e-7) {
            ...
            expn = (exp (-KK (iq, lyu) * dtau1) - exp (-KK (iq, lyu) * dtau) * exp2) / denom;
          }
          palint += GU (iu, iq, lyu) * expn;
        }
      ...
      if (negumu && mazim == 0) {
        ...
        for (iq = nn; iq >= 1; iq--) {
          ...
          if (ds->bc.fbeam > 0.) {
            if (ds->flag.spher == TRUE) {
              dfuint +=
                exp (-ZBEAMA (ds->nlyr) * TAUCPR (ds->nlyr)) * (ZBEAM0 (iq, ds->nlyr) + ZBEAM1 (iq, ds->nlyr) * TAUCPR (ds->nlyr));
            } else {
...
void c_check_inputs (disort_state* ds, int scat_yes, int deltam, int corint, double* tauc, int callnum) {
  int          inperr = FALSE;
  register int iu, j, k, lc, lu, nu;
  /*CE: not used int irmu */
  double umumin;
  /* CE: not used flxalb, rmu */
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    ...
    if (ds->flag.lamber) {
      ...
      /* commented out BRDF test because it uses a lot of computational time; BM, Tobias Koelling, 30.11.2016 */
      /* 
      ...
      */
      if (!ds->flag.quiet)
        c_errmsg ("check_inputs--BRDF not tested to save computational time", DS_WARNING);
    }
  } else {
    if (ds->bc.albedo < 0. || ds->bc.albedo > 1.) {
      inperr = c_write_bad_var (VERBOSE, "ds.bc.albedo");
  ...
  if (ds->flag.planck && ds->flag.ibcnd != SPECIAL_BC) {
    if (ds->wvnmlo < 0. || ds->wvnmhi <= ds->wvnmlo) {
      ...
      fprintf (stderr, "ds.wvnmlo=%f, ds.wvnmhi=%f\n", ds->wvnmlo, ds->wvnmhi);
    }
...
double c_dref (double wvnmlo, double wvnmhi, double mu, int brdf_type, disort_brdf* brdf, int callnum) {
  ...
  for (jg = 1; jg <= NMUG; jg++) {
    ...
    sum = 0.;
    for (k = 1; k <= NMUG / 2; k++) {
      sum += GWT (k) * GMU (k) * c_bidir_reflectivity (wvnmlo, wvnmhi, GMU (k), mu, M_PI * GMU (jg), brdf_type, brdf, callnum, 1);
    }
    ans += GWT (jg) * sum;
  }
  if (ans < 0. || ans > 1.) {
    if (mu > 0.0) /* mu=0 is a case that often gives bad results */
...
void c_print_avg_intensities (disort_state* ds, disort_output* out) {
  register int iu, iumax, iumin, lenfmt, lu, np, npass;
...
void c_print_inputs (disort_state* ds,
  ...
  /*CE:  ds->header is not initialized at this point, therefore removed it from fprintf */
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    ...
    fprintf (stdout,
             "    Incident beam with intensity =%11.3e and polar angle cosine = %8.5f  and azimuth angle =%7.2f\n",
             ds->bc.fbeam,
             ds->bc.umu0,
             ds->bc.phi0);
  ...
  for (lc = 1; lc <= ds->nlyr; lc++) {
    ...
    if (ds->flag.planck) {
      fprintf (stdout,
               "%4d%10.4f%10.4f%10.5f%12.5f%10.4f%10.4f%10.5f%9.4f%14.3f\n",
               lc,
               DTAUC (lc),
               TAUC (lc),
               SSALB (lc),
      ...
      fprintf (stdout,
               "%4d%10.4f%10.4f%10.5f%12.5f%10.4f%10.4f%10.5f%9.4f\n",
               lc,
               DTAUC (lc),
               TAUC (lc),
               SSALB (lc),
...
void c_print_intensities (disort_state* ds, disort_output* out) {
  register int iu, j, jmax, jmin, lenfmt, lu, np, npass;
...
void c_gaussian_quadrature (int m, double* gmu, double* gwt) {
  static int    initialized = FALSE;
  register int  iter, k, lim, nn, np1;
  double        cona, t, en, nnp1, p = 0, p2pri, pm1, pm2, ppr, prod, tmp, x, xi;
  static double tol;
...
double c_ratio (double a, double b) {
  static int    initialized = FALSE;
  static double tiny, huge, powmax, powmin;
  double        ans, absa, absb, powa, powb;
...
void c_albtrans (disort_state*  ds,
                 ...
                 double*        wk) {
  int          lyrcut, ncol;
  register int iq, iu, l, lc, mazim, ncd, ncut;
  double       delm0, rcond, sgn, sphalb, sphtrn;
...
void c_albtrans_intensity (disort_state*  ds,
                           ...
                           double*        taucpr,
                           double*        wk) {
  register int iq, iu, iumax, iumin, lc, lu;
  double       denom, dtau, exp1, exp2, expn, mu, palint, sgn, utaupr[2];
...
int c_write_bad_var (int quiet, char* varnam) {
  const int  maxmsg = 50;
  static int nummsg = 0;
...
int c_write_too_small_dim (int quiet, char* dimnam, int minval) {
  if (quiet != QUIET) {
...
void c_sgbco (double* abd, int lda, int n, int ml, int mu, int* ipvt, double* rcond, double* z) {
  int          info;
  register int is, j, ju, k, kb, kp1, l, la, lm, lz, m, mm;
...
void c_sgbfa (double* abd, int lda, int n, int ml, int mu, int* ipvt, int* info) {
  register int i0, j, j0, j1, ju, jz, k, kp1, l, lm, m, mm, nm1;
  double       t;
...
void c_sgbsl (double* abd, int lda, int n, int ml, int mu, int* ipvt, double* b, int job) {
  register int k, kb, l, la, lb, lm, m, nm1;
  double       t;
...
double c_sasum (int n, double* sx) {
   ...
   */
  for (i = m + 1; i <= n; i += 4) {
    ans += fabs (SX (i)) + fabs (SX (i + 1)) + fabs (SX (i + 2)) + fabs (SX (i + 3));
  }
...
double c_sdot (int n, double* sx, double* sy) {
   ...
   */
  for (i = m + 1; i <= n; i += 4) {
    ans += SX (i) * SY (i) + SX (i + 1) * SY (i + 1) + SX (i + 2) * SY (i + 2) + SX (i + 3) * SY (i + 3);
  }
...
void c_twostr (disort_state* ds, disort_output* out, int deltam, double* gg, int* ierror, double radius) {
  register int lc, ierr;
  int          lyrcut, iret, ncut, nn;
  int          ipvt[ds->nstr * ds->nlyr], layru[ds->ntau];
  double       cmu, bplanck, tplanck;
  double *b, *cband, *ch, *chtau, *dtaucpr, *expbea, *flyr, *ggprim, *kk, *ll, *oprim, *pkag, *pkagc, *rr, *tauc, *taucpr, *u0c,
    *utaupr;
  disort_pair* fl;
  twostr_xyz*  ts;
  twostr_diag* diag;
  ...
  fl = (disort_pair*)calloc (ds->ntau, sizeof (disort_pair));
  if (!fl)
    c_errmsg ("twostr alloc error for fl", DS_ERROR);
  ts = (twostr_xyz*)calloc (ds->nlyr, sizeof (twostr_xyz));
  if (!ts)
    c_errmsg ("twostr alloc error for ts", DS_ERROR);
  diag = (twostr_diag*)calloc (2 * ds->nlyr, sizeof (twostr_diag));
  if (!diag)
    c_errmsg ("twostr alloc error for diag", DS_ERROR);
...
double c_chapman (int lc, double taup, double* tauc, int nlyr, double* zd, double* dtau_c, double zenang, double r) {
  register int id, j;
  double       zenrad, xp, xpsinz, sum, fact, fact2, rj, rjp1, dhj, dsj;
...
void c_twostr_fluxes (disort_state*  ds,
                      ...
                      disort_pair*   fl) {
  register int    lu, lyu;
  double          fdntot, fnet, plsorc, dirint;
  register double fact1, fact2;
...
void c_twostr_solns (disort_state* ds,
  ...
  register int  lc;
  static int    initialized = FALSE;
  static double big, large, small, little;
  double        q_1, q_2, qq, q0a, q0, q1a, q2a, q1, q2, deltat, denomb, z0p, z0m, arg, sgn, fact3, denomp, beta, fact1, fact2;
...
void c_twostr_print_inputs (disort_state* ds,
  ...
  fprintf (stdout,
           "    Incident beam with intensity =%11.3e and polar angle cosine = %8.5f\n"
  ...
  if (ds->flag.planck) {
    fprintf (stdout,
             "    Thermal emission in wavenumber interval :%14.4f%14.4f\n"
             "    bottom temperature =%10.2f     top temperature =%10.2f    top emissivity =%8.4f\n",
             ds->wvnmlo,
             ds->wvnmhi,
             ds->bc.btemp,
...
void c_twostr_set (disort_state* ds,
  ...
  register int lc, lu, lev;
  double       zenang, abstau, chtau_tmp, f, tempc, taup, abscut = 10.;
...
void c_twostr_solve_bc (disort_state* ds,
                        ...
                        twostr_diag*  diag) {
  int             info;
  register int    irow, lc, nloop, nrow, job;
  double          wk0, wk1, wk, rpp1_m, rp_m, rpp1_p, rp_p, sum, refflx;
  register double fact1, fact2, fact3, fact4;
  ...
  if (lyrcut) {
    ...
    sum = cmu * ds->bc.albedo *
          (exp (-ZB_A (ncut) * TAUCPR (ncut)) * (YB_0D (ncut) + YB_1D (ncut) * TAUCPR (ncut)) +
    ...
    B (++irow) = 2. * sum + ds->bc.albedo * ds->bc.umu0 * ds->bc.fbeam / M_PI * refflx * EXPBEA (ncut) +
                 (1. - ds->bc.albedo) * bplanck -
...
double c_planck_func2 (double wnumlo, double wnumhi, double t) {
  register int  m, n, smallv, k, i, mmax;
  static int    initialized = FALSE;
  double        ans, del, val, val0, oldval, exm, ex, mv, vsq, wvn, arg, hh, d[2], p[2], v[2];
  const double  vcp[7] = {10.25, 5.7, 3.9, 2.9, 2.3, 1.9, 0.0};
  static double sigdpi, vmax, conc, c1;
...
void c_disort_state_alloc (disort_state* ds) {
   ...
   */
  ds->nmom_nstr = MAX (ds->nmom, ds->nstr);
...
#endif
  case BRDF_HAPKE:
    ds->brdf.hapke = (hapke_brdf_spec*)calloc (1, sizeof (hapke_brdf_spec));
    if (!ds->brdf.hapke) {
      c_errmsg ("calloc error for ds->brdf.hapke", DS_ERROR);
    }
    break;
  case BRDF_ROSSLI:
    ds->brdf.rossli = (rossli_brdf_spec*)calloc (1, sizeof (rossli_brdf_spec));
    if (!ds->brdf.rossli) {
      c_errmsg ("calloc error for ds->brdf.rossli", DS_ERROR);
    }
    break;
  default:;
    break;
...
void c_disort_out_alloc (disort_state* ds, disort_output* out) {
  ...
  if (ds->flag.ibcnd == SPECIAL_BC) {
    out->albmed = c_dbl_vector (0, 2 * ds->numu, "out->albmed");
    out->trnmed = c_dbl_vector (0, 2 * ds->numu, "out->trnmed");
  } else {
    out->albmed = NULL;
    out->trnmed = NULL;
  }
...
double* c_dbl_vector (int nl, int nh, char* name) {
...
int* c_int_vector (int nl, int nh, char* name) {
  unsigned int len_safe;
  int          nl_safe, nh_safe;
  int*         m;
...
int c_setout (float* sdtauc, int nlyr, int ntau, float* sutau, float* z, float* zout) {
  ...
  for (itau = 1; itau <= ntau; itau++)
    SUTAU (itau) = c_inter (nlyr + 1, itype, ZOUT (itau), z, tauint, &hh);
...
double c_inter (int npoints, int itype, double arg, float* xarr, double* yarr, double* hh) {
  ...
  case 1:
    ...
    if (YARR (ip + 1) == YARR (ip)) {
      ...
      *hh  = -(XARR (ip + 1) - XARR (ip)) / log (YARR (ip + 1) / YARR (ip));
