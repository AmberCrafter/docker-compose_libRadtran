...
#define MCALB_AMBRALS2D_SPECTRAL 6
#define MCALB_COXANDMUNK 7
#define MCALB_HAPKE 8
#define MCALB_ROSSLI 9
#define MCALB_ROSSLI2D 10
#define MCALB_TSANG 11
...
#define MCBACKWARD_EDNPV 18   /* diffuse irradiance on PV cell */
...
#define CLD_LWCREFFCF 4 /* liquid water content + effective radius + subpixel cloud fraction */
...
#define MCSTATUS_TRIANGLE_SURFACE 41
...
typedef struct {
  ...
  char spectral_albedo_filename[FILENAME_MAX];

  double** albedo2D;
  double   lambertian_albedo;  // the base case albedo as given in the input to mystic
  double   albedo;             // usually the base case albedo from above, except triangle surface may override it
  ...
  char spectral_rpv_filename[FILENAME_MAX];
  char spectral_ambrals_filename[FILENAME_MAX];

  rpv_struct*       rpv;     /* rpv structure     */
  rossli_brdf_spec* ambrals; /* AMBRALS structure */

  int*            surface_count; /* number of occurrence of each surface indices (used with mc_spectral_albedo/rpv/ambrals) */
  unsigned char** surface_index; /* xy-field of surface indices                                                             */
...
typedef struct {
  ...
  double***   back;        /* counter for backward photons */
  double***   back2;       /* counter for backward photons, standard deviation <*/
...
typedef struct photon_struct {
  ...
  size_t intersect_triangle_id; /* id of triangle where photon intersects surface */
  ...
  double***     pathlength_per_box;   /* photon pathlength per box                 */
  ...
  double***  q_jacobian;     /* weight for jacobian, used by Robert's Jacobians, lidar  */
  double*    r_jacobian;     /* pathlength for jacobian, used by jacobian implementation by Robert, also for lidar */
  double**** q_jacobian_sca; /* weight for scattering Jacobians (implementation Claudia)      */
  int        Nz_alis;        /* number of atmosph. layers, needed by ALIS or boxairmass */
  int        Nx_boxairmass;  /* needed for 3D boxairmass factors */
  int        Ny_boxairmass;
  ...
  struct photon_path* escapepath; /* photon path linked list                */
...
typedef struct {
  ...
  // CE standard deviation not yet correct
  int jacobian_std;
  ...
  int backward_Nz; /* number of zout's                 */
  ...
  int     pan_transmittance;            /*   panorama flag               */
  ...
  double* pan_theta_from_file; /* theta and phi from mc_panorama_file file */
  double* pan_phi_from_file;   /* theta and phi from mc_panorama_file */
  int     pan_n_from_file;     /* mc_panorama_file, number of angles */
...
typedef struct {
  ...
  float*** cf;   /* subpixel cloud fraction, 0 - 1   */
  ...
  int no_scattering; /* flag to switch off scattering */
  int no_absorption; /* flag to switch off absorption */
...
int random_rpv_direction (double rho0, double k, double theta, double mu1, double* mu2, double* phi);

photon_struct* calloc_photon (sample_struct* sample, int Nx, int Ny, int Nz, int source, int nlambda_abs, int Nsc, int n_caoth);
...
int random_rossli_direction (double iso, double vol, double geo, double mu1, double* mu2, double* phi);
...
#if HAVE_LIDAR
int read_lidar (char* filename, sample_struct* sample, int quiet);
#endif
...
int find_ris_factor (photon_struct* photon, atmosphere_struct* atmos, sample_struct* sample, elevation_struct* elev, float* refind);
