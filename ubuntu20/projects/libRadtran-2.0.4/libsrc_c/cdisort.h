...
typedef struct {
  ...
  int usrtau,                 /* TRUE=> radiant quantities returned at user-specified optical depths  */
    ...
    brdf_type,                /* type of BRDF, can be Hapke, RPV, Cox&Munk, Ross-Li                   */
    quiet,                    /* quiet output                                                         */
...
typedef struct {
  ...
  double rfldir, /* Direct-beam flux (w/o delta-M scaling),                              */
...
typedef struct {
  ...
  double fbeam, /* Intensity of incident parallel beam at top boundary                  */
...
typedef struct {
  ...
  double u10, /* BRDF C&M: u10        */
    pcl,      /* BRDF C&M: pcl        */
    xsal;     /* BRDF C&M: sal        */
} cam_brdf_spec;

typedef struct {
  /* Hapke BRDF specifications */
  double b0, /* BRDF hapke: b0    */
    h,       /* BRDF hapke: h    */
    w;       /* BRDF hapke: w    */
} hapke_brdf_spec;

typedef struct {
  /* Ross-Li BRDF specifications */
  double iso,  /* BRDF rossli: iso    */
    vol,       /* BRDF rossli: vol    */
    geo;       /* BRDF rossli: geo    */
  int hotspot; /* switch for turning on hot spot */
  int isCaM;   /* !!! not used by cdisort, only by mystic !!! */
} rossli_brdf_spec;
...
#endif
  hapke_brdf_spec*  hapke;  /* specification for hapke BRDF  */
  rossli_brdf_spec* rossli; /* specification for Ross-Li BRDF  */
} disort_brdf;
...
typedef struct {
  ...
  disort_bc   bc;
  disort_brdf brdf;
  int         nlyr, /* Number of computational layers                                       */
  ...
  double wvnmlo,    /* Wavenumber [cm^-1] lower range, used for Planck function             */
  ...
  double *dtauc,    /* Optical depths of computational layers, DTAUC(lc)                   */
...
typedef struct {
  ...
  double *        albmed, /* Albedo of medium, ALBMED(iu) (ds.flag.ibcnd = SPECIAL_BC case only)  */
...
typedef struct {
  double xb_0d, /* x-sub-zero-sup-minus in expansion of pseudo-spherical beam source, Eq. KST(22) */
...
#define BRDF_ROSSLI 3 /* has no access to this header file      */
#define BRDF_HAPKE 4
