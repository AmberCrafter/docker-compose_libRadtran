...
int int_convolute (double*  x_spc,
                   double*  y_spc,
                   int      spc_num,
                   double*  x_conv,
                   double*  y_conv,
  ...
  for (i = 0; i < spec_num; i++) {
    ...
    status = calc_splined_value (x_spec[i], &(y_spec[i]), x_spc, spc_num, a0, a1, a2, a3);
  ...
  *y_spec_conv = (double*)calloc (spc_num, sizeof (double));
  ...
  for (i = 0; i < spc_num; i++) {
    ...
    status = calc_splined_value (x_spc[i], &((*y_spec_conv)[i]), x_spec_tmp, spec_tmp_num, a0, a1, a2, a3);
