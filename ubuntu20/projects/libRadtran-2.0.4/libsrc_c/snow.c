...
void snowalbedo (double  omega,
  ...
  if (chi * taus > 7.0)
    ...
    *albedo_direct =
      (2 * (PP * (1. - gamma + omegas * bs) + omegas * (1. + bs) * (gamma * chi * umu0 - PP) / (1. - chi * chi * umu0 * umu0)) *
         exp (-taus / umu0) -
       omegas * bs * (QQP - QQM) + omegas * (1. + bs) * (QQP / (1. + chi * umu0) - QQM / (1. - chi * umu0))) /
      QQ;
  ...
  if (chi * taus > 7.0) /* Diffuse snow albedo, semi-infinite approximation. Eq. 7 */
    *albedo_diffuse = (2. * omegas / (1 + PP)) * (((1. + bs) / (chi * chi)) * (chi - log (1 + chi)) - bs / 2.);
  else
    /* Diffuse snow albedo, Eq. 6 */
    *albedo_diffuse =
      (2 * PP * ((1. - gamma + omegas * bs) * (1. - taus) - (gamma * omegas) * (1. + bs) / (1. - omegas)) * exp (-taus) -
       2 * PP *
         (omegas * (1. + bs) * (2. / (chi * chi) + (gamma * taus) / (1 - omegas)) + (1. - gamma + omegas * bs) * taus * taus) *
         dei1 +
       2. * omegas * (1. + bs) / (chi * chi) * (QQP * (dei2 + chi - log (1. + chi)) - QQM * (dei3 - chi - log (fabs (1. - chi)))) -
       omegas * bs * (QQP - QQM)) /
      QQ;
}
