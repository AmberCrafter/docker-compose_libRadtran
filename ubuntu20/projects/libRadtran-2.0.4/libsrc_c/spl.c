...
int spline (double* x, double* y, int number, double start, double step, int* newnumber, double** new_x, double** new_y) {
  int     i = 0, j = 0;
  int     status = 0;
  double  xx = 0, xnew = 0;
...
int spline_coeffc (double* x, double* y, int number, double** a0, double** a1, double** a2, double** a3) {
  int      i = 0, status = 0;
  double*  h = NULL;
...
int fspline_coeffc (float* x, float* y, int number, float** a0, float** a1, float** a2, float** a3) {
  int     i = 0, status = 0;
  float*  h = NULL;
...
int appspl_coeffc (double* x, double* y, double* w, int number, double** a0, double** a1, double** a2, double** a3) {
  int      n     = number - 1;
  int      i     = 0;
  ...
  for (i = 0; i < n - 1; i++)
    mat[i][2] = 2 * (h1[i] + h1[i + 1]) + b[i] * h2[i] * h2[i] + b[i + 1] * H[i] * H[i] + b[i + 2] * h2[i + 1] * h2[i + 1];
  ...
  for (i = 1; i < n; i++)
    a[i] = y[i] - b[i] / 3.0 * (c[i - 1] * h2[i - 1] - d[i] * c[i] + c[i + 1] * h2[i]);
...
int calc_splined_value (double xnew, double* ynew, double* x, int number, double* a0, double* a1, double* a2, double* a3) {
  int    i     = 0;
  double poly1 = 0, poly2 = 0, poly3 = 0;
...
int fcalc_splined_value (float xnew, float* ynew, float* x, int number, float* a0, float* a1, float* a2, float* a3) {
  int   i     = 0;
...
int linear_eqd (double* x, double* y, int number, double start, double step, int* newnumber, double** new_x, double** new_y) {
  int     i = 0, j = 0;
  int     status = 0;
  double  xx = 0, xnew = 0;
  double *a0 = NULL, *a1 = NULL, *a2 = NULL, *a3 = NULL;
