...
class setup_mc_group():
	...
	def __init__(self):
		...
		mc_backward_increment = option(
			...
			tokens = [
				addToken(name="Input.rte.mc.backward.isstep", datatype=int, optional=True, default='1'),
                addToken(name="Input.rte.mc.backward.jsstep", datatype=int, optional=True, default='1'),
                ] ,
			parents=['uvspec'],
			non_parents= ['mc_forward_output' ],
			speaker='rte_solver',
...
def get_mc_documentation():
	...
	Backward tracing of photons. \code{mc_backward_increment} takes two integers:
	\fcode{
	  mc_backward_increment ix_step iy_step
	}
	where \code{ix_step}, \code{iy_step} is the increment for the backward sampling.
    Pixels \code{ix_start}, \code{ix_start} + \code{ix_step}, \code{ix_start + 2*ix_step} (and similar for the y direction)
