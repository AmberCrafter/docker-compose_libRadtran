...
int uvspec (input_struct input, output_struct* output) {
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d distributing internal wavelength grid to transmittance wavelength grid in %s (%s)\n",
             status,
             function_name,
             file_name);
    return status;
...
#define EPSILON 1.E-6 /*relative rounding error for floats */
  ...
  if (Input.aer.seasn > 0 || Input.aer.vulcan > 0 || Input.aer.haze > 0 || Input.aer.visibility > 0) {
    ...
    if (Input.aer.seasn <= 0) {
      fprintf (stderr,
               "Error in %s (%s), aerosol information incomplete; specify aerosol_season or aerosol_default!\n",
               function_name,
               file_name);
      status--;
    }
    ...
    if (Input.aer.vulcan <= 0) {
      fprintf (stderr,
               "Error in %s (%s), aerosol information incomplete; specify aerosol_vulcan or aerosol_default!\n",
               function_name,
               file_name);
      status--;
    }
    ...
    if (Input.aer.haze <= 0) {
      fprintf (stderr,
               "Error in %s (%s), aerosol information incomplete; specify aerosol_haze or aerosol_default!\n",
               function_name,
               file_name);
      status--;
    }
    ...
    if (Input.aer.visibility <= 0) {
      fprintf (stderr,
               "Error in %s (%s), aerosol information incomplete; specify aerosol_visibility or aerosol_default!\n",
               function_name,
               file_name);
      status--;
    }
  ...
  if (Input.aer.seasn > 0 && Input.aer.n_species == NOT_DEFINED_INTEGER &&
      ((Input.rte.solver == SOLVER_POLRADTRAN && Input.rte.polradtran[POLRADTRAN_NSTOKES] > 1) || Input.rte.mc.polarisation == 1)) {
    fprintf (stderr,
             "Error in %s (%s), default aerosol is not implemented for polarized radiance calculations!\nPlease use option "
             "aerosol_species_file (e.g., OPAC aerosol).\n",
             function_name,
             file_name);
    status--;
  }
  ...
  if (!Input.aer.standard && Input.aer.spec) {
    fprintf (stderr,
             "Error in %s (%s), aerosol information incomplete; specify aerosol_default or each\n",
             function_name,
             file_name);
  ...
  case OUTLEVEL_PRESS:
    for (i = 1; i < Input.atm.nzout; i++) {
      if (Input.atm.press_zout[i] - Input.atm.press_zout[i - 1] >= 0 && Input.atm.press_zout[i - 1] != ZOUT_SURFACE &&
          Input.atm.press_zout[i] != ZOUT_TOA && !(Input.atm.press_zout[i] == ZOUT_CPT && i == Input.atm.nzout - 1)) {
        if ((Input.atm.press_zout[i] == ZOUT_CPT || Input.atm.press_zout[i - 1] == ZOUT_CPT) && i != Input.atm.nzout - 1) {
          ...
          fprintf (stderr,
                   "Error, outputlevel 'pressure_out' not sorted in descending order %f > %f \n",
                   Input.atm.press_zout[i],
                   Input.atm.press_zout[i - 1]);
        }
  ...
  if ((strlen (Input.filename[FN_WLTRANS]) > 0) && (strlen (Input.filename[FN_WLBANDS]) > 0)) {
  ...
  if ((strlen (Input.filename[FN_EXTRATERRESTRIAL]) > 0) && (strlen (Input.filename[FN_WLBANDS]) > 0)) {
  ...
  if ((Input.wl.start < 0 || Input.wl.end < 0) &&
      ...
      (strlen (Input.filename[FN_WLTRANS]) == 0) && (strlen (Input.filename[FN_WLBANDS]) == 0) &&
  ...
  if ((Input.wl.start_index > 0 || Input.wl.end_index > 0) && (Input.wl.start > 0 || Input.wl.end > 0)) {
  ...
  /*
  if (Input.rte.solver == SOLVER_TWOMAXRND && strlen (Input.filename[FN_CLOUD_FRACTION]) == 0) {
    fprintf (stderr, "Error, rte_solver twomaxrnd makes only sense with cloud_fraction_file\n");
    return -1;
  }
  */

  if (Input.rte.solver == SOLVER_DYNAMIC_TWOSTREAM && strlen (Input.filename[FN_CLOUD_FRACTION]) == 0) {
    fprintf (stderr, "Error, rte_solver dynamic_twostream makes only sense with cloud_fraction_file\n");
    return -1;
  }
  ...
  if (Input.heating != HEAT_NONE) {
    ...
    if (Input.heating == HEAT_LAYER_CD || Input.heating == HEAT_LAYER_FD) {
      if (Input.atm.nzout < 2 && Input.atm.zout_source != OUTLEVEL_ATM_LEVELS && Input.atm.zout_source != OUTLEVEL_ALL_LEVELS &&
          Input.atm.zout_source != OUTLEVEL_MODEL_LEVELS && Input.atm.zout_source != OUTLEVEL_MODEL_LAYERS &&
  ...
  for (i = 0; i < Input.n_output_user; i++)
    if (Input.output_user[i] == OUTPUT_USER_WIND_U || Input.output_user[i] == OUTPUT_USER_WIND_V ||
        Input.output_user[i] == OUTPUT_USER_WIND_W || Input.output_user[i] == OUTPUT_USER_HEAT_AD_X ||
        Input.output_user[i] == OUTPUT_USER_HEAT_AD_Y || Input.output_user[i] == OUTPUT_USER_HEAT_AD_Z ||
...
/*Quick fix for new option names no_absorption and no_scattering*/
...
int get_caoth_index (caoth_inp_struct** caoth, int* n_caoth, char* name, int old_input) {
  ...
  if (isp == *n_caoth) {
    ...
    *caoth = newcaoth;
    ...
    (*caoth)[isp].fullname = (char*)calloc (strlen ((*caoth)[isp].name1) + strlen ((*caoth)[isp].name2) + 1, sizeof (char));
...
int caoth_check (input_struct Input) {
  ...
  cloudcover = 0.0;
  if (Input.rte.mc.core_isactive) {
    int isp_wc3D = 0;
    for (isp_wc3D = 0; isp_wc3D < Input.n_caoth; isp_wc3D++) {
      if (strcmp (Input.caoth[isp_wc3D].name, "wc") == 0) {
        if (Input.caoth[isp_wc3D].source == CAOTH_FROM_3D)
          break;
      }
    }
    if (isp_wc3D == Input.n_caoth) {
      fprintf (stderr, "Error: If 'mc_core' is specified, a 3D cloud file must be given! Use 'wc_file 3D filename'!\n");
      status--;
    }
  }
  ...
  for (isp = 0; isp < Input.n_caoth; isp++) {
    if (Input.caoth[isp].modify[MODIFY_VAR_TAU][MODIFY_TYPE_SET] >= 0.0 &&
      ...
      fprintf (stderr,
               "Error in %s (%s), it doesn't make sense to specify both modify tau and modify tau550 for %s;\n",
               function_name,
               file_name,
    ...
    if ((Input.rte.solver == SOLVER_POLRADTRAN && Input.rte.polradtran[POLRADTRAN_NSTOKES] > 1) || Input.rte.mc.polarisation == 1) {
      ...
      default:
        fprintf (stderr,
                 "Error in %s (%s), for polarized radiance calculations and %s \n",
                 function_name,
                 file_name,
                 Input.caoth[isp].fullname);
        fprintf (stderr,
                 "you have to use '%s_properties%s XXX', where XXX must contain\n",
                 Input.caoth[isp].name1,
                 Input.caoth[isp].name2);
        fprintf (stderr,
                 "phase function matrices (e.g. mie for wc, hey or baum_v36 for ic, or your own optical properties data file)!\n");
        status--;
      }
    }

    /* need to specify a file for the given caoth properties */
    if ((Input.caoth[isp].properties != PROP_NONE && Input.caoth[isp].properties != PROP_EXPLICIT) &&
        strlen (Input.caoth[isp].filename) == 0) {
      fprintf (stderr,
               "Error, %s_properties%s only makes sense together with '%s_file%s'!\n",
               Input.caoth[isp].name1,
               Input.caoth[isp].name2,
               Input.caoth[isp].name1,
               Input.caoth[isp].name2);
    ...
    if (strlen (Input.caoth[isp].properties_filename) > 0 && !Input.caoth[isp].layer) {
      fprintf (stderr,
               "Error, species_properties %s requires optical properties defined per layer for 1D clouds.\n",
               Input.caoth[isp].name);
      fprintf (stderr, "Please specify species_layer %s and retry if that is what you mean!\n", Input.caoth[isp].name);
      status--;
    }

    if ((Input.caoth[isp].properties == PROP_YANG || Input.caoth[isp].properties == PROP_KEY) && Input.caoth[isp].habit < 0) {
    ...
    if (Input.caoth[isp].cloudcover != NOT_DEFINED_FLOAT) {
      if (Input.caoth[isp].cloudcover < 0.0 || Input.caoth[isp].cloudcover > 1.0) {
        fprintf (stderr,
                 "Error, cloudcover %f for %s not between 0 and 1\n",
                 Input.caoth[isp].cloudcover,
                 Input.caoth[isp].fullname);
        status--;
      }
  ...
  for (isp = 0; isp < (*Input).n_caoth; isp++) {
    ...
    case PROP_ECHAM4:
      if (strncasecmp ((*Input).caoth[isp].name, "wc", 2) != 0 && strncasecmp ((*Input).caoth[isp].name, "ic", 2) != 0) {
        fprintf (stderr,
                 "Error, profile properties for ECHAM4 does not work with profile %s! Need to use 'wc' or 'ic'!\n",
                 (*Input).caoth[isp].name);
        status--;
      }
      break;
    ...
    case PROP_HU:
      if (strncasecmp ((*Input).caoth[isp].name, "ic", 2) == 0) {
        fprintf (stderr,
                 "Error, profile_properties in input file is a water cloud property and is not allowed for profile 'ic'! Define "
                 "other profile name (e.g. 'wc')!\n");
        status--;
      }
      break;
    case PROP_FU:
    ...
    case PROP_YANG2013:
      if (strncasecmp ((*Input).caoth[isp].name, "wc", 2) == 0) {
        fprintf (stderr,
                 "Error, profile_properties in input file is an ice cloud property and is not allowed for profile 'wc'! Define "
                 "other profile name (e.g. 'ic')!\n");
        status--;
      }
      break;
    default:
    ...
    if ((*Input).caoth[isp].source == CAOTH_FROM_ECMWF && (*Input).caoth[isp].layer != 1) {
      ...
      if ((*Input).verbose)
        fprintf (stderr,
                 " ... switching on layer for %s, as ECMWF cloud data is representative for layers \n",
                 (*Input).caoth[isp].fullname);
    }
