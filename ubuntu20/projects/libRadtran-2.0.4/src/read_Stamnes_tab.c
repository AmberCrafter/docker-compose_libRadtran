...
int read_stamnes_table (char* filename, STAMNES_TABLE** table) {
  int      i = 0, j = 0, status = 0;
  int      rows = 0, columns = 0, max_columns = 0;
  double** value = NULL;

  /* read file to two-dimensional double array value */
  status = ASCII_file2double (filename, &rows, &max_columns, &columns, &value);
...
int stamnes_calculate (STAMNES_TABLE* table, double meas_zenith, double meas_ratio, double* column) {
  ...
  *column = -1;
  ...
  status = spline_coeffc (table->zenith, table->ozone[counter - 1], table->n_zenith, &a0, &a1, &a2, &a3);
  ...
  status = spline_coeffc (table->zenith, table->ozone[counter], table->n_zenith, &b0, &b1, &b2, &b3);
  ...
  status = calc_splined_value (meas_zenith, &lower, table->zenith, table->n_zenith, a0, a1, a2, a3);
  ...
  status = calc_splined_value (meas_zenith, &upper, table->zenith, table->n_zenith, b0, b1, b2, b3);
  ...
  *column =
    lower + (upper - lower) / (table->ratio[counter] - table->ratio[counter - 1]) * (meas_ratio - table->ratio[counter - 1]);

  return 0; /* if o.k. */
}

double cloud_clear_ratio (double** table, double meas_zenith, double meas_ratio, int rows, double* cratio) {
  int     status = 0, i;
  double *a0 = NULL, *a1 = NULL, *a2 = NULL, *a3 = NULL;
  ...
  /* calculate interpolated data */
  status = calc_splined_value (meas_zenith, cratio, x, rows, a0, a1, a2, a3);
  if (status != 0) {
    free (a0);
    free (a1);
    free (a2);
...
int main (int argc, char** argv) {
  ...
  status = stamnes_calculate (table, meas_zenith, meas_ratio, &column);
  ...
  if (clear) {
    ...
    /* read clear sky file */
    status = ASCII_file2double (filename_clear, &crows, &cmax_columns, &cmin_columns, &clear_table);
