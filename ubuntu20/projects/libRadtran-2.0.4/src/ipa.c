...
int setup_ipa (input_struct input, output_struct* output) {
  int isp = 0, isipa = 0;
...
int compare_levels_ipa_df (float* z1, int n1, float* z2, int n2) {
  int kc = 0;
  ...
  for (kc = 0; kc <= n1; kc++)
    if (fabs (z1[kc] / 1000.0 - z2[n1 - kc]) > EPSILON * z1[kc]) {
      fprintf (stderr,
               "Error, difference at level %d, altitude %g vs. %g (line %d, function %s in %s)\n",
               kc,
               z1[kc],
               z2[n1 - kc],
               __LINE__,
...
int read_caoth_ipa (input_struct     input,
  ...
  status = ASCII_checkfile (input_caoth.filename, &tmpnipa, &min_columns, &max_columns, &max_length);
  if (status)
  ...
  *output_caoth_ipa = calloc (*nipa, sizeof (caoth_out_struct));
  ...
  if (status != 0) {
    fprintf (stderr, "Error %d copying ipa %s to %s ", status, input_caoth.fullname, input_caoth.fullname);
