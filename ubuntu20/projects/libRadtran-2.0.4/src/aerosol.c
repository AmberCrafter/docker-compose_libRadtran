...
static int read_aerosol_profile (char* filename, int nlyr, int nlambda_r, float* zd_common, float** dtau, int scale);

static int read_aerosol_profile_from_map (char*     filename,
                                          float     latitude,
                                          float     longitude,
                                          struct tm UTC,
...
static int read_aerosol_moments (char* filename, float** moment, int* nmom);
...
static int read_aerosol_species_from_ASCII_file (char* filename, aer_out_struct* aer, int quiet, int verbose);
...
static int scale_aer_tau (float** dtau, float* lambda_r, int nlambda_r, int nlyr, float tau, float* zd, float altitude);

static int
scale_aer_angstrom (float** dtau, float* lambda_r, int nlambda_r, int nlyr, float alpha, float beta, float* zd, float altitude);

static int scale_aer_king_byrne (float** dtau,
                                 float*  lambda_r,
                                 int     nlambda_r,
                                 int     nlyr,
                                 float   alpha_0,
...
static int extrapolate_atmosphere (float altitude, atm_out_struct* out, int quiet, int verbose) {
  ...
  out->microphys.press[out->nlev] =
    out->microphys.press[out->nlev - 1] *
  ...
  out->microphys.dens[MOL_AIR][out->nlev] =
    out->microphys.press[out->nlev] * 1E-04 / (boltzmann * out->microphys.temper[out->nlev]);
...
int setup_aerosol (input_struct input, output_struct* output) {
  ...
  void F77_FUNC (aerprof, AERPROF) (int* nlyr, float* zd, float* aer_pro, int* seasn, int* vulcan, float* visib);
  void F77_FUNC (aeropt, AEROPT) (int*   vulcan,
                                  float* aerext,
                                  float* aerabs,
                                  float* aersym,
                                  float* aerwvn,
                                  ...
                                  int*   haze,
                                  int*   maxawvn);
  void F77_FUNC (aerwvn, AERWVN) (float* aerwvn,
                                  float* aerabs,
                                  float* aersym,
                                  float* aerext,
                                  float* aer_pro,
  ...
  if (input.aer.vulcan > 0) {
    ...
    output->aer.nlev = 0;
    set_common_z_grid (output->atm.zd_common, output->atm.nlev_common, &tmp_zd, &tmp_nlev);
    ...
    if (strlen (input.aer.filename[FN_AER_TAU]) > 0) {
      if (!tau_from_netCDF)
        ...
        status += add_sigma_nc_file2grid (&tmp_zd,
                                          &tmp_nlev,
    ...
    for (ipa = 0; ipa < output->nipa; ipa++) {
      ...
      F77_FUNC (aerprof, ARPROF) (&nlyr, zalt, aer_pro, &input.aer.seasn, &input.aer.vulcan, &(output->aer.visibility));
      ...
      if (input.aer.profile_modtran) {
        ...
        for (lc = lc_6 + 1; lc < output->aer.nlev; lc++)
          zalt[lc] =
            zalt[lc - 1] - (zalt[lc_6] - output->alt.altitude) / zalt[lc_6] * (output->aer.zd[lc - 1] - output->aer.zd[lc]);
        ...
        arb_wvn (output->aer.nlev, zalt, aer_pro, lc_alt, new_z, new_prof, 1, 1);
      ...
      F77_FUNC (aeropt, AEROPT)
      (&input.aer.vulcan, tmp_aerext, tmp_aerabs, tmp_aersym, tmp_aerwvn, tmp_rh, &nlyr, zalt, &input.aer.haze, &maxawvn);
      ...
      if (input.aer.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SCALE] >
          0.0) /* we don't care about the 0 here because the aerosol is scaled to 0 later anyway */
        for (lc = 0; lc < output->aer.nlev - 1; lc++)
          for (iv = 0; iv < output->wl.nlambda_r; iv++)
            output->aer.optprop.dtau[iv][lc] =
              output->aer.optprop.dtau[iv][lc] / input.aer.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SCALE];
      ...
      if (strlen (input.aer.filename[FN_AER_EXPLICIT]) > 0) {
        ...
        if ((status = ASCII_checkfile (input.aer.filename[FN_AER_EXPLICIT], &rows, &min_columns, &max_columns, &max_length)) != 0) {
        ...
        status = redistribute_optprop (&(tmpaer->optprop),
                                       output->wl.nlambda_r,
                                       output->wl.nlambda_rte_lower,
                                       output->wl.nlambda_rte_upper,
                                       tmpaer->zd,
      ...
      if (strlen (input.aer.filename[FN_AER_TAU]) > 0) {
        if (!tau_from_netCDF) {
          ...
          status = read_aerosol_profile_from_map (input.aer.filename[FN_AER_TAU],
                                                  input.latitude,
                                                  input.longitude,
                                                  input.UTC,
                                                  input.atm.time_interpolate,
      ...
      if (strlen (input.aer.filename[FN_AER_GG]) > 0) {
        status = read_aerosol_profile (input.aer.filename[FN_AER_GG],
                                       output->aer.nlev - 1,
                                       output->wl.nlambda_r,
                                       output->aer.zd,
                                       output->aer.optprop.g1,
                                       0);
        if (status != 0) {
      ...
      if (strlen (input.aer.filename[FN_AER_SSA]) > 0) {
        status = read_aerosol_profile (input.aer.filename[FN_AER_SSA],
                                       output->aer.nlev - 1,
                                       output->wl.nlambda_r,
                                       output->aer.zd,
                                       output->aer.optprop.ssa,
                                       0);
        if (status != 0) {
      ...
      if (strlen (input.aer.filename[FN_AER_MOMENTS]) > 0) {
        ...
        status = read_aerosol_moments (input.aer.filename[FN_AER_MOMENTS], &moment, &rows);
        if (status != 0) {
        ...
        for (iv = 0; iv < output->wl.nlambda_r; iv++) {
          for (lc = 0; lc < output->aer.nlev - 1; lc++) {
            ...
            output->aer.optprop.moment[iv][lc][0] = calloc (output->aer.optprop.nmom[iv][lc], sizeof (float));
      ...
      if (strlen (input.aer.filename[FN_AER_REF]) > 0) {
        ...
        status = read_3c_file_float (input.aer.filename[FN_AER_REF], &lref, &re, &im, &nref);
        ...
        status = arb_wvn (nref, lref, re, output->wl.nlambda_r, output->wl.lambda_r, output->aer.re_r, linear, 0);
        ...
        status = arb_wvn (nref, lref, im, output->wl.nlambda_r, output->wl.lambda_r, output->aer.im_r, linear, 0);
      ...
      if (strlen (input.aer.filename[FN_AER_SIZ]) > 0) {
        ...
        status = read_2c_file (input.aer.filename[FN_AER_SIZ], &(output->aer.xsiz), &(output->aer.ysiz), &(output->aer.nsiz));
      ...
      if (strlen (input.aer.filename[FN_AER_SPECIES]) != 0) {
        ...
        if (status != 0) {
          fprintf (stderr,
                   "Error %d in %s (%s) during read_aerosol_species from %s \n",
                   status,
                   function_name,
                   file_name,
      ...
      if (input.aer.modify[MODIFY_VAR_SSA][MODIFY_TYPE_SET] >= 0.0)
        for (lc = 0; lc < output->aer.nlev - 1; lc++)
          for (iv = 0; iv < output->wl.nlambda_r; iv++) {
            ...
            if (output->aer.optprop.ssa[iv][lc] > 1.0)
              output->aer.optprop.ssa[iv][lc] = 1.0;
          }
      ...
      if (input.aer.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SET] >= 0.0) {
        status = scale_aer_tau (output->aer.optprop.dtau,
                                output->wl.lambda_r,
                                output->wl.nlambda_r,
                                output->aer.nlev - 1,
                                input.aer.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SET],
                                output->aer.zd,
                                output->alt.altitude);
      ...
      if (input.aer.beta >= 0.0 && input.aer.alpha_2 < -800) {
        status = scale_aer_angstrom (output->aer.optprop.dtau,
                                     output->wl.lambda_r,
                                     output->wl.nlambda_r,
                                     output->aer.nlev - 1,
                                     input.aer.alpha,
      ...
      if (input.aer.alpha_2 >= -800.0) {
        status = scale_aer_king_byrne (output->aer.optprop.dtau,
                                       output->wl.lambda_r,
                                       output->wl.nlambda_r,
                                       output->aer.nlev - 1,
                                       input.aer.alpha_0,
      ...
      if (input.aer.modify[MODIFY_VAR_TAU550][MODIFY_TYPE_SET] >= 0.0 || input.aer.tau_wvl_tau >= 0.0) {
        ...
        if (status != 0) {
          ...
          fprintf (stderr,
                   "The range of the internal wavelength grid is from %f to %f nm. \n",
                   lambda[0],
                   lambda[output->wl.nlambda_r - 1]);
  ...
  set_common_z_grid (output->aer.zd, output->aer.nlev, &output->atm.zd_common, &output->atm.nlev_common);
  ...
  if (strlen (input.rte.mc.filename[FN_MC_AERIS]) > 0) {
    if (input.verbose)
      fprintf (stderr,
               " ... aerosol concentration importance sampling, scaling factors from %s \n ... scaling factors: ",
               input.rte.mc.filename[FN_MC_AERIS]);

    status = read_1c_file (input.rte.mc.filename[FN_MC_AERIS], &(output->mc.alis.aer_scaling_factors), &(output->mc.alis.Nc));
    if (status != 0)
...
int read_optprop_files (char*     filename,
  ...
  if (strcasecmp (string[0][1], "NULL")) { /* if the uppermost string is not NULL */
    ...
    if ((status = ASCII_file2float (string[0][1], &arows, &amax_columns, &amin_columns, &data)) != 0) {
  ...
  for (lc = 0; lc < nlev - 1; lc++) {
    ...
    if ((status = ASCII_file2float (string[lc + 1][1], &arows, &amax_columns, &amin_columns, &data)) != 0) {
    ...
    status = arb_wvn2 (tmp_nlambda, tmp_lambda, tmp_dtau, nlambda_rte_lower, nlambda_rte_upper, lambda_r, temp, linear);
    ...
    status = arb_wvn2 (tmp_nlambda, tmp_lambda, tmp_gg, nlambda_rte_lower, nlambda_rte_upper, lambda_r, temp, linear);
    ...
    status = arb_wvn2 (tmp_nlambda, tmp_lambda, tmp_ssa, nlambda_rte_lower, nlambda_rte_upper, lambda_r, temp, linear);
    ...
    for (k = 0; k < tmp_nmom; k++) {
      status = arb_wvn2 (tmp_nlambda, tmp_lambda, tmp_mom[k], nlambda_rte_lower, nlambda_rte_upper, lambda_r, temp_mom[k], linear);
      if (status != 0) {
...
static int read_aerosol_profile (char* filename, int nlyr, int nlambda_r, float* zd_common, float** dtau, int scale) {
  ...
  status = read_2c_file_float (filename, &aero_zd, &aero_tau, &rows);
  ...
  status = regrid_float (aero_zd, aero_tau, rows, zd_common, aero_tau_common, nlyr + 1, scale);
...
static int read_aerosol_profile_from_map (char*     filename,
  ...
  status += arb_wvn (nlev_atm, log_p_atm, z_atm, (int)nnew, log_p_aero, aero_zd, INTERP_METHOD_LINEAR, 0);
  if (status != 0) {
  ...
  status = regrid_float (aero_zd, aero_tau, mlev, zd_common, aero_tau_common, nlyr + 1, scale);
...
static int scale_aer_king_byrne (float** dtau,
  ...
  for (iv = 0; iv < nlambda_r; iv++) {
    ...
    depth =
      exp (alpha_0) * pow (lambda_r[iv] / 1000.0, alpha_1) * pow (lambda_r[iv] / 1000.0, alpha_2 * log (lambda_r[iv] / 1000.0));
    fact = depth / ext;
...
static int aerosol_mie_calculation (input_struct input, output_struct* output) {
  ...
  for (iv = 0; iv < output->wl.nlambda_r; iv++) {
    ...
    status = mie_calc_sizedist (*mie_inp,
                                mie_out,
                                MIEV0,
                                USER,
                                WATPROP_SEGELSTEIN,
                                crefin,
                                (output->wl.lambda_r[iv]) / 1000.0,
                                temperature,
                                1,
                                output->aer.xsiz,
                                output->aer.ysiz,
    ...
    for (lc = 0; lc < output->aer.nlev - 1; lc++) {
      ...
      for (ip = 0; ip < 1; ip++) {
        ...
        for (k = 0; k < kmax; k++) /* FIXME: Normalization should be done only for phase function */
...
static int read_aerosol_species (input_struct input, output_struct* output) {
  ...
  if (input.aer.n_species != NOT_DEFINED_INTEGER) {
    ...
    status =
      ASCII_file2string (aerosol_species_name_list, &(output->aer.n_species), &(max_columns), &(min_columns), &(aerosol_species));
    if (status != 0) {
  ...
  if (species_from_netCDF) {
    ...
    status = read_aerosol_species_from_ASCII_file (aerosol_species_file, &(output->aer), input.quiet, input.verbose);
  }
  if (status != 0) {
  ...
  if (input.verbose) {
    ...
    for (lev = 0; lev < output->aer.nlev - 1; lev++) {
      fprintf (stderr, " %11.6f  ", output->aer.zd[lev + 1]);
      for (i_aer = 0; i_aer < output->aer.n_species; i_aer++)
...
int read_aerosol_species_from_netCDF_map (input_struct input, output_struct* output, char* filename) {
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d during interpolation of 'rho_air_aero' (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  status =
    arb_wvn (output->atm.nlev, log_p_atm, output->atm.zd, output->aer.nlev, log_p_aer, output->aer.zd, INTERP_METHOD_LINEAR, 0);
  if (status != 0) {
    fprintf (stderr,
             "Error %d during interpolation of 'z_aer' (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  if ((status = ASCII_calloc_float_3D (&(aerosol_mass_dens), nt, output->aer.n_species, aer_nlayer)) != 0) {
    fprintf (stderr,
             "Error, allocating memory for 'aerosol_mass_dens' (line %d, function %s in %s)\n",
             __LINE__,
             __func__,
             __FILE__);
    return -1;
  ...
  for (t = 0; t <= nt - 1; t++) {
    ...
    for (i_aer = 0; i_aer < output->aer.n_species; i_aer++) {
      ...
      status = alloc_and_read_netCDF_column_float (ncid,
                                                   output->aer.species_names[i_aer],
                                                   &aerosol_mass_dens[t][i_aer],
                                                   aer_nlayer,
                                                   itime,
                                                   ilat,
                                                   ilon,
      ...
      if (status == 0) {
        ...
        if (strncasecmp ("g m**-3", data_unit, 7) == 0 || strncasecmp ("g/m**3", data_unit, 6) == 0) {
          ...
          fprintf (stderr,
                   " ... Error, unknown aerosol format %s (line %d, function %s in %s)\n",
                   data_unit,
                   __LINE__,
                   __func__,
                   __FILE__);
...

{
  ...
  status = ASCII_file2float (filename, &rows, &max_columns, &min_columns, &aer_data);
...
static int aerosol_species_to_optical_properties (input_struct input, output_struct* output) {
  ...
  for (i_aer = 0; i_aer < output->aer.n_species; i_aer++) {
    ...
    status = read_caoth_prop (aerosol_opt_prop_file,
                              output->wl.lambda_r,
                              output->wl.nlambda_r,
                              output->wl.nlambda_rte_lower,
                              output->wl.nlambda_rte_upper,
  ...
  status =
    average_aerosol_prop_opac (&(output->aer), output->wl.nlambda_r, output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper);
  if (status != 0) {
  ...
  if (aer->ssprop.type == PHASE_HYBRID || aer->ssprop.type == PHASE_MOMENTS) {
    ...
    for (iv = nlambda_lower; iv <= nlambda_upper; iv++) {
      ...
      for (lc = 0; lc < aer->nlev - 1; lc++) {
          ...
          if (aer->massdens[i_aer][lc] > 0) {
            ...
            for (ip = 0; ip < nphamat; ip++)
              for (im = 0; im < aer->optprop_opac[i_aer].nmom[iv][lc]; im++)
                mom_ssa_tau_tot[ip][im] += aer->optprop_opac[i_aer].moment[iv][lc][ip][im] * aer->optprop_opac[i_aer].ssa[iv][lc] *
        ...
        aer->optprop.g1[iv][lc] = aer->optprop.moment[iv][lc][0][1];
        aer->optprop.ff[iv][lc] = 1.0;
      }
    }
  ...
  if (aer->ssprop.type == PHASE_HYBRID || aer->ssprop.type == PHASE_EXPLICIT) {
    ...
    for (iv = nlambda_lower; iv <= nlambda_upper; iv++) {
      ...
      for (lc = 0; lc < aer->nlev - 1; lc++) {
        ...
        for (i_aer = 0; i_aer < aer->n_species; i_aer++)
          if (aer->massdens[i_aer][lc] > 0) {
            interp_optprop_weight[n_tot] = aer->optprop_opac[i_aer].ssa[iv][lc] * aer->optprop_opac[i_aer].dtau[iv][lc];
        ...
        status = sort_and_add_weighted_phase (n_tot,
...
int aerosol_set_visibility (aer_inp_struct inp, char* path, float* visib, int verbose) {
  ...
  *visib = inp.visibility;
  ...
  if (inp.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SET] >= 0.0 || inp.modify[MODIFY_VAR_TAU][MODIFY_TYPE_SCALE] >= 0.0 ||
      inp.beta >= 0.0 || inp.modify[MODIFY_VAR_TAU550][MODIFY_TYPE_SET] >= 0.0) {
    ...
    sprintf (shettle_filename, "%s/aerosol/shettle/tau550.%1d.%1d", path, inp.vulcan, inp.seasn);

    status = read_2c_file (shettle_filename, &shettle_vis, &shettle_tau550, &shettle_n);
    if (status != 0) {
