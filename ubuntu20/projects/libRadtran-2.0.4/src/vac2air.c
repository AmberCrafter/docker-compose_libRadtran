...
static int get_options (int argc, char** argv, char* programname, char* infilename, char* reverse, char* linear) {
  int c = 0;
...
int main (int argc, char** argv) {
  ...
  /* read input file */
  status = ASCII_file2double (infilename, &rows, &max_columns, &min_columns, &value);
  ...
  for (i = 1; i < min_columns; i++) {
    ...
    /* shift i-th column */
    status = vac2air (lambda, irradiance, rows, reverse, linear, &irradiance_shifted);
