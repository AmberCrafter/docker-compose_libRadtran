...
int main (int argc, char** argv) {
  ...
  file_qext = (char*)calloc (strlen (filename) + 7, sizeof (char));
  strcpy (file_qext, filename);
  file_qext = strncat (file_qext, ".qext", 6);

  file_ssa = (char*)calloc (strlen (filename) + 6, sizeof (char));
  strcpy (file_ssa, filename);
  file_ssa = strncat (file_ssa, ".ssa", 5);

  file_gg = (char*)calloc (strlen (filename) + 5, sizeof (char));
  strcpy (file_gg, filename);
  file_gg = strncat (file_gg, ".gg", 4);
