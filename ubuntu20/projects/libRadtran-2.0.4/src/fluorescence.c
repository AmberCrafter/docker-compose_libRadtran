...
int setup_fluorescence (input_struct input, output_struct* output) {
  ...
  case FLUORESCENCE_FROM_FLUORESCENCE_FILE:
    ...
    if (input.flu.source == FLUORESCENCE_FROM_FLUORESCENCE_FILE) /* copy fluorescence file name */
    ...
    status = arb_wvn2_double (output->flu.file_nlambda_fluorescence,
                              output->flu.file_lambda_fluorescence,
                              output->flu.file_fluorescence,
                              output->wl.nlambda_rte_lower,
                              output->wl.nlambda_rte_upper,
                              tmp,
                              output->flu.fluorescence_r,
                              linear);
