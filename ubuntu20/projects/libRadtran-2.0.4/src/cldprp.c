...
static int get_options (int argc, char** argv, char* programname, char* infilename, int* properties, int* unscaled) {
  int c = 0;
...
int main (int argc, char** argv) {
  ...
  status = get_options (argc, argv, programname, infilename, &properties, &unscaled);
  ...
  for (i = 0; i < n; i++) {
    ...
    case FU96:
      status = ic_fu96 (wvl[i], nlyr, fupath, iwc, effr, &tau, &gg, &ssa, &f, zd, iclayer, unscaled);
    ...
    case FU98:
      status = ic_fu98 (wvl[i], nlyr, fupath, iwc, effr, &tau, &gg, &ssa, zd, iclayer);
    ...
    case HU:
      F77_FUNC (wcloud, WCLOUD) (&(wvl[i]), &newsiz, &nlyr, path, &npath, wccon, effr, &tau, &gg, &ssa, zd, &wclayer);
    ...
    case YANG:
      ...
      if (newkey) {
#if HAVE_YANG
        status = yang56 (wvl[i] / 1000.0, reff[i], hab, yangpath, &tau, &ssa, &g1, &g2, &ff, newkey);
