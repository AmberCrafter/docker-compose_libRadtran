...
static int x_section (int       scheme,
                      int       ispcs,
                      float***  press,
                      float***  temp_avg,
                      int       nlev,
...
static int x_section_h2o (int       scheme,
                          float***  press,
                          float***  temp_avg,
                          float**** dens,
                          float**** dens_avg,
...
static int kato_crs (float***   temp_avg,
                     float***   press,
                     float****  dens,
                     float****  dens_avg,
                     float*     z,
...
static int fu_profile (float***    temper,
                       float***    press,
                       float*      z,
                       float****   dens,
                       float****   dens_avg,
...
static int avhrr_kratz (int     channel,
                        int     interval,
                        float*  z0,
                        float*  p0,
                        float*  t0,
...
static int avhrr_kratz_profile (float***    temper,
                                float***    press,
                                float*      z,
                                float****   dens,
                                int         nlev,
    ...
    *ngauss = crs->wght[ispcs][iband];
    ...
    if (ispcs == KATO_H2O) {
      if (iband == 9 || iband == 10 || iband == 11 || iband == 13 || iband == 15) {
        status =
          x_section (crs->scheme, ispcs, press, temp_avg, nlev, Nx, Ny, iband, path, ngauss, extinction, weight, quiet, verbose);
        if (status != 0) {
        ...
        status = x_section_h2o (crs->scheme,
                                press,
                                temp_avg,
                                dens,
                                dens_avg,
...
static int avhrr_kratz (int     channel,
  ...
  case 1:
    switch (interval) {
    case 1:
      F77_FUNC (avhrr11, AVHRR11) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 2:
      F77_FUNC (avhrr12, AVHRR12) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 3:
      F77_FUNC (avhrr13, AVHRR13) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 4:
      F77_FUNC (avhrr14, AVHRR14) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 5:
      F77_FUNC (avhrr15, AVHRR15) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    default:
  ...
  case 2:
    switch (interval) {
    case 1:
      F77_FUNC (avhrr21, AVHRR21) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 2:
      F77_FUNC (avhrr22, AVHRR22) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 3:
      F77_FUNC (avhrr23, AVHRR23) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 4:
      F77_FUNC (avhrr24, AVHRR24) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    default:
      fprintf (stderr, "error, AVHRR channel %d has no interval %d\n", channel, interval);
      return -1;
  ...
  case 3:
    switch (interval) {
    case 1:
      F77_FUNC (avhrr31, AVHRR31) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 2:
      F77_FUNC (avhrr32, AVHRR32) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 3:
      F77_FUNC (avhrr33, AVHRR33) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 4:
      F77_FUNC (avhrr34, AVHRR34) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    case 5:
      F77_FUNC (avhrr35, AVHRR35) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    default:
      fprintf (stderr, "error, AVHRR channel %d has no interval %d\n", channel, interval);
      return -1;
  ...
  case 4:
    switch (interval) {
    case 1:
      F77_FUNC (avhrr41, AVHRR41) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    default:
      fprintf (stderr, "error, AVHRR channel %d has no interval %d\n", channel, interval);
      return -1;
  ...
  case 5:
    switch (interval) {
    case 1:
      F77_FUNC (avhrr51, AVHRR51) (z0, p0, t0, u0, ux, &nlev, &co2, &n2o, &f11, &f12, &ch4, tmp_od, tmp_wght, ntau);
      break;
    default:
      fprintf (stderr, "error, AVHRR channel %d has no interval %d\n", channel, interval);
      return -1;
...
static int avhrr_kratz_profile (float***    temper,
  ...
  avhrr_kratz (1, 5, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[0].crs[0][0], ck[0].weight, &(ck[0].ngauss));
  avhrr_kratz (1, 4, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[1].crs[0][0], ck[1].weight, &(ck[1].ngauss));
  avhrr_kratz (1, 3, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[2].crs[0][0], ck[2].weight, &(ck[2].ngauss));
  avhrr_kratz (1, 2, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[3].crs[0][0], ck[3].weight, &(ck[3].ngauss));
  avhrr_kratz (1, 1, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[4].crs[0][0], ck[4].weight, &(ck[4].ngauss));

  /* channel 2 */
  avhrr_kratz (2, 4, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[5].crs[0][0], ck[5].weight, &(ck[5].ngauss));
  avhrr_kratz (2, 3, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[6].crs[0][0], ck[6].weight, &(ck[6].ngauss));
  avhrr_kratz (2, 2, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[7].crs[0][0], ck[7].weight, &(ck[7].ngauss));
  avhrr_kratz (2, 1, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[8].crs[0][0], ck[8].weight, &(ck[8].ngauss));

  /* channel 3 */
  avhrr_kratz (3, 5, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[9].crs[0][0], ck[9].weight, &(ck[9].ngauss));
  avhrr_kratz (3, 4, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[10].crs[0][0], ck[10].weight, &(ck[10].ngauss));
  avhrr_kratz (3, 3, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[11].crs[0][0], ck[11].weight, &(ck[11].ngauss));
  avhrr_kratz (3, 2, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[12].crs[0][0], ck[12].weight, &(ck[12].ngauss));
  avhrr_kratz (3, 1, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[13].crs[0][0], ck[13].weight, &(ck[13].ngauss));

  /* channel 4 */
  avhrr_kratz (4, 1, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[14].crs[0][0], ck[14].weight, &(ck[14].ngauss));

  /* channel 5 */
  avhrr_kratz (5, 1, z0, p0, t0, u0, ux, nlev, co2, n2o, f11, f12, ch4, ck[15].crs[0][0], ck[15].weight, &(ck[15].ngauss));

  return 0; /* ok */
}

int sbdart_profile (float***    temper,
                    float***    press,
                    float*      z,
                    float***    h2o,
                    float***    o3,
...
#include "sbtaugas.h"
  ...
  void F77_FUNC (sbtaugas, SBTAUGAS) (int*   nlev,
                                      float* zi,
                                      float* pi,
                                      float* ti,
                                      float* wh,
...
static int generic_profile (char* filename, float* z, int nlev, int* nwght, ck_profile* ck) {
  ...
  for (iv = 0; iv < nwvn; iv++) {
    ...
    for (lu = 0; lu < nlev - 1; lu++) {
      ...
      /* changed from vara_double to vara_float; netcdf distiguishes between single and double, but a netcdf double */
      /* may be read by the _float() function according to Tobias KOelling; BM, 6.9.2018                            */
      status = nc_get_vara_float (ncid, id_k, crsstart, crscount, ck[iv].crs[0][0][lu]);
      if (status != NC_NOERR) {
        fprintf (stderr, "Error %d reading %s\n", status, filename);
        return status;
      }
...
int crs_ck (ck_struct*         ck,
  ...
  if (first) {
    ...
    case CK_LOWTRAN:
	  ...
	  */
          ...
          crs_ck->profile[is][iv].crs = calloc (Nx, sizeof (float***));
          for (ix = 0; ix < Nx; ix++) {
            crs_ck->profile[is][iv].crs[ix] = calloc (Ny, sizeof (float**));
            for (iy = 0; iy < Ny; iy++) {
              crs_ck->profile[is][iv].crs[ix][iy] = calloc (nlev, sizeof (float*));
              for (lc = 0; lc < nlev; lc++)
                crs_ck->profile[is][iv].crs[ix][iy][lc] = calloc (n_maxint, sizeof (float));
            }
          }
        }
      }
      break;
    case CK_FU: /* for FU, memory allocation is done in ckdfu() -- BM, 6.9.2018 */
      break;
    case CK_FILE:
  ...
  for (ipa = 0; ipa < nipa; ipa++) {
    ...
    case CK_AVHRR_KRATZ:
      ...
      status = avhrr_kratz_profile (temp,
                                    press,
                                    z,
                                    dens,
                                    nlev,
...
static int x_section (int       scheme,
  ...
  sprintf (sband, "%d", iband % 10000);  // truncate iband to prevent buffer overflow
  ...
  for (ix = 0; ix < Nx; ix++) {
    for (iy = 0; iy < Ny; iy++) {
      for (lc = 0; lc < nlev - 1; lc++) {
        ...
        for (k = 1; k <= *ngauss; ++k) {
          xabs[ix][iy][lc][k] = ofrcnx * ofrcny * xsect[(k << 2) - 4] + frcnx * ofrcny * xsect[(k << 2) - 3] +
                                frcnx * frcny * xsect[(k << 2) - 2] + ofrcnx * frcny * xsect[(k << 2) - 1];
        }
    ...
    for (iy = 0; iy < Ny; iy++) {
      ...
      for (lc = 0; lc < nlev - 1; lc++) {
        ...
        if (irho <= 10) {
          ...
          for (k = 1; k <= *ngauss; ++k) {
            ansyrl = ofrcnx * ofrcny * xsect[(k << 3) - 8] + frcnx * ofrcny * xsect[(k << 3) - 7] +
                     frcnx * frcny * xsect[(k << 3) - 6] + ofrcnx * frcny * xsect[(k << 3) - 5];

            ansyru = ofrcnx * ofrcny * xsect[(k << 3) - 4] + frcnx * ofrcny * xsect[(k << 3) - 3] +
                     frcnx * frcny * xsect[(k << 3) - 2] + ofrcnx * frcny * xsect[(k << 3) - 1];

            xabs[ix][iy][lc][k] = (ansyru - ansyrl) * (rholog - r[irho - 1]) / (r[irho] - r[irho - 1]) + ansyrl;
          }
          ...
          for (k = 1; k <= *ngauss; ++k) {
            xabs[ix][iy][lc][k] = ofrcnx * ofrcny * xsect[(k << 3) - 8] + frcnx * ofrcny * xsect[(k << 3) - 7] +
                                  frcnx * frcny * xsect[(k << 3) - 6] + ofrcnx * frcny * xsect[(k << 3) - 5];
          }
        }
