...
int main (int argc, char** argv) {
  ...
  /* of the atmosphere */
  if ((status = ASCII_file2double (uvspec_output_file, &rows, &max_columns, &min_columns, &data)) != 0) {
  ...
  if ((status = ASCII_file2double (Lu2006_filename, &rows, &max_columns, &min_columns, &data)) != 0) {
  ...
  if ((status = ASCII_file2double (SB1981_filename, &rows, &max_columns, &min_columns, &data)) != 0) {
  ...
  if ((status = ASCII_file2double (Vasilkov2005_filename, &rows, &max_columns, &min_columns, &data)) != 0) {
  ...
  if ((status = ASCII_file2double (Bricaud1995_filename, &rows, &max_columns, &min_columns, &data)) != 0) {
  ...
  for (iv = ivs; iv < nlambda; iv++) {
    ...
    while (lambdap < lambda[iv]) { /* Only consider shorter wavelengths, Mobley (1994) */
      ...
      b_b_Ep =
        (0.5 * bw[ivp] + (0.002 + 0.02 * (0.5 - 0.25 * log10 (C)) * 550.0 / lambdap) * (0.3 * pow (C, 0.62) - bw_550));  //*dnu*f_R;
