...
static int read_crs_from_files (input_struct input, output_struct* output, int first, int** molabs_src);

static int crs_o3 (crs_out_struct* crs_out,
                   int             first,
                   float*          lambda,
                   int             n_lambda,
                   int*            molabs_src,
...
static int crs_co2 (crs_out_struct* crs_out,
                    int             first,
                    float*          lambda,
                    int             n_lambda,
                    int*            molabs_src,
...
static int crs_o2 (crs_out_struct* crs_out,
                   int             first,
                   float*          lambda,
                   int             n_lambda,
                   int*            molabs_src,
...
static int crs_no2 (crs_out_struct* crs_out,
                    int             first,
                    float*          lambda,
                    int             n_lambda,
                    int*            molabs_src,
...
static int crs_so2 (crs_out_struct* crs_out,
                    int             first,
                    float*          lambda,
                    int             n_lambda,
                    int*            molabs_src,
...
static int crs_oclo (crs_out_struct* crs_out,
                     int             first,
                     float*          lambda,
                     int             n_lambda,
                     int*            molabs_src,
...
static int crs_hcho (crs_out_struct* crs_out,
                     int             first,
                     float*          lambda,
                     int             n_lambda,
                     int*            molabs_src,
...
static int crs_o4 (crs_out_struct* crs_out,
                   int             first,
                   int             o4abs,
                   float*          lambda,
                   int             n_lambda,
...
int setup_crs (input_struct input, output_struct* output) {
  ...
  if (output->molecular3d) {
    ...
    if (!input.quiet)
      fprintf (stderr,
               "3DAbs 3D allocate crs memory Nx %d Ny %d nlev %d MOL_NN %d nlam %d Nzatm %d!\n",
               output->atm.Nxatm,
               output->atm.Nyatm,
               output->atm.nlev,
               MOL_NN,
    ...
    default:
      ...
      ASCII_free_float_5D (output->crs.crs, 1, 1, output->atm.Nzatm, MOL_NN);
...
int setup_rayleigh (input_struct input, output_struct* output) {
  ...
  case CRS_MODEL_BODHAINE:
    ...
    status = crs_rayleigh_bodhaine (output->wl.lambda_r,
                                    output->wl.nlambda_r,
                                    output->mixing_ratio[MX_CO2],
                                    input.rayleigh_depol,
                                    &(output->rayleigh_depol),
                                    &crs);
    if (status != 0) {
  ...
  case CRS_MODEL_BODHAINE29:
    ...
    status = crs_rayleigh_bodhaine29 (output->wl.lambda_r, output->wl.nlambda_r, &crs);
    if (status != 0) {
  ...
  case CRS_MODEL_PENNDORF:
    ...
    status = crs_rayleigh_penndorf (output->wl.lambda_r,
                                    output->wl.nlambda_r,
                                    output->mixing_ratio[MX_CO2],
                                    input.rayleigh_depol,
                                    &(output->rayleigh_depol),
...
int setup_raman (input_struct input, output_struct* output) {
  ...
  if ((status = ASCII_calloc_double (&output->crs.crs_raman_RL, output->atm.nlev, output->crs.number_of_ramanshifts + 1)) != 0) {
    fprintf (stderr, "Error %d allocating memory for output->crs.crs_raman \n", status);
    fprintf (stderr, "       (line %d, function '%s' in '%s')\n", __LINE__, __func__, __FILE__);
    return status;
  }

  if ((status = ASCII_calloc_double (&output->crs.crs_raman_RG, output->atm.nlev, output->crs.number_of_ramanshifts + 1)) != 0) {
    fprintf (stderr, "Error %d allocating memory for output->crs.crs_raman \n", status);
    fprintf (stderr, "       (line %d, function '%s' in '%s')\n", __LINE__, __func__, __FILE__);
    return status;
  }
...
/*****************************************************/
int read_crs_from_files (input_struct input, output_struct* output, int first, int** molabs_src) {
...
static int crs_o3 (crs_out_struct* crs_out,
  ...
  if (first) {
    ...
    case CRS_MODEL_BOGUMIL:
      ...
      fact = 1.0E-20;
      break;
    case CRS_MODEL_SERDYUCHENKO:
      strcpy (filenm, "crs/O3_Serdyuchenko_2013.xs");
      fact = 1.0E-20;
      break;
    default:
    ...
    status = read_4c_file_float (fullnm, &lambda_raw, &(coeff_crs[0]), &(coeff_crs[1]), &(coeff_crs[2]), &n_crs);
        ...
        for (iy = 0; iy < Nyatm; iy++) {
          for (it = 0; it < ntemp; it++) {
            if (ia > (n_crs - 1) || ib < 0)
              ...
              a = coeff_crs[0][ia] + coeff_crs[1][ia] * (temper[ix][iy][it] - 273.15) +
                  coeff_crs[2][ia] * (temper[ix][iy][it] - 273.15) * (temper[ix][iy][it] - 273.15);
              if (ia == ib) {
                crs_out->crs[ix][iy][it][MOL_O3][iv] = a * fact;
              } else {
                b = coeff_crs[0][ib] + coeff_crs[1][ib] * (temper[ix][iy][it] - 273.15) +
...
static int crs_co2 (crs_out_struct* crs_out,
  ...
  if (first) {
    ...
    if (lambda[0] <= 200.0 && lambda[n_lambda - 1] >= 118.0) {
      ...
      status = read_2c_file_float (fullnm, &lambda_raw, &coeff_crs, &n_crs);
...
static int crs_o2 (crs_out_struct* crs_out,
  ...
  int           n_crs_Yo88 = 0, n_crs_Yo2005 = 0, n_crs_Og75 = 0, n_crs_Le83 = 0, status = 0, n_tmp = 0;
  unsigned int  n_crs_SR                      = 0;
  ...
  if (first) {
    ...
    if (lambda[0] <= (float)(1e7 / 49000.5) && lambda[n_lambda - 1] >= (float)(1e7 / 57000.0)) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        status = read_4c_file_float (fullnm, &tmp, &(crs_SR[i][0]), &(crs_SR[i][1]), &(crs_SR[i][2]), &n_tmp);
    ...
    if (lambda[0] <= 240.0 && lambda[n_lambda - 1] >= 205.0) {
      ...
      status = read_2c_file_float (fullnm, &lambda_Yo88, &crs_Yo88, &n_crs_Yo88);
    ...
    if (lambda[0] <= (float)(1e7 / 57000.0) && lambda[n_lambda - 1] >= 160.0) {
      ...
      status = read_2c_file_float (fullnm, &lambda_Yo2005, &crs_Yo2005, &n_crs_Yo2005);
    ...
    if (lambda[0] <= (160.0) && lambda[n_lambda - 1] >= 108.0) {
      ...
      status = read_2c_file_float (fullnm, &lambda_Og75, &crs_Og75, &n_crs_Og75);
    ...
    if (lambda[0] <= (121.9) && lambda[n_lambda - 1] >= 121.4) {
      ...
      status = read_3c_file_float (fullnm, &lambda_Le83, &crs_Le83_288, &crs_Le83_alpha, &n_crs_Le83);
    ...
    if (molabs_src[iv] == MOLABS_SRC_CALC) {
      ...
      if (lambda[iv] >= 194.0 && lambda[iv] < 205.0)
        for (ix = 0; ix < Nxatm; ix++) {
          for (iy = 0; iy < Nyatm; iy++) {
            for (it = 0; it < ntemp; it++)
              crs_out->crs[ix][iy][it][MOL_O2][iv] +=
                6.884 * (204.87 / lambda[iv]) * exp (-69.7374 * pow (log (204.87 / lambda[iv]), 2)) * 1e-24;
          }
        }
...
static int crs_no2 (crs_out_struct* crs_out,
  ...
  case CRS_MODEL_BOGUMIL:
    if (first) {
      ...
      fact = 1.0E-20;
      strcat (fullnm, filenm);
      status = read_4c_file (fullnm, &lambda_raw, &(coeff_crs[0]), &(coeff_crs[1]), &(coeff_crs[2]), &n_crs);
  ...
  case CRS_MODEL_BURROWS:
    ...
    status = linear_coeffc (lambda_raw, coeffc_raw, n_raw, &a0, &a1, &a2, &a3);
    if (status != 0) {
    ...
    for (iv = 0; iv < n_lambda; iv++) {
      if (molabs_src[iv] == MOLABS_SRC_CALC) {
        if (lambda[iv] < lambda_raw[0] || lambda[iv] > lambda_raw[n_raw - 1])
          ...
          status = calc_splined_value (lambda[iv], &crs, lambda_raw, n_raw, a0, a1, a2, a3);
        if (status != 0) {
    ...
    free (coeffc_raw);
    break;
  case CRS_MODEL_VANDAELE:
    if (first) {
      strcpy (filenm, "crs/no2_vandaele_220K.dat");
      strcpy (fullnm, path);
      strcat (fullnm, filenm);
      status = read_2c_file (fullnm, &lambda_raw, &(coeff_crs[0]), &n_crs);

      strcpy (filenm, "crs/no2_vandaele_294K.dat");
      strcpy (fullnm, path);
      strcat (fullnm, filenm);
      status = read_2c_file (fullnm, &lambda_raw, &(coeff_crs[1]), &n_crs);
          ...
          for (iy = 0; iy < Nyatm; iy++) {
            for (it = 0; it < ntemp; it++) {
              if (ia > (n_crs - 1) || ib < 0)
                ...
                a = (coeff_crs[1][ia] - coeff_crs[0][ia]) / (294. - 220.) * (temper[ix][iy][it] - 220.) + coeff_crs[0][ia];

                if (ia == ib) {
                  crs_out->crs[ix][iy][it][MOL_NO2][iv] = a * fact;
                } else {
                  b        = (coeff_crs[1][ib] - coeff_crs[0][ib]) / (294. - 220.) * (temper[ix][iy][it] - 220.) + coeff_crs[0][ib];
                  gradient = (a - b) / (lambda_raw[ia] - lambda_raw[ib]);
...
static int crs_oclo (crs_out_struct* crs_out,
  ...
  if (first) {
    ...
    status = read_4c_file_float (fullnm, &lambda_raw, &(crs_at_T[0]), &(crs_at_T[1]), &(crs_at_T[2]), &n_crs);
...
static int crs_o4 (crs_out_struct* crs_out,
  ...
  switch (type) {
  case CRS_MODEL_GREENBLATT:
    strcpy (filenm, "crs/crs_o4_greenblatt.dat");
    break;
  case CRS_MODEL_THALMAN:
    strcpy (filenm, "crs/crs_o4_thalman_volkamer_2013.dat");
    break;
  default:
...
static int read_crs (crs_out_struct* crs_out,
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d calculating interpolation coefficients for cross section file %s\n",
             status,
             crs_out->filename[mol_id]);
    return status;
  }
...
int setup_crs_from_lookup (float*          lambda,
  ...
  if (status) {
    ...
    for (i = 0; i < nwvl; i++) {
      ...
      start[1] = 0;
      count[0] = 1;
      count[1] = nspecies_in_reptran_file;
  ...
  for (i_mol = 1; i_mol < MOL_NN; i_mol++) {
    ...
    if (ispecies_in_reptran_file[i_mol] > -1)
      for (j = 0; j < n_lambda; j++)
        if (iwvl_in_reptran_file[j] > -1 && cross_section_source[iwvl_in_reptran_file[j]][ispecies_in_reptran_file[i_mol]] == 1)
...
int calc_crs_from_lookup (float*          lambda,
  ...
  if (status != 0) {
    ...
    status += nc_inq_varid (
      ncid,
      "wvl_index",
      &varid); /* wvl_index links the wavelengths in the lookup table file to the wavelengths in the representative wavelengths file */
    ...
    if (n_active_wvl ==
        0) /* this should not happen because calc_crs_from_lookup is called only if at least one wavelength was selected */
      ...
      for (i_wvl = 0; i_wvl < n_lambda; i_wvl++)
        if (iwvl_active[i_wvl] > -1)  // read the cross sections only at the wavelengths where they are needed
          for (i_vmr = 0; i_vmr < n_vmrs; i_vmr++) {
            start[0] = i_t_pert;
            start[1] = i_vmr;
            start[2] = iwvl_in_lookup_file[i_wvl];
            start[3] = 0;
            count[0] = 1;
            count[1] = 1;
            count[2] = 1;
      ...
      for (iy = 0; iy < atm->Nyatm; iy++) {
        for (i_lev = 0; i_lev < atm->nlev; i_lev++) {
          ...
          if (press_tmp < pressure[n_pressure - 1]) {
            if (!quiet) {
              fprintf (stderr,
                       "     Warning: Pressure at lev %d is %e Pa, thus lower than %e Pa, which is the lower limit in the "
                       "absorption lookup table.\n",
                       i_lev,
                       press_tmp,
                       pressure[n_pressure - 1]);
              fprintf (stderr,
                       "       Using %e Pa for calculating the absorption cross sections at this level.\n",
                       pressure[n_pressure - 1]);
            }
            ...
            if (!quiet) {
              fprintf (stderr,
                       "     Warning: Pressure at lev %d is %e Pa, thus higher than %e Pa, which is the upper limit in the "
                       "absorption lookup table.\n",
                       i_lev,
                       press_tmp,
                       pressure[0]);
          ...
          if (n_vmrs > 1) {
            ...
            if (i_mol != MOL_H2O) {
              fprintf (
                stderr,
                "Error: The absorption lookup table %s considers mixing ratio dependence for a gas species other than H2O.\n",
                lookup_file);
              return -1;
            }
            ...
            if (vmr < vmrs[0]) {
              if (!quiet) {
                fprintf (stderr,
                         "     Warning: VMR at lev %d is lower than %e, which is the lower limit in the absorption lookup table.\n",
                         i_lev,
                         vmrs[0]);
              ...
              if (!quiet) {
                fprintf (
                  stderr,
                  "     Warning: VMR at lev %d is higher than %e, which is the upper limit in the absorption lookup table.\n",
                  i_lev,
                  vmrs[n_vmrs - 1]);
                fprintf (stderr,
                         "       Using %e for calculating the absorption cross sections at this level.\n",
                         vmrs[n_vmrs - 1]);
              }
              vmr = vmrs[n_vmrs - 1];
            }
          ...
          for (i_p = 0; i_p < 2; i_p++) {
            ...
            if (dt < t_pert[0]) {
              if (!quiet) {
                fprintf (stderr,
                         "     Warning: Temperature pertubation at lev %d is %e K, thus lower than %e K, which is the lower limit "
                         "in the absorption lookup table.\n",
                         i_lev,
                         dt,
                         t_pert[0]);
              ...
              if (!quiet) {
                fprintf (stderr,
                         "     Warning: Temperature pertubation at lev %d is %e K, thus higher than %e K, which is the upper limit "
                         "in the absorption lookup table.\n",
                         i_lev,
                         dt,
                         t_pert[n_t_pert - 1]);
                fprintf (stderr,
                         "       Using %e K for calculating the absorption cross sections at this level.\n",
                         t_pert[n_t_pert - 1]);
              }
                ...
                for (i_t = 0; i_t < 2; i_t++) {
                  ...
                  if (n_vmrs > 1) {
                    for (i_vmr = 0; i_vmr < 2; i_vmr++) {
                      xsec_pre_vmrs[i_vmr] =
                        (double)xsec[i_lower_t + i_t][i_lower_vmr + i_vmr][iwvl_active[i_wvl]][i_lower_p + i_p];
                    }
                    xsec_pre_t[i_t] = xsec_pre_vmrs[0] + (xsec_pre_vmrs[1] - xsec_pre_vmrs[0]) * (vmr - vmrs[i_lower_vmr]) /
                                                           (vmrs[i_lower_vmr + 1] - vmrs[i_lower_vmr]);
                  } else {
                    xsec_pre_t[i_t] = (double)xsec[i_lower_t + i_t][0][iwvl_active[i_wvl]][i_lower_p + i_p];
                  }
                }

                xsec_pre_interpolated[iwvl_active[i_wvl]][i_p] = xsec_pre_t[0] + (xsec_pre_t[1] - xsec_pre_t[0]) *
                                                                                   (dt - t_pert[i_lower_t]) /
                                                                                   (t_pert[i_lower_t + 1] - t_pert[i_lower_t]);
              }
            }
          }
          ...
          for (i_wvl = 0; i_wvl < n_lambda; i_wvl++) {
            if (iwvl_active[i_wvl] > -1) {
              ...
              crs_out->crs[ix][iy][i_lev][i_mol][i_wvl] = xsec_pre_p[0] + (xsec_pre_p[1] - xsec_pre_p[0]) *
                                                                            (press_tmp - pressure[i_lower_p]) /
                                                                            (pressure[i_lower_p + 1] - pressure[i_lower_p]);
              crs_out->crs[ix][iy][i_lev][i_mol][i_wvl] *=
                1e-16; /* The lookup table file contains the cross sections in units of 10^(-20)m^2; here we need cm^2, thus we multiply with 10^(-16). */
              ...
              if (crs_out->crs[ix][iy][i_lev][i_mol][i_wvl] < 0) { /* Negative absorption cross sections are set to zero */
                ...
                if (!quiet)
                  fprintf (stderr,
                           "Warning in calc_crs_from_lookup: The absorption cross section of %s in grid cell %d,%d,%d at %e nm "
                           "wavelength is negative; set to zero!\n",
                           gas_number2string (i_mol),
                           ix,
                           iy,
                           i_lev,
