...
static int read_2D_elevation (char* filename, int* Nx, int* Ny, double* delX, double* delY, double*** elevation, int quiet);
...
int setup_elevation2D (char* filename, int Nx, int Ny, double delX, double delY, elevation_struct* elev, int visualize, int quiet) {
  int      status = 0;
  int      ie = 0, je = 0;
  ...
  status = read_2D_elevation (filename, &(elev->Nx), &(elev->Ny), &(elev->delX), &(elev->delY), &elevation2D, quiet);
  ...
  if (!equal ((double)(elev->Nx) * elev->delX, (double)Nx * delX)) {
    fprintf (stderr,
             "Error, x-size of elevation area (%g) does not equal x-size of sample area (%g)\n",
  ...
  if (!equal ((double)(elev->Ny) * elev->delY, (double)Ny * delY)) {
    fprintf (stderr,
             "Error, y-size of elevation area (%g) does not equal y-size of sample area (%g)\n",
  ...
  for (ie = 0; ie <= elev->Nx; ie++)
    if (fabs (elevation2D[ie][0] - elevation2D[ie][elev->Ny]) > 0) {
      ...
      fprintf (stderr,
               "elevation2D [%d][%d] = %g, elevation2D [%d][%d] = %g\n",
               ie,
               0,
               elevation2D[ie][0],
               ie,
               elev->Ny,
               elevation2D[ie][elev->Ny]);
      return -1;
    }
  ...
  if (visualize) {
    ...
    GLmystic_setdomain (GLMYSTIC_ELEVATION, 0, 0, 0, (double)(elev->Nx) * elev->delX, (double)(elev->Ny) * elev->delY, 0);
