...
int conv (char* specfilename, char* convfilename, int* rows, int* columns, double** x, double*** ys) {
  ...
  /* read file */
  status = ASCII_file2double (specfilename, rows, &max_columns, &min_columns, &data);
  if (status != 0) {
  ...
  *x = ASCII_column (data, *rows, 0);
  for (k = 1; k < min_columns; k++) {
    ...
    if ((status = int_convolute (*x, y_spec, *rows, x_conv, y_conv, n_conv, &((*ys)[k - 1]), 0)) < 0) {
