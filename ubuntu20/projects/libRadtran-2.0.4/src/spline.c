    ...
    if (user_x) {
      ...
      if (!linear) {         /* spline interpolation */
        if (weighting < 0) { /* interpolating spline */
          if (!quiet)
            fprintf (stderr, " ... spline interpolation\n");
          if ((status = spline (x, y, rows, start, step, &newnumber, &new_x, &new_y)) != 0) {
          ...
          if ((status = appspl (x, y, w, rows, start, step, &newnumber, &new_x, &new_y)) != 0) {
            fprintf (stderr, "sorry, cannot do interpolation!\n");
        ...
        if ((status = linear_eqd (x, y, rows, start, step, &newnumber, &new_x, &new_y)) != 0) {
          fprintf (stderr, "sorry, cannot do interpolation!\n");
