...
static inline double legendre_polynomial (double p1, double p0, double mu, int l);
...
static int
get_options (int argc, char** argv, char* programname, char* infilename, int* n_legendre, int* grid_res, int* coeff, int* norm) {
  int   c     = 0;
  char* dummy = NULL;
  ...
  *norm       = 0;
  ...
  strncpy (infilename, argv[optind + 0], FILENAME_MAX - 1);
...
int main (int argc, char** argv) {
  ...
  status = get_options (argc, argv, programname, infilename, &n_legendre, &grid_res, &coeff, &norm);
  ...
  for (l = 0; l < n_legendre; l++) {
    if (coeff) /* Factor to obtain Legendre coefficients,*/
