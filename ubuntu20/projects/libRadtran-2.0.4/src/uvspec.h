...
#define FN_NN                                                                                                                      \
  40 /* Number of general files, increase 
...
#define FN_AER_NN                                                                                                                  \
  9 /* Number of aerosol related files, increase 
...
/* Number of mystic related files, increase when adding new mystic files */
#define FN_MC_NN 21
...
#define FN_MC_RPV 7
#define FN_MC_RPV2D_TYPE 8
#define FN_MC_RPV_SPECTRAL 9
#define FN_MC_ROSSLI 10
#define FN_MC_LIDAR 11
#define FN_MC_UMU 12
#define FN_MC_SUNSHAPE_FILE 13
#define FN_MC_AERIS 14
#define FN_MC_AIRMASS 15
#define FN_MC_AMBRALS 16
#define FN_MC_AMBRALS_TYPE 17
#define FN_MC_AMBRALS_SPECTRAL 18
#define FN_MC_TRIANGULAR_SURFACE 19
#define FN_MC_PANORAMA_ANGLES 20
...
#define CK_ABS_NN                                                                                                                  \
  7 /* implemented for ck_lowtran_absorption (old: absorption); Merge with molecules for atmospheric molecules for spectral calculations (MOL_NN) */
#define CK_ABS_O4 0
#define CK_ABS_N2 1
#define CK_ABS_CO 2
#define CK_ABS_SO2 3
...
#define CRS_MODEL_BODHAINE 1      /* rayleigh*/
#define CRS_MODEL_NICOLET 2       /* rayleigh*/
#define CRS_MODEL_PENNDORF 3      /* rayleigh*/
#define CRS_MODEL_BODHAINE29 4    /* rayleigh*/
#define CRS_MODEL_BASS_AND_PAUR 5 /* o3 */
#define CRS_MODEL_MOLINA 6        /* o3 */
#define CRS_MODEL_DAUMONT 7       /* o3 */
#define CRS_MODEL_SERDYUCHENKO 8  /* o3 */
#define CRS_MODEL_BOGUMIL 9       /* o3 and no2 */
#define CRS_MODEL_BURROWS 10      /* no2 */
#define CRS_MODEL_VANDAELE 11     /* no2 */
#define CRS_MODEL_GREENBLATT 12   /* o4 */
#define CRS_MODEL_THALMAN 13      /* o4 */
...
#define SOLVER_TWOMAXRND 19   /* twostream maximum random overlap, Nina Crnivec */
#define SOLVER_TWOMAXRND3C 20 /* twostream maximum random overlap with tripleclouds, Nina Crnivec */
#define SOLVER_TWOSTR 21
#define SOLVER_DISORT 22
#define SOLVER_SSLIDAR 23
#define SOLVER_DYNAMIC_TWOSTREAM 24
#define SOLVER_DYNAMIC_TENSTREAM 25
...
#define PAN_MODE_FILE 3

#define PAN_FLAG_NN 7
...
#define PAN_FLAG_TRANSMITTANCE 6
...
typedef struct {
  ...
  int isstep; /* x increment for backward pixel index */
  int jsstep; /* y increment for backward pixel index */
...
typedef struct {
  ...
  int   nca;          // Neighboring Column Approximation; Carolin Klinger 2019
  char* nca_options;  //  chose version of NCA
  ...
  int   core_isactive;  /* Activate massive delta scaling in the hidden core */
  float core_threshold; /* Distance of the hidden core from the cloud boundary in units of tau_scat*/
  float core_scale;     /*delta scaling factor to be used inside the hidden core. set f=g if <0*/
  char* core_inputfile; /* Read the hidden core from the given file*/
  char* core_savefile;  /* Save the hidden core to the given file*/
  ...
  int*   pan; /* panorama flag (distr_photons_over_pixel,np_pixel,quicklook,with_direct_rad,weight_with_cos,transmittance) */
...
typedef struct {
  ...
  float*        phi;
  float*        umu;
  float         twomaxrnd3C_scale_cf;
  int           dynamic_iterations; /* number of iterations for dynamic twostream and tenstream */
...
typedef struct {
  float*
        rpv; /* sigma, t1, t2: Deguenther and Meerkoetter [2000]; t1: snow; includes forward peak; scale: optional scaling factor for the BRDF, to adjust albedo */
  int   source;
...
typedef struct {
  ...
  /* ??? changed to float, BM 7.9.2018 ??? */
  float***** tau_rayleigh_r;  /* wavelength-dependent Rayleigh scattering optical depth   */
  float***** tau_molabs_r;    /* wavelength-dependent molecular absorption optical depth  */
  float***** tau_molabs_md_r; /* wavelength-dependent molecular absorption optical depth  */
...
typedef struct {
  ...
  /* nthreed = number of 3D layers, 3D CAOTHs + all other layers that need to be 3D;     */
  /* currently 3D clouds + close-to-the surface layer where triangles occur; could       */
  /* potentially be used to define layers without 3D clouds where we want 3D output even */
  /* in absence of a 3D cloud                                                            */
  int  nthreed;
...
typedef struct {
  ...
  float**** crs;    /* cross section [ix, iy, layer, quadrature point], changed from double to float, BM, 6.9.2018 */
...
typedef struct {
  ...
  char*
        atmosphere_filename; /*Can be either whole path of atmosphere file or 'subarctic_winter'/'_summer', 'midlatitude_winter'/'_summer', 'tropics', 'US_standard' */
...
typedef struct {
  ...
  float*    dtauc;      /* free'ed */
  float*    dtauc_clr;  /* = dtauc - dtau[i_wc]  - dtau[i_ic]  = optical thickness excluding wc and ic; for twomaxrnd     */
  float*    dtauc_cldk; /* = dtauc - dtau[i_wcn] - dtau[i_wcn] = optical thickness excluding wcn and icn; for twomaxrnd3c */
  float*    dtauc_cldn; /* = dtauc - dtau[i_wck] - dtau[i_ick] = optical thickness excluding wck and ick; for twomaxrnd3c */
  float*    dtauc_md;   /* free'ed */
  float*    ssalb;
  float*    ssalbR;
  float*    ssalb_clr;  /* = ssalb excluding wc and ic; for twomaxrnd     */
  float*    ssalb_cldk; /* = ssalb excluding wcn and icn; for twomaxrnd3c */
  float*    ssalb_cldn; /* = ssalb excluding wck and ick; for twomaxrnd3c */
  float*    ssalbRL;
  float*    utau;
  int       nphamat;
  float***  pmom;
  float*    pmom01_clr;  /* first moment (= asymmetry parameter) excluding wc and ic; for twomaxrnd     */
  float*    pmom01_cldk; /* first moment (= asymmetry parameter) excluding wcn and icn; for twomaxrnd3c */
  float*    pmom01_cldn; /* first moment (= asymmetry parameter) excluding wck and ick; for twomaxrnd3c */
  int**     ntheta;
  float***  theta;
  double*** mu;
  float***  phase;
  ...
  int isstep; /* increment for backward pixel index; only every  */
  int jsstep; /* isstep'th / jsstep'th pixel is sampled.         */
  ...
  // triangular surface output, i.e. irradiance at triangles
  struct t_triangle_radiation_field** triangle_results_r;  // internal wavelength grid
  struct t_triangle_radiation_field** triangle_results_t;  // transmittance grid
  struct t_triangle_radiation_field** triangle_results_o;  // output grid
} output_struct;
...
int get_caothoff_index (caothoff_inp_struct** caothoff, int* n_caothoff, char* name);
int get_caoth_index (caoth_inp_struct** caoth, int* n_caoth, char* name, int old_input);
