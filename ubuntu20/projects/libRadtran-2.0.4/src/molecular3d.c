...
/************************************/
static int read_atmosphere_3d (char*           filename,
                               atm_out_struct* out,
                               float*          mol_mass,
...
static int write_dummy_profile_file (char* filename, int quiet, int verbose);

static int
redistribute_3D_temp (float**** data, int nx, int ny, float* z_old, int nlyr_old, float* z_new, int nlyr_new, double* temper_1D);
...
int setup_molecular3d (input_struct input, output_struct* output) {
  ...
  if (!input.atmosphere3d)
    ...
    status = write_dummy_profile_file (input.atmosphere3d_filename, input.quiet, input.verbose);
...
int setup_optprop_molecular3d (input_struct input, output_struct* output) {
  ...
  if (!input.atmosphere3d)
    ...
    ASCII_calloc_double_3D_arylen_restricted (&tmp_molabs,
                                              output->atm.nlev_common - 1,
                                              output->wl.nlambda_r,
                                              output->wl.nlambda_rte_lower,
                                              output->wl.nlambda_rte_upper,
                                              output->atm.nq_r);
    ASCII_calloc_double_3D_arylen_restricted (&tmp_rayleigh,
                                              output->atm.nlev_common - 1,
                                              output->wl.nlambda_r,
                                              output->wl.nlambda_rte_lower,
                                              output->wl.nlambda_rte_upper,
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d setting up absorption cross sections for 3D atmosphere in %s (%s)\n",
               status,
               function_name,
               file_name);
      return status;
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d setting up Rayleigh cross sections for 3D atmosphere in %s (%s)\n",
               status,
               function_name,
               file_name);
      return status;
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d setting up optical properties of trace gases for 3D atmosphere in %s (%s)\n",
               status,
               function_name,
               file_name);
      return status;
    ...
    redistribute_molecular (&(output->atm.optprop),
                            output->wl.nlambda_r,
                            output->wl.nlambda_rte_lower,
                            output->wl.nlambda_rte_upper,
                            output->atm.nq_r,
    ...
    redistribute_3D_temp (&(output->atm.microphys.temper),
                          output->atm.Nxatm,
                          output->atm.Nyatm,
                          output->atm.zd,
                          output->atm.nlev,
    ...
    for (ix = 0; ix < output->atm.Nxatm; ix++)
      for (iy = 0; iy < output->atm.Nyatm; iy++)
        for (lc = 0; lc < output->atm.nlev_common - 1; lc++)
          output->atm.microphys.temper_avg[ix][iy][lc] =
            0.5 * (output->atm.microphys.temper[ix][iy][lc] + output->atm.microphys.temper[ix][iy][lc + 1]);
    ...
    */
    ...
    if (!(output->mc.kabs3D = calloc_float_5D_restricted ((size_t)output->atm.Nxatm,
                                                          (size_t)output->atm.Nyatm,
                                                          (size_t)output->atm.nlev_common - 1,
    ...
    if (!(output->mc.ksca3D = calloc_float_5D_restricted ((size_t)output->atm.Nxatm,
                                                          (size_t)output->atm.Nyatm,
                                                          (size_t)output->atm.nlev_common - 1,
                                                          (size_t)output->wl.nlambda_rte_lower,
                                                          (size_t)output->wl.nlambda_rte_upper,
        ...
        for (lc = 0; lc < output->atm.nlev_common - 1; lc++) {
          for (iv = output->wl.nlambda_rte_lower; iv <= output->wl.nlambda_rte_upper; iv++) {
            for (iq = 0; iq < output->atm.nq_r[iv]; iq++) {
              ...
              output->mc.kabs3D[ix][iy][lc][iv][iq] =
                output->atm.optprop.tau_molabs_r[ix][iy][lc][iv][iq] / (output->atm.zd_common[lc] - output->atm.zd_common[lc + 1]);

              output->mc.ksca3D[ix][iy][lc][iv][iq] = output->atm.optprop.tau_rayleigh_r[ix][iy][lc][iv][iq] /
                                                      (output->atm.zd_common[lc] - output->atm.zd_common[lc + 1]);
    ...
    */
    ...
    default:
      ...
      ASCII_free_float_5D (output->crs.crs, output->atm.Nxatm, output->atm.Nyatm, output->atm.nlev, MOL_NN);
      break;
    }
    ...
    */
    ...
    if (!(output->atm.optprop.tau_molabs_r = calloc_float_5D_restricted (1,
                                                                         1,
                                                                         (size_t)output->atm.nlev_common - 1,
                                                                         (size_t)output->wl.nlambda_rte_lower,
                                                                         (size_t)output->wl.nlambda_rte_upper,
    ...
    if (!(output->atm.optprop.tau_rayleigh_r = calloc_float_5D_restricted (1,
                                                                           1,
                                                                           (size_t)output->atm.nlev_common - 1,
                                                                           (size_t)output->wl.nlambda_rte_lower,
                                                                           (size_t)output->wl.nlambda_rte_upper,
...
/*                                                                                 */
/***********************************************************************************/
int optical_properties_molecular3d (input_struct input, output_struct* output, caoth3d_out_struct* caoth3d, int iv, int iq) {
    ...
    for (iy = 0; iy < Ny; iy++) {
      for (lc = start_lc; lc < nlyr; lc++) {
        ...
        case CK_FU:
          /* only in the Fu and Liou case, the Rayleigh scattering */
          /* cross section depends on the subband                  */
          output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy] =
            0.001 * (output->mc.ksca3D[ix][iy][lc][iv][iq] + output->mc.kabs3D[ix][iy][lc][iv][iq]);
          if (output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy] > 0)
            output->caoth3d[isp_mol].ssa[nlyr - lc - 1][ix][iy] =
              0.001 * output->mc.ksca3D[ix][iy][lc][iv][iq] / output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy];
        ...
        case CK_REPTRAN_CHANNEL:
          output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy] =
            0.001 * (output->mc.ksca3D[ix][iy][lc][iv][0] + output->mc.kabs3D[ix][iy][lc][iv][iq]);
          if (output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy] > 0)
            output->caoth3d[isp_mol].ssa[nlyr - lc - 1][ix][iy] =
              0.001 * output->mc.ksca3D[ix][iy][lc][iv][0] / output->caoth3d[isp_mol].ext[nlyr - lc - 1][ix][iy];
          else /* avoid dividing 0/0 in case of no extinction */
            output->caoth3d[isp_mol].ssa[nlyr - lc - 1][ix][iy] = 0;
          break;
        default:
...
static int read_atmosphere_3d (char*           filename,
  ...
  if (read_netcdf == 1) {
    ...
    if (!(tmpdata = calloc_float_3D (out->nlev, out->Nxatm, out->Nyatm, "tmpdata")))
      return -1;
  ...
  for (ix = 0; ix < out->Nxatm; ix++) {
    for (iy = 0; iy < out->Nyatm; iy++) {
      for (iz = 0; iz < out->nlev; iz++) {
        out->microphys.dens[MOL_AIR][ix][iy][iz] =
          out->microphys.press[ix][iy][iz] * 1E-04 / (BOLTZMANN * out->microphys.temper[ix][iy][iz]);

        /* convert from kg/kg (specific humidity) to concentration 1/cm^3  */
        out->microphys.dens[MOL_H2O][ix][iy][iz] = mol_mass[MOL_AIR] / mol_mass[MOL_H2O] *
                                                   out->microphys.dens[MOL_H2O][ix][iy][iz] *
                                                   out->microphys.dens[MOL_AIR][ix][iy][iz];
      }
      ...
      status = average_dens (out->microphys.temper[ix][iy],
                             ...
                             &(out->microphys.temper_avg[ix][iy]),
                             NO);
      ...
      status = average_dens (out->microphys.dens[MOL_AIR][ix][iy],
                             ...
                             &(out->microphys.dens_avg[MOL_AIR][ix][iy]),
                             NO);

      if (status != 0) {
        fprintf (stderr,
                 "Error %d calculating average air density (line %d, function '%s' in '%s')\n",
                 status,
                 __LINE__,
                 __func__,
                 __FILE__);
      ...
      status = average_dens (out->microphys.dens[MOL_H2O][ix][iy],
                             ...
                             out->nlyr + 1,
                             interpol_method_gas[MOL_H2O],
                             &(out->microphys.dens_avg[MOL_H2O][ix][iy]),
                             NO);

      if (status != 0) {
        fprintf (stderr,
                 "Error %d calculating average water vapor density (line %d, function '%s' in '%s')\n",
                 status,
                 __LINE__,
                 __func__,
                 __FILE__);
...
static int
redistribute_3D_temp (float**** data, int nx, int ny, float* z_old, int nlev_old, float* z_new, int nlev_new, double* temper_1D) {
  int      lc = 0, status = 0;
  int      ix = 0, iy = 0;
  ...
  for (ix = 0; ix < nx; ix++) {
    for (iy = 0; iy < ny; iy++) {
      ...
      status = arb_wvn (nlev_old, z_old, tmp[ix][iy], nlev_tmp, z_tmp, data_tmp, 1, 1);
