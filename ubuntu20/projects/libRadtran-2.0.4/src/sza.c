...
static int get_sza_from_time_and_location (float     lat,
                                           float     lon,
                                           struct tm UTC,
                                           float     delta_time_start,
                                           float     delta_time_end,
                                           float*    sunshine_fraction,
                                           float*    sza,
                                           float*    phi0,
                                           int       quiet,
                                           int       verbose);
...
int setup_sza (input_struct input, output_struct* output) {
  ...
  if (input.source == SRC_SOLAR) {
    ...
    case SZA_BY_TIME_AND_LOCATION:
      ...
      /*            sza_tmp = 0, phi_tmp = 0                       */
      status = get_sza_from_time_and_location (input.latitude,
                                               input.longitude,
                                               input.UTC,
                                               input.delta_time_start,
                                               input.delta_time_end,
                                               &(output->sunshine_fraction),
                                               &(sza_tmp),
                                               &(phi0_tmp),
                                               input.quiet,
                                               input.verbose);
      ...
      if (status != 0) {
        fprintf (stderr,
                 "Error %d returned by get_sza_from_time_and_location (line %d, function %s in %s) \n",
                 status,
                 __LINE__,
                 __func__,
                 __FILE__);
    ...
    case SZA_FROM_SZA_FILE:
      ...
      if (output->atm.file_lambda_sza[output->atm.file_nlambda_sza - 1] < output->wl.lambda_r[output->wl.nlambda_r - 1])
        output->atm.file_lambda_sza[output->atm.file_nlambda_sza - 1] = output->wl.lambda_r[output->wl.nlambda_r - 1];

      /**** Interpolate sza to internal wavelength grid ****/

      linear = 1; /* changed to linear, BM March 29, 2001 */
      status = arb_wvn (output->atm.file_nlambda_sza,
                        output->atm.file_lambda_sza,
                        output->atm.file_sza,
                        output->wl.nlambda_r,
                        output->wl.lambda_r,
                        output->atm.sza_r,
...
static int read_sza (char* sza_filename, atm_inp_struct* atm_inp, atm_out_struct* atm_out) {
  ...
  status = ASCII_file2float (sza_filename, &rows, &max_columns, &min_columns, &sza_data);
  if (status != 0) {
