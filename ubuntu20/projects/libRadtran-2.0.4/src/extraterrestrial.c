...
int setup_extraterrestrial (input_struct input, output_struct* output) {
  ...
  case CK_FILE:
    if (!input.quiet)
      if (strlen (input.filename[FN_EXTRATERRESTRIAL]) > 0)
        fprintf (stderr,
                 " ... ignoring %s, reading extraterrestrial flux from %s\n",
  ...
  if (loc == NULL &&
      (input.ck_scheme == CK_KATO || input.ck_scheme == CK_KATO2 || input.ck_scheme == CK_KATO2ANDWANDJI ||
       input.ck_scheme == CK_KATO2_96) &&
      !input.quiet) /* no match found */
    fprintf (stderr,
  ...
  case CK_CRS:
    ...
    if (input.source == SRC_SOLAR && (output->spectrum_unit != UNIT_PER_NM && output->spectrum_unit != UNIT_NOT_DEFINED) &&
        !input.quiet) {
    ...
    if (output->wl.use_reptran) {
      for (i_h = 0; i_h < output->wl.nlambda_h; i_h++) {
        ...
        for (i = 0; i < output->wl.nlambda_in_reptran_band[i_band]; i++) {
          output->wl.fbeam[i_h] +=
            output->wl.weight_reptran_band[i_band][i] * output->wl.extra_reptran_r[output->wl.reptran_band[i_band][i]];
  ...
  if (strlen (input.filename[FN_FILTERFUNCTION]) > 0) {
    ...
    status = arb_wvn (n_flt, x_flt, y_flt, output->wl.nlambda_h, output->wl.lambda_h, output->wl.filter, 1, 0);
  ...
  if (input.filter_function_normalize) {
    for (iv = 1; iv < output->wl.nlambda_h; iv++)
      filter_sum +=
        0.5 * (output->wl.filter[iv] + output->wl.filter[iv - 1]) * (output->wl.lambda_h[iv] - output->wl.lambda_h[iv - 1]);
    for (iv = 0; iv < output->wl.nlambda_h; iv++)
...
static int read_extraterrestrial (char*          filename,
  ...
  /* read extraterrestrial irradiance */
  if (is_cdf == 0) {
    status = read_2c_file_float (filename, &tmp_lambda, &tmp_fbeam, &(wl_out->nlambda_h));
    if (status != 0) {
      fprintf (stderr, "Error %d reading %s\n", status, filename);
      return status;
    }
...
static int cp_lambdat_lambdah (output_struct* output) {
  ...
  for (iv = 0; iv < output->wl.nlambda_h; iv++)
    if ((output->wl.use_reptran ||
         ((output->wl.lambda_t[iv] >= output->wl.start) && (output->wl.lambda_t[iv] <= output->wl.end)))) {
