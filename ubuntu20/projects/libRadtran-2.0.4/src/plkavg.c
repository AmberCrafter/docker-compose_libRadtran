...
int main (int argc, char** argv) {
  ...
  char programname[FILENAME_MAX] = "", threefilename[FILENAME_MAX] = "", twofilename[FILENAME_MAX] = "";

  /* get command line options */
  status = get_options (argc, argv, programname, &wvnlo, &wvnhi, &T, threefilename, twofilename, &quiet);
  ...
  if (strlen (threefilename) > 0) {
    ...
    status = read_3c_file_float (threefilename, &awvnlo, &awvnhi, &aT, &n);
    if (status != 0) {
    ...
    if (strlen (twofilename) > 0) {
      ...
      status = read_2c_file_float (twofilename, &awvnlo, &awvnhi, &n);
      if (status != 0) {
