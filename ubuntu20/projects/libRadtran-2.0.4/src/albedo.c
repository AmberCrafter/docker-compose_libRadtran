...
static int read_rossli (char* filename, rossli_out_struct* rossli);
static int read_rpv (char* filename, rpv_out_struct* rpv, float sigma, float t1, float t2, float scale);
static int interpolate_rossli (rossli_out_struct* rossli, float* lambda, int nlambda_lower, int nlambda_upper);
static int interpolate_hapke (hapke_out_struct* hapke, float* lambda, int nlambda_lower, int nlambda_upper);
static int interpolate_rpv (rpv_out_struct* rpv, float* lambda, int nlambda_lower, int nlambda_upper);
static int read_RPV_spectral_BRDFs (char*                filename,
                                    surfaces_out_struct* surf,
                                    float*               lambda,
                                    int                  nlambda,
                                    int                  nlambda_lower,
                                    int                  nlambda_upper,
                                    ...
                                    float                scale,
                                    int                  quiet);
static int read_AMBRALS_spectral_BRDFs (char*                filename,
                                        surfaces_out_struct* surf,
                                        float*               lambda,
                                        int                  nlambda,
                                        int                  nlambda_lower,
                                        int                  nlambda_upper,
                                        int                  quiet);
static int read_spectral_albedos (char*                filename,
                                  surfaces_out_struct* alb,
                                  float*               lambda,
                                  int                  nlambda,
                                  int                  nlambda_lower,
                                  int                  nlambda_upper,
...
int setup_albedo (input_struct input, output_struct* output) {
  ...
  if (input.alb.surf_type_map == TRUE) {
    ...
    status = get_number_from_netCDF_map (input.latitude,
                                         ...
                                         input.filename[FN_SURFACE_TYPE_MAP],
                                         &(output->alb.surface),
                                         TYPE_INT,
                                         input.alb.netCDF_surf_name,
                                         input.verbose,
                                         input.quiet);
    if (status != 0) {
      fprintf (stderr,
               "Error %d reading surface type map from \n %s\n (line %d, function %s in %s) \n",
               status,
               input.filename[FN_SURFACE_TYPE_MAP],
               __LINE__,
               __func__,
    ...
    if (input.alb.source == ALBEDO_FROM_ALBEDO_MAP || input.alb.source == ALBEDO_FROM_EMISSIVITY_MAP) {
      ...
      if (strlen (input.alb.netCDF_alb_name) > 0)
        ...
        if ((status = nc_inq_varid (ncid, "FAL", &id_data)) == NC_NOERR) /* forecast albedo */
          ...
          fprintf (stderr,
                   "Error '%s' while getting id for surface albedo from '%s' \n",
                   nc_strerror (status),
                   input.filename[FN_ALBEDO_MAP]);
        ...
        if ((status = nc_inq_varid (ncid, "EMIS", &id_data)) == NC_NOERR) /* emissivity */
          ...
          fprintf (stderr,
                   "Error '%s' while getting id for surface emissivity from '%s' \n",
                   nc_strerror (status),
                   input.filename[FN_ALBEDO_MAP]);
          fprintf (stderr, "      (line %d, function %s in %s)\n", __LINE__, __func__, __FILE__);
          return -1;
  ...
  case ALBEDO_USER_LIBRARY:
    ...
    if (input.alb.source == ALBEDO_FROM_ALBEDO_FILE) /* copy albedo file name */
      ...
      if (output->alb.surface == NOT_DEFINED_INTEGER) {
    ...
    if (input.alb.source == ALBEDO_USER_LIBRARY && output->alb.surface == 0) {
      ...
      status = arb_wvn2 (output->alb.file_nlambda_albedo,
  ...
  case ALBEDO_IGBP_LIBRARY:
    ...
    if (input.n_caoth > 0 && (input.i_ic != -1 || input.i_wc != -1)) /* cloud has been detected, IGBP wants sza=53 */
      u0 = cos (53. / 360. * 2. * PI);
    else {
      /* no cloud detected */
    ...
    if (output->alb.surface == 0) /* black surface */
      ...
      if (status != 0) {
        fprintf (stderr,
                 "Error %d, while reading file %s in setup_albedo (albedo.c) !\n",
                 status,
                 input.filename[FN_ALBEDO_LIB_WVL]);
        return -1;
      }
      ...
      for (iv = 0; iv < output->wl.nlambda_r; iv++) {
        ...
        if (output->alb.albedo_r[iv] == -999) {
          if (4000.0 <= output->wl.lambda_r[iv] && output->wl.lambda_r[iv] <= 4545.45) {
            ...
            output->alb.albedo_r[iv] = output->alb.file_albedo[14] + (output->wl.lambda_r[iv] - 4000.0) / (4545.45 - 4000.0) *
  ...
  case ROSSLI_FROM_AMBRALS_FILE:
    ...
    if (strlen (filename) == 0) {
      fprintf (stderr,
               "Error (program bug), no filename is given for RossLi input (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
    }
    ...
    if (input.verbose)
      fprintf (stderr,
               " ... read BRDF function (RossLi) from file %s (line %d, function %s in %s)\n",
               filename,
               __LINE__,
               __func__,
               __FILE__);
    ...
    status =
      interpolate_rossli (&(output->rossli), output->wl.lambda_r, output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper);
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by interpolate_rossli() (line %d, function %s in %s)\n",
               status,
               __LINE__,
               __func__,
               __FILE__);
  ...
  case HAPKE_FROM_HAPKE_FILE:
    ...
    if (strlen (filename) == 0) {
      fprintf (stderr,
               "Error (program bug), no filename is given for Hapke input (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
    }
    ...
    if (input.verbose)
      fprintf (stderr,
               " ... read BRDF function (Hapke) from file %s (line %d, function %s in %s)\n",
               filename,
               __LINE__,
               __func__,
               __FILE__);
    ...
    /* interpolate rpv values to internal wavelength grid */
    status = interpolate_hapke (&(output->hapke), output->wl.lambda_r, output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper);
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by interpolate_hapke() (line %d, function %s in %s)\n",
               status,
               __LINE__,
               __func__,
               __FILE__);
  ...
  case RPV_USER_LIBRARY:
    ...
    if (input.rpv.source == RPV_FROM_RPV_FILE) /* copy albedo file name */
      ...
      if (input.rpv.source == RPV_IGBP_LIBRARY) {
        ...
        default:
          fprintf (stderr,
                   "Error: unknown surface type %d while reading IGBP_RPV (line %d, function %s in %s)\n",
                   output->alb.surface,
                   __LINE__,
                   __func__,
                   __FILE__);
      ...
      fprintf (stderr,
               "Program bug during creation of the rpv filename (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
      return -1;
    ...
    if (strlen (filename) == 0) {
      fprintf (stderr,
               "Error (program bug), no filename is given for rpv input (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
    }
    ...
    if (input.verbose)
      fprintf (stderr,
               " ... read BRDF function (RPV) from file %s (line %d, function %s in %s)\n",
               filename,
               __LINE__,
               __func__,
               __FILE__);

    status = read_rpv (filename,
                       &(output->rpv),
                       input.rpv.rpv[BRDF_RPV_SIGMA],
                       input.rpv.rpv[BRDF_RPV_T1],
                       input.rpv.rpv[BRDF_RPV_T2],
                       input.rpv.rpv[BRDF_RPV_SCALE]);
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by read_rpv from %s (line %d, function %s in %s)\n",
               status,
               input.filename[FN_RPV],
               __LINE__,
               __func__,
               __FILE__);
      return status;
    ...
    /* interpolate rpv values to internal wavelength grid */
    status = interpolate_rpv (&(output->rpv), output->wl.lambda_r, output->wl.nlambda_rte_lower, output->wl.nlambda_rte_upper);
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by interpolate_rpv() (line %d, function %s in %s)\n",
               status,
               __LINE__,
               __func__,
               __FILE__);
  ...
  /* 2D spectral RPV BRDF */
  if (strlen (input.rte.mc.filename[FN_MC_RPV_SPECTRAL]) > 0) {

    status = read_RPV_spectral_BRDFs (input.rte.mc.filename[FN_MC_RPV_SPECTRAL],
                                      &(output->surfaces),
                                      output->wl.lambda_r,
                                      output->wl.nlambda_r,
                                      output->wl.nlambda_rte_lower,
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by read_RPV_spectral_BRDFs from %s (line %d, function %s in %s)\n",
               status,
               input.rte.mc.filename[FN_MC_RPV_SPECTRAL],
               __LINE__,
               __func__,
               __FILE__);
      return status;
  ...
  /* 2D spectral AMBRALS BRDF */
  if (strlen (input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL]) > 0) {

    status = read_AMBRALS_spectral_BRDFs (input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL],
                                          &(output->surfaces),
                                          output->wl.lambda_r,
                                          output->wl.nlambda_r,
                                          output->wl.nlambda_rte_lower,
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by read_AMBRALS_spectral_BRDFs from %s (line %d, function %s in %s)\n",
               status,
               input.rte.mc.filename[FN_MC_AMBRALS_SPECTRAL],
               __LINE__,
               __func__,
               __FILE__);
      return status;
  ...
  if (strlen (input.rte.mc.filename[FN_MC_ALBEDO_SPECTRAL]) > 0) {
    ...
    status = read_spectral_albedos (input.rte.mc.filename[FN_MC_ALBEDO_SPECTRAL],
                                    &(output->surfaces),
                                    output->wl.lambda_r,
                                    output->wl.nlambda_r,
                                    output->wl.nlambda_rte_lower,
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d returned by read_spectral_albedos from %s (line %d, function %s in %s)\n",
               status,
               input.rte.mc.filename[FN_MC_ALBEDO_SPECTRAL],
               __LINE__,
               __func__,
               __FILE__);
      return status;
...
static int read_albedo (char* filename, alb_out_struct* alb_out) {
  ...
  status = read_2c_file_float (filename, &(alb_out->file_lambda_albedo), &(alb_out->file_albedo), &(alb_out->file_nlambda_albedo));
...
static int interpolate_rossli (rossli_out_struct* rossli, float* lambda, int nlambda_lower, int nlambda_upper) {
  ...
  status = arb_wvn2 (rossli->n, rossli->lambda, rossli->iso, nlambda_lower, nlambda_upper, lambda, rossli->iso_r, linear);
  ...
  status = arb_wvn2 (rossli->n, rossli->lambda, rossli->vol, nlambda_lower, nlambda_upper, lambda, rossli->vol_r, linear);
  ...
  status = arb_wvn2 (rossli->n, rossli->lambda, rossli->geo, nlambda_lower, nlambda_upper, lambda, rossli->geo_r, linear);
...
static int interpolate_hapke (hapke_out_struct* hapke, float* lambda, int nlambda_lower, int nlambda_upper) {
  ...
  /* interpolate to internal RTE grid */
  status = arb_wvn2 (hapke->n, hapke->lambda, hapke->w, nlambda_lower, nlambda_upper, lambda, hapke->w_r, linear);
  ...
  status = arb_wvn2 (hapke->n, hapke->lambda, hapke->b0, nlambda_lower, nlambda_upper, lambda, hapke->b0_r, linear);
  ...
  status = arb_wvn2 (hapke->n, hapke->lambda, hapke->h, nlambda_lower, nlambda_upper, lambda, hapke->h_r, linear);
...
static int read_rpv (char* filename, rpv_out_struct* rpv, float sigma, float t1, float t2, float scale) {
  ...
  if (status != 0) {
    ...
    if (status != 0) {
      ...
      status = read_5c_file_float (filename, &(rpv->lambda), &(rpv->rho0), &(rpv->k), &(rpv->theta), &(rpv->scale), &(rpv->n));

      if (status != 0) {

        /* try 4 columns instead */
        status = read_4c_file_float (filename, &(rpv->lambda), &(rpv->rho0), &(rpv->k), &(rpv->theta), &(rpv->n));
...
/**************************************************************/
/* Read 4-column AMBRALS/Rossli file.                         */
/**************************************************************/

static int read_rossli (char* filename, rossli_out_struct* rossli) {
  int status = read_4c_file_float (filename, &(rossli->lambda), &(rossli->iso), &(rossli->vol), &(rossli->geo), &(rossli->n));
...
static int interpolate_rpv (rpv_out_struct* rpv, float* lambda, int nlambda_lower, int nlambda_upper) {
  ...
  /* interpolate to internal RTE grid */
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->rho0, nlambda_lower, nlambda_upper, lambda, rpv->rho0_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->k, nlambda_lower, nlambda_upper, lambda, rpv->k_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->theta, nlambda_lower, nlambda_upper, lambda, rpv->theta_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->sigma, nlambda_lower, nlambda_upper, lambda, rpv->sigma_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->t1, nlambda_lower, nlambda_upper, lambda, rpv->t1_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->t2, nlambda_lower, nlambda_upper, lambda, rpv->t2_r, linear);
  ...
  status = arb_wvn2 (rpv->n, rpv->lambda, rpv->scale, nlambda_lower, nlambda_upper, lambda, rpv->scale_r, linear);
...
static int read_RPV_spectral_BRDFs (char*                filename,
  ...
  for (il = 0; il < surf->n; il++) {
    ...
    if (il > 0) {
      ...
      status = read_rpv (surf->filename[il], &(surf->rpv[il]), sigma, t1, t2, scale);
      if (status != 0) {
...
/********************************************************************/
/* Read surface types from file, that is, label plus a filename.    */
/* containing a wavelength-dependent AMBRALS. These are assigned to */
/* the labels used in the ambrals_file. Please note that "CaM" is   */
/* interpreted as "Cox and Munk" and isn't assigned a RPV.          */
/* "Cox and Munk" is assigned index 0.                              */
/********************************************************************/
...
static int read_AMBRALS_spectral_BRDFs (char*                filename,
  ...
  surf->label    = calloc (surf->n, sizeof (char*));
  surf->filename = calloc (surf->n, sizeof (char*));
  surf->rossli   = calloc (surf->n, sizeof (rossli_out_struct));
  ...
  for (il = 0; il < surf->n; il++) {
    /* allocate memory */
    surf->rossli[il].iso_r = (float*)calloc (nlambda, sizeof (float));
    surf->rossli[il].vol_r = (float*)calloc (nlambda, sizeof (float));
    surf->rossli[il].geo_r = (float*)calloc (nlambda, sizeof (float));

    /* read wavelength-dependent AMBRALS data */
    if (il > 0) {
      if (!quiet)
        fprintf (stderr, " ... reading type %s from %s\n", surf->label[il], surf->filename[il]);

      status = read_rossli (surf->filename[il], &(surf->rossli[il]));
      if (status != 0) {
        fprintf (stderr, "Error %d returned by read_rossli(%s)\n", status, surf->filename[il]);
        return status;
      }

      /* We read a Rossli BRDF from an AMBRALS file; need to convert. */
      /* See also ROSSLI_FROM_AMBRALS                                 */
      for (int iv = 0; iv < surf->rossli[il].n; iv++) {
        surf->rossli[il].iso[iv] /= PI;
        surf->rossli[il].geo[iv] /= PI;
        surf->rossli[il].vol[iv] /= 4. / 3.;
      }

      /* interpolate to internal wavelength grid */
      status = interpolate_rossli (&(surf->rossli[il]), lambda, nlambda_lower, nlambda_upper);
      if (status != 0) {
        fprintf (stderr, "Error %d returned by interpolate_rossli()\n", status);
        return status;
      }
    }
  }
...
static int read_spectral_albedos (char*                filename,
  ...
  for (il = 0; il < alb->n; il++) {
    ...
    status = arb_wvn2 (nlambda_tmp, lambda_tmp, albedo_tmp, nlambda_lower, nlambda_upper, lambda, alb->albedo_r[il], 1);
...
static void number2surfacestring (int surface_nr, char* surface_string) {
  ...
  if (surface_nr == 0)
    strcpy (surface_string, "BLACK");
  else if (surface_nr == 1)
    strcpy (surface_string, "EVERGREEN_NEEDLE_FOREST");
  else if (surface_nr == 2)
    strcpy (surface_string, "EVERGREEN_BROAD_FOREST");
  else if (surface_nr == 3)
    strcpy (surface_string, "DECIDUOUS_NEEDLE_FOREST");
  else if (surface_nr == 4)
    strcpy (surface_string, "DECIDUOUS_BROAD_FOREST");
  else if (surface_nr == 5)
    strcpy (surface_string, "MIXED_FOREST");
  else if (surface_nr == 6)
    strcpy (surface_string, "CLOSED_SHRUBS");
  else if (surface_nr == 7)
    strcpy (surface_string, "OPEN_SHRUBS");
  else if (surface_nr == 8)
    strcpy (surface_string, "WOODY_SAVANNA");
  else if (surface_nr == 9)
    strcpy (surface_string, "SAVANNA");
  else if (surface_nr == 10)
    strcpy (surface_string, "GRASSLAND");
  else if (surface_nr == 11)
    strcpy (surface_string, "WETLAND");
  else if (surface_nr == 12)
    strcpy (surface_string, "CROPLAND");
  else if (surface_nr == 13)
    strcpy (surface_string, "URBAN");
  else if (surface_nr == 14)
    strcpy (surface_string, "CROP_MOSAIC");
  else if (surface_nr == 15)
    strcpy (surface_string, "ANTARCTIC_SNOW");
  else if (surface_nr == 16)
    strcpy (surface_string, "DESERT");
  else if (surface_nr == 17)
    strcpy (surface_string, "OCEAN_WATER");
  else if (surface_nr == 18)
    strcpy (surface_string, "TUNDRA");
  else if (surface_nr == 19)
    strcpy (surface_string, "FRESH_SNOW");
  else if (surface_nr == 20)
    strcpy (surface_string, "SEA_ICE");
  else {
