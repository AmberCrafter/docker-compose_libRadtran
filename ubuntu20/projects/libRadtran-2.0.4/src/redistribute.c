...
static int redistribute_moment (float***** moment,
                                int***     nmom,
                                int        nlambda,
                                int        nlambda_lower,
                                int        nlambda_upper,
...
int redistribute_caoth_microphys (caoth_microphys_struct* microphys, float* zd, int nlyr, float* zd_common, int nlyr_common);
...
int redistribute_caoth3d (caoth3d_out_struct* caoth3d, float* zd, int nlyr, float* zd_common, int nlyr_common, int quiet);

int redistribute_3D (float**** data,
                     int       nx,
                     int       ny,
                     float*    z_old,
                     int       nlyr_old,
...
int setup_redistribute (input_struct input, output_struct* output) {
  ...
  int    lc = 0, ialt = 0, status = 0, old_nlev = 0, ipa = 0, lu = 0, li = 0, lk = 0, lv = 0, ntmp_zd = 0;
  ...
  if (input.atm.zout_interpolate == NO_ZOUT_INTERPOLATE) {
    ...
    if (input.alt.altitude_dz > 0) {
      nuser = (int)((output->atm.zd[0] - output->atm.zd[output->atm.nlev - 1]) / input.alt.altitude_dz + 0.5) + 1;
      ...
      int status = set_common_z_grid (zuser, nuser, &output->atm.zd_common, &output->atm.nlev_common);
      CHKERR (status);
  ...
  status = redistribute_molecular (&(output->atm.optprop),
                                   output->wl.nlambda_r,
  ...
  status = redistribute_molecular_crs (&(output->crs),
                                       output->wl.nlambda_r,
                                       output->wl.nlambda_rte_lower,
                                       output->wl.nlambda_rte_upper,
                                       MOL_NN,
  ...
  status = redistribute_optprop (&(redistributed_aer->optprop),
                                 ...
                                 tmp_zd,
                                 old_nlev - 1,
                                 redistributed_aer->optprop.nphamat,
                                 output->atm.zd_common,
                                 output->atm.nlev_common - 1,
                                 0);
  if (status != 0) {
    fprintf (stderr,
             "Error %d during redistribute_optprop (aero) (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d copying redistributed_aer to output->aer (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
    ...
    if (output->caoth3d[isp].nthreed > 0) {
      ...
      if (output->caoth3d[isp].nlyr == output->atm.nlev_common - 1) {
        ...
        for (lc = 0; lc <= output->caoth3d[isp].nlyr; lc++) {
          if (output->caoth3d[isp].zd[lc] != output->atm.zd_common[output->caoth3d[isp].nlyr - lc]) {
            fprintf (stderr,
                     "LEVELS %f AND %f DIFFER!\n",
  ...
  if (output->cf.nlev > 0) {
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d during redistribute cloudfraction (line %d, function %s in %s)  \n",
               status,
               __LINE__,
               __func__,
               __FILE__);
    ...
    if (output->nipa > 1) {
      for (ipa = 0; ipa < output->nipa; ipa++) {
        ...
        if (status != 0) {
          fprintf (stderr,
                   "Error %d during redistribute ipa cloudfraction (line %d, function %s in %s)  \n",
                   status,
                   __LINE__,
                   __func__,
                   __FILE__);
  ...
  if (input.raman) {
    ...
    int status = set_common_z_grid (output->atm.zout_sur, output->atm.nzout, &tmp_zd, &ntmp_zd);
    CHKERR (status);
    ...
    if ((output->atm.zout_user_index = calloc (output->atm.nzout_user, sizeof (int))) == NULL) {
      fprintf (stderr,
               "Error, allocating memory for output->atm.zout_user_index (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
      return -1;
    ...
    if ((output->atm.zout_comp_index = calloc (output->atm.nzout, sizeof (int))) == NULL) {
      fprintf (stderr,
               "Error, allocating memory for output->atm.zout_comp_index (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
      return -1;
  ...
  if (output->alt.altitude != 0) {
    ...
    if (ialt == output->atm.nlev_common) {
      fprintf (stderr,
               "Error, user-defined altitude %f not found in common grid (line %d, function %s in %s)\n",
               output->alt.altitude,
               __LINE__,
               __func__,
               __FILE__);
  ...
  if (output->surface_temperature < 0) {
    ...
    if (strlen (input.filename[FN_SURFACE_TEMP_MAP]) > 0) {
      ...
      status = get_number_from_netCDF_map (input.latitude,
                                           ...
                                           input.filename[FN_SURFACE_TEMP_MAP],
  ...
  if (input.verbose) {
    ...
    for (iv = output->wl.nlambda_rte_lower; iv <= output->wl.nlambda_rte_upper; iv++) {
      ...
      fprintf (stderr,
               "      |            |           o3             o2            co2            no2            bro           oclo       "
               "    hcho        wc.dtau        ic.dtau\n");
      fprintf (stderr,
               " ------------------------------------------------------------------------------------------------------------------"
               "--------------------------------------\n");
      ...
      for (lc = 0; lc < output->atm.nlyr; lc++) {
        ...
        babso_o3 = 0.5 * deltaz *
                   (output->atm.microphys.dens[MOL_O3][0][0][lc] * output->crs.crs[0][0][lc][MOL_O3][iv] +
                    output->atm.microphys.dens[MOL_O3][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_O3][iv]);
        if (output->atm.microphys.denstab_id != MOL_O3)
          babso_md += babso_o3;
        vcol_o3 += 0.5 * deltaz * (output->atm.microphys.dens[MOL_O3][0][0][lc] + output->atm.microphys.dens[MOL_O3][0][0][lc + 1]);

        babso_co2 = 0.5 * deltaz *
                    (output->atm.microphys.dens[MOL_CO2][0][0][lc] * output->crs.crs[0][0][lc][MOL_CO2][iv] +
                     output->atm.microphys.dens[MOL_CO2][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_CO2][iv]);
        if (output->atm.microphys.denstab_id != MOL_CO2)
          babso_md += babso_co2;
        vcol_co2 +=
          0.5 * deltaz * (output->atm.microphys.dens[MOL_CO2][0][0][lc] + output->atm.microphys.dens[MOL_CO2][0][0][lc + 1]);

        babso_o2 = 0.5 * deltaz *
                   (output->atm.microphys.dens[MOL_O2][0][0][lc] * output->crs.crs[0][0][lc][MOL_O2][iv] +
                    output->atm.microphys.dens[MOL_O2][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_O2][iv]);
        if (output->atm.microphys.denstab_id != MOL_O2)
          babso_md += babso_o2;
        vcol_o2 += 0.5 * deltaz * (output->atm.microphys.dens[MOL_O2][0][0][lc] + output->atm.microphys.dens[MOL_O2][0][0][lc + 1]);

        babso_no2 = 0.5 * deltaz *
                    (output->atm.microphys.dens[MOL_NO2][0][0][lc] * output->crs.crs[0][0][lc][MOL_NO2][iv] +
                     output->atm.microphys.dens[MOL_NO2][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_NO2][iv]);
        if (output->atm.microphys.denstab_id != MOL_NO2)
          babso_md += babso_no2;
        vcol_no2 +=
          0.5 * deltaz * (output->atm.microphys.dens[MOL_NO2][0][0][lc] + output->atm.microphys.dens[MOL_NO2][0][0][lc + 1]);

        babso_bro = 0.5 * deltaz *
                    (output->atm.microphys.dens[MOL_BRO][0][0][lc] * output->crs.crs[0][0][lc][MOL_BRO][iv] +
                     output->atm.microphys.dens[MOL_BRO][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_BRO][iv]);
        if (output->atm.microphys.denstab_id != MOL_BRO)
          babso_md += babso_bro;
        vcol_bro +=
          0.5 * deltaz * (output->atm.microphys.dens[MOL_BRO][0][0][lc] + output->atm.microphys.dens[MOL_BRO][0][0][lc + 1]);

        babso_oclo = 0.5 * deltaz *
                     (output->atm.microphys.dens[MOL_OCLO][0][0][lc] * output->crs.crs[0][0][lc][MOL_OCLO][iv] +
                      output->atm.microphys.dens[MOL_OCLO][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_OCLO][iv]);
        if (output->atm.microphys.denstab_id != MOL_OCLO)
          babso_md += babso_oclo;
        vcol_oclo +=
          0.5 * deltaz * (output->atm.microphys.dens[MOL_OCLO][0][0][lc] + output->atm.microphys.dens[MOL_OCLO][0][0][lc + 1]);

        babso_hcho = 0.5 * deltaz *
                     (output->atm.microphys.dens[MOL_HCHO][0][0][lc] * output->crs.crs[0][0][lc][MOL_HCHO][iv] +
                      output->atm.microphys.dens[MOL_HCHO][0][0][lc + 1] * output->crs.crs[0][0][lc + 1][MOL_HCHO][iv]);
        if (output->atm.microphys.denstab_id != MOL_HCHO)
          babso_md += babso_hcho;
        vcol_hcho +=
          0.5 * deltaz * (output->atm.microphys.dens[MOL_HCHO][0][0][lc] + output->atm.microphys.dens[MOL_HCHO][0][0][lc + 1]);
        ...
        case CK_REPTRAN_CHANNEL:
          ...
          case MOLABS_LOOKUP:
            fprintf (stderr,
                     "%5d | %10.4f | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | ",
                     lc,
      ...
      fprintf (stderr,
               ...
               "---------\n");
      fprintf (stderr,
               "%6s  %10.4f | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e\n",
               "AMFsum",
               0.0 / 0.0,
               babsotot_o3,
               babsotot_o2,
      ...
      fprintf (stderr,
               ...
               "---------\n");
      fprintf (stderr,
               "AMF column    %4.0f | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e\n",
               0.0 / 0.0,
               vcol_o3,
               vcol_o2,
               vcol_co2,
      ...
      fprintf (stderr,
               ...
               "---------\n");
      fprintf (stderr,
               "AMF BOA crs   %4.0f | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e | %12.6e\n",
               0.0 / 0.0,
      ...
      if (output->atm.microphys.denstab_id) {
        fprintf (stderr,
                 "Density matrix absorption column for airmass calculations: %16.10e %16.10e %16.10e\n",
                 babso_amftot,
                 vertical_column,
                 crs_amf);
        fprintf (stderr,
...
int regrid_float (float* xin, float* yin, int nin, float* xout, float* yout, int nout, int scale) {
  int iin = 0, iout = 0;
  ...
  for (iin = 0; iin < nin - 1; iin++)
    if (xin[iin] <= xin[iin + 1]) {
      fprintf (stderr,
               "Error, input grid not sorted in descending order! (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
  ...
  for (iout = 0; iout < nout - 1; iout++)
    if (xout[iout] <= xout[iout + 1]) {
      fprintf (stderr,
               "Error, output grid not sorted in descending order! (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
  ...
  for (iin = nin - 1; iin >= 0; iin--) {
    ...
    if (xin[iin] != xout[iout]) {
      fprintf (stderr,
               "Error, level %f from input not contained in output! (line %d, function %s in %s)\n",
               xin[iin],
               __LINE__,
               __func__,
               __FILE__);
  ...
  for (; iout >= 0; iout--) {
    ...
    while (xout[iout] < xin[iin - 1]) {
      if (scale)
        yout[iout] = yin[iin] * (xout[iout - 1] - xout[iout]) / (xin[iin - 1] - xin[iin]);
      else
        yout[iout] = yin[iin];
      iout--;
    }
...
int regrid (double* xin, double* yin, int nin, double* xout, double* yout, int nout, int scale) {
  int iin = 0, iout = 0;
  ...
  iout = nout - 1;
  for (iin = nin - 1; iin >= 0; iin--) {
    while (xout[iout] < xin[iin] - EPSILON * fabs (xout[iout]))
      if (--iout < 0)
        break;
    ...
    if (fabs (xin[iin] - xout[iout]) > EPSILON * fabs (xout[iout])) {
      fprintf (stderr,
               "Error, level %16.12f from input not contained in output! (line %d, function %s in %s)\n",
               xin[iin],
               __LINE__,
               __func__,
               __FILE__);
      fprintf (stderr,
               "   input level = %16.12f, nearest output levels = %16.12f, %16.12f\n",
               xin[iin],
               xout[iout + 1],
               xout[iout]);
...
int set_common_z_grid (float* zd, int nlev, float** zd_common, int* nlev_common) {
  ...
  if (nlev < 0)
    return err_out ("nlev has to be positive", nlev);
  ...
  if ((tmp = (float*)calloc (nlev + *nlev_common, sizeof (float))) == NULL) {
    return err_out ("Error: Allocation of tmp in set_common_z_grid() (redistribute.c) \n", -1);
  }
  ...
  *nlev_common = k;
  ...
  if (*nlev_common == 0) { /* Special for first time, and no need to sort this */
    ...
    *nlev_common = nlev;
    ...
    qsort ((*zd_common), *nlev_common, sizeof (float), QSORT_CAST compare_float);
  }
...
int add_file2grid (float** zd, int* nlev, char* filename) {
  ...
  status = set_common_z_grid (znew, nnew, zd, nlev);
  CHKERR (status);
  free (znew);
...
int add_sigma_nc_file2grid (float**   zd,
                            int*      nlev,
                            char*     filename,
                            float     latitude,
                            float     longitude,
  ...
  status = get_all_netCDF_indices (filename,
                                   latitude,
                                   longitude,
                                   &(ncid),
                                   &(ilat),
  ...
  status += arb_wvn (nlev_atm, log_p_atm, z_atm, (int)nnew, log_p_aero, znew, INTERP_METHOD_LINEAR, 0);
  if (status != 0) {
...
int redistribute_1D (void** data, float* z_old, int nlyr_old, float* z_new, int nlyr_new, int scale, int type, int alloc) {
  int     lc = 0, status = 0;
  ...
  if (status != 0) {
    fprintf (stderr,
             "redistribute_1D(): Error %d returned by regrid (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  default:
    fprintf (stderr,
             "Error, unknown data type %d in redistribute_1D (line %d, function %s in %s)\n",
             type,
             __LINE__,
             __func__,
             __FILE__);
...
int redistribute_3D (float**** data,
  ...
  /* rename old data to tmp */
  tmp = *data;

  /* and reallocate with changed resolution */
  if (!(*data = calloc_float_3D_sparse (nlyr_new, nx, ny, threednew, "data"))) {
    free_float_3D (tmp);
    return -1;
  }
...
int redistribute_threed (int** threed, float* z_old, int nlyr_old, float* z_new, int nlyr_new) {
  int     lc = 0, status = 0;
  double *xin = NULL, *yin = NULL;
  double *xout = NULL, *yout = NULL;
  ...
  *threed = calloc (nlyr_new, sizeof (int));
  ...
  status = regrid (xin, yin, nlyr_old + 1, xout, yout, nlyr_new + 1, 0);
...
static int redistribute_phase (float*****  phase,
                               float*****  theta,
                               double***** mu,
                               int****     ntheta,
                               int         nlambda,
...
int sort_vec (float* xin, int nin, float* xout, int nout, int** i_loc) {
  ...
  if (nout < 1)
    return err_out ("nout has to be >= 1", nout);
...
int redistribute_caoth (wl_out_struct  wl,
  ...
  calloc_caoth_out (redistributed_caoth, (*caoth).name, (*caoth).fullname, wl.nlambda_r, atm.nlev_common, nphamat, 1);

  status = cp_caoth_out (redistributed_caoth, *caoth, copy_ssprop, 1, quiet);
  if (status)
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d during redistribute_optprop (%s) (line %d, function %s in %s)\n",
             status,
             (*caoth).fullname,
             __LINE__,
             __func__,
  ...
  if (alloc_microphys) {
    status =
      redistribute_caoth_microphys (&(redistributed_caoth->microphys), caoth_zd, old_nlev - 1, atm.zd_common, atm.nlev_common - 1);

    if (status != 0) {
      fprintf (stderr,
               "Error %d regridding microphysical parameters for %s (line %d, function %s in %s)\n",
               status,
               (*caoth).fullname,
               __LINE__,
               __func__,
  ...
  calloc_caoth_out (caoth, (*redistributed_caoth).name, (*redistributed_caoth).fullname, wl.nlambda_r, atm.nlev_common, nphamat, 1);

  status = cp_caoth_out (caoth, *redistributed_caoth, copy_ssprop, 1, quiet);

  if (status != 0) {
    fprintf (stderr,
             "Error %d copying redistributed %s to output structure (line %d, function %s in %s)\n",
             status,
...
int redistribute_optprop (optprop_struct* optprop,
                          int             nlambda,
                          int             nlambda_lower,
                          int             nlambda_upper,
                          float*          zd,
  ...
  /** dtau **/
  status += redistribute_2D ((void*)(&(optprop->dtau)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /** ssa  **/
  status += redistribute_2D ((void*)(&(optprop->ssa)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /** f  **/
  status += redistribute_2D ((void*)(&(optprop->f)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /**  g1  **/
  status += redistribute_2D ((void*)(&(optprop->g1)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /**  g2  **/
  status += redistribute_2D ((void*)(&(optprop->g2)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /**  ff  **/
  status += redistribute_2D ((void*)(&(optprop->ff)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  /** dscale **/
  status += redistribute_2D ((void*)(&(optprop->dscale)),
                             nlambda,
                             nlambda_lower,
                             nlambda_upper,
                             zd,
  ...
  status += redistribute_moment (&(optprop->moment),
                                 &(optprop->nmom),
                                 nlambda,
                                 0,
                                 nlambda - 1,
  ...
  /** phase functions **/
  status += redistribute_phase (&(optprop->phase),
                                &(optprop->theta),
                                &(optprop->mu),
                                &(optprop->ntheta),
                                nlambda,
...
int redistribute_caoth3d (caoth3d_out_struct* caoth3d, float* zd, int nlyr, float* zd_common, int nlyr_common, int quiet) {
  ...
  status += redistribute_threed ((&(caoth3d->threed)), zd, nlyr, zd_common, nlyr_common);
  ...
  /** lwc **/
  status += redistribute_3D ((&(caoth3d->lwc)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** cf **/
  status += redistribute_3D ((&(caoth3d->cf)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** reff **/
  status += redistribute_3D ((&(caoth3d->reff)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** ext **/
  status += redistribute_3D ((&(caoth3d->ext)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** ssa **/
  status += redistribute_3D ((&(caoth3d->ssa)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** g1 **/
  status += redistribute_3D ((&(caoth3d->g1)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** g2 **/
  status += redistribute_3D ((&(caoth3d->g2)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** ff **/
  status += redistribute_3D ((&(caoth3d->ff)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** f **/
  status += redistribute_3D ((&(caoth3d->f)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);

  /** dscale **/
  status += redistribute_3D ((&(caoth3d->dscale)), nx, ny, zd, nlyr, zd_common, nlyr_common, threedold, caoth3d->threed, 0);
...
int redistribute_caoth_microphys (caoth_microphys_struct* microphys, float* zd, int nlyr, float* zd_common, int nlyr_common) {
  ...
  /** lwc for layers  **/
  status += redistribute_1D ((void*)(&(microphys->lwc_layer)), zd, nlyr, zd_common, nlyr_common, 0, REDISTRIBUTE_FLOAT, 0);

  /** reff for layers **/
  status += redistribute_1D ((void*)(&(microphys->effr_layer)), zd, nlyr, zd_common, nlyr_common, 0, REDISTRIBUTE_FLOAT, 0);

  /** lwc at levels  **/
  status += redistribute_1D ((void*)(&(microphys->lwc)), zd, nlyr, zd_common, nlyr_common, 0, REDISTRIBUTE_FLOAT, 0);

  /** reff at levels **/
  status += redistribute_1D ((void*)(&(microphys->effr)), zd, nlyr, zd_common, nlyr_common, 0, REDISTRIBUTE_FLOAT, 0);
...
int redistribute_molecular_crs (crs_out_struct* crs,
  ...
  if ((status = ASCII_calloc_float_5D (&crs->crs, 1, 1, nlyr_common + 1, nmol, nlambda)) != 0) {
  ...
  for (iv = nlambda_lower; iv <= nlambda_upper; iv++) {
    for (id = 0; id < nmol; id++) {
      ...
      status = regrid (xin, yin, nlyr + 1, xout, yout, nlyr_common + 1, scale);
      if (status != 0) {
...
int redistribute_molecular (atm_optprop_struct* optprop,
                            int                 nlambda,
                            int                 nlambda_lower,
                            int                 nlambda_upper,
                            int*                nq,
  ...
  /* Allocate memory for temporary ary */
  /*
  if ((status = ASCII_calloc_float_5D_arylen_restricted (&tmpopt, Nx, Ny, nlyr, 
  ...
  */
  ...
  if (!(tmpopt = calloc_float_5D_restricted ((size_t)Nx,
                                             (size_t)Ny,
                                             (size_t)nlyr,
                                             (size_t)nlambda_lower,
                                             (size_t)nlambda_upper,
                                             nq,
                                             "tmpopt")))
    return -1;

  for (ix = 0; ix < Nx; ix++)
    for (iy = 0; iy < Ny; iy++) /* cp original stuff to tmp ary */
      for (lc = 0; lc < nlyr; lc++)
        for (iv = nlambda_lower; iv <= nlambda_upper; iv++)
  ...
  /* free original ary and allocate with changed resolution */
  //  ASCII_free_float_5D_restricted (optprop->tau_molabs_r, Nx, Ny, nlyr, nlambda, nlambda_lower, nlambda_upper);
  free_float_5D_restricted (optprop->tau_molabs_r, nlambda_lower);

  /* 
  if ((status = ASCII_calloc_float_5D_arylen_restricted
  ...
  */
  ...
  if (!(optprop->tau_molabs_r = calloc_float_5D_restricted ((size_t)Nx,
                                                            (size_t)Ny,
                                                            (size_t)nlyr_common,
                                                            (size_t)nlambda_lower,
                                                            (size_t)nlambda_upper,
                                                            nq,
                                                            "optprop->tau_molabs_r")))
    return -1;
  ...
  //ASCII_free_float_5D_restricted (tmpopt, Nx, Ny, nlyr, nlambda, nlambda_lower, nlambda_upper);
  free_float_5D_restricted (tmpopt, nlambda_lower);
  ...
  /* Allocate memory for temporary ary */
  if ((status = ASCII_calloc_double_3D_arylen_restricted (&tmpopt_md, nlyr, nlambda, nlambda_lower, nlambda_upper, nq)) != 0) {
    fprintf (stderr, "Error allocating memory for tmpopt\n");
    return status;
  }
  ...
  /* free original ary and allocate with changed resolution */
  //  ASCII_free_float_5D_restricted (optprop->tau_molabs_md_r, 1, 1, nlyr, nlambda, nlambda_lower, nlambda_upper);
  free_float_5D_restricted (optprop->tau_molabs_md_r, nlambda_lower);
  ...
  */
  ...
  if (!(optprop->tau_molabs_md_r = calloc_float_5D_restricted (1,
                                                               1,
                                                               (size_t)nlyr_common,
                                                               (size_t)nlambda_lower,
                                                               (size_t)nlambda_upper,
                                                               nq,
                                                               "optprop->tau_molabs_md_r")))
    return -1;
  ...
  */
  ...
  //  ASCII_free_float_5D_restricted (optprop->tau_rayleigh_r, Nx, Ny, nlyr, nlambda, nlambda_lower, nlambda_upper);
  free_float_5D_restricted (optprop->tau_rayleigh_r, nlambda_lower);
  ...
  */
  ...
  if (!(optprop->tau_rayleigh_r = calloc_float_5D_restricted ((size_t)Nx,
                                                              (size_t)Ny,
                                                              (size_t)nlyr_common,
                                                              (size_t)nlambda_lower,
                                                              (size_t)nlambda_upper,
                                                              nq,
                                                              "optprop->tau_rayleigh_r")))
    return -1;
...
int redistribute_amf (atm_microphys_struct* microphys,
  ...
  if (microphys->denstab_id > 0) {
    ...
    if ((status = ASCII_calloc_float (&crs->crs_amf, nlambda, nlyr_common + 1)) != 0)
      return status;
    ...
    /* Allocate memory for temporary ary */
    if ((status = ASCII_calloc_double (&tmp2, microphys->nsza_denstab, nlyr + 1)) != 0)
      return status;
    ...
    if ((status = ASCII_calloc_float (&microphys->denstab_amf, microphys->nsza_denstab, nlyr_common + 1)) != 0)
      return status;
    ...
    for (is = 0; is < microphys->nsza_denstab; is++) {
      ...
      status = arb_wvn (nlyr + 1, xfin, yfin, nlyr_common + 1, xfout, yfout, 1, 0);
      if (status != 0) {
