...
int singleScatteringRadar (ssRadarStruct* ssRadar, char* inputFilename, char* outputFilename) {
  ...
  if (verbose) {
    ...
    fprintf (stderr,
             "This program calculates the pure Rayleigh scattering reflectivity factor\nin an arbitrary plan-parallel atmospheric "
             "environment.\n");
  }
  ...
  if (readSSRadarFile (ssRadar, inputFilename)) {
    ...
    exitSSRadar (&ssRadar->allDistributionParameters,
                 &rangeGates,
                 &rangeGatesVertical,
                 &rangeGateReflectivityFactor,
                 &reflectiveLayersHeights,
                 &reflectiveLayersThicknesses,
                 &reflectiveLayersReflectivities,
                 &allLayersHeights,
                 &allLayersThicknesses,
                 &allLayersReflectivities,
                 &indexArray,
                 ssRadar->numberOfReflectiveLayers);
    return -1;
  }
  ...
  for (i = 0; i < ssRadar->numberOfReflectiveLayers; i++) {
    ...
    for (j = 0; j < ssRadar->allDistributionParameters[i]->numberOfGridPoints; j++) {
      ssRadar->allDistributionParameters[i]->reflectivityFactor +=
        ssRadar->allDistributionParameters[i]->dropletDistribution[j] *
  ...
  rangeGatesVertical[0] =
    ssRadar->radarZPosition + umuFactor * (ssRadar->firstRangeGate - ssRadar->radarZPosition) * ssRadar->cosZenithAngleUmu;
  ...
  rangeGates[ssRadar->numberOfRangeGates] =
    umuFactor * ssRadar->radarZPosition / ssRadar->cosZenithAngleUmu - rangeGates[ssRadar->numberOfRangeGates - 1];
  ...
  upperBorder = (rangeGatesVertical[umuIndexR] > reflectiveLayersHeights[ssRadar->numberOfReflectiveLayers - 1] +
                                                   reflectiveLayersThicknesses[ssRadar->numberOfReflectiveLayers - 1])
                  ? rangeGatesVertical[umuIndexR]
                  : reflectiveLayersHeights[ssRadar->numberOfReflectiveLayers - 1] +
                      reflectiveLayersThicknesses[ssRadar->numberOfReflectiveLayers - 1];
  ...
  for (i = 0; i < ssRadar->numberOfReflectiveLayers - 1; i++)
    numberOfAllLayers +=
      findEmptyLayers (reflectiveLayersHeights[i], reflectiveLayersHeights[i + 1], reflectiveLayersThicknesses[i]);
  if (upperBorder > reflectiveLayersHeights[ssRadar->numberOfReflectiveLayers - 1] +
                      reflectiveLayersThicknesses[ssRadar->numberOfReflectiveLayers - 1])
    numberOfAllLayers++;
  ...
  for (j = 0; j < ssRadar->numberOfReflectiveLayers; j++) {
    ...
    i++;
    if (j < ssRadar->numberOfReflectiveLayers - 1 &&
        findEmptyLayers (reflectiveLayersHeights[j], reflectiveLayersHeights[j + 1], reflectiveLayersThicknesses[j])) {
  ...
  if (upperBorder > reflectiveLayersHeights[ssRadar->numberOfReflectiveLayers - 1] +
                      reflectiveLayersThicknesses[ssRadar->numberOfReflectiveLayers - 1]) {
    allLayersHeights[counter] = reflectiveLayersHeights[ssRadar->numberOfReflectiveLayers - 1] +
                                reflectiveLayersThicknesses[ssRadar->numberOfReflectiveLayers - 1];
      ...
      if (indexArray[i] < numberOfAllLayers) {
        if (indexArray[i + 1] == indexArray[i]) {
          ...
          rangeGateReflectivityFactor[i] =
            ((allLayersHeights[indexArray[i] + 1] - rangeGatesVertical[i]) / ssRadar->rangeGateWidth) *
            allLayersReflectivities[indexArray[i]];
          if (indexArray[i + 1] > indexArray[i] + 1) {
            for (j = 1; j < indexArray[i + 1] - indexArray[i]; j++) {
              rangeGateReflectivityFactor[i] +=
                ((allLayersThicknesses[indexArray[i] + j]) / ssRadar->rangeGateWidth) * allLayersReflectivities[indexArray[i] + j];
              addIndex = j;
            }
          }
          rangeGateReflectivityFactor[i] +=
            ((rangeGatesVertical[i] + ssRadar->rangeGateWidth - allLayersHeights[indexArray[i] + addIndex + 1]) /
             ssRadar->rangeGateWidth) *
            allLayersReflectivities[indexArray[i] + addIndex + 1];
        }
      }
    }
  } else {
    for (i = 0; i < ssRadar->numberOfRangeGates; i++) {
      if (indexArray[i] < numberOfAllLayers) {
        if (indexArray[i + 1] == indexArray[i])
          ...
          addIndex                       = 0;
          rangeGateReflectivityFactor[i] = ((-allLayersHeights[indexArray[i]] + rangeGatesVertical[i]) / ssRadar->rangeGateWidth) *
                                           allLayersReflectivities[indexArray[i]];
          if (indexArray[i + 1] < indexArray[i] + 1) {
            for (j = 1; j < indexArray[i + 1] - indexArray[i]; j++) {
              rangeGateReflectivityFactor[i] += ((allLayersThicknesses[indexArray[i] - j - 1]) / ssRadar->rangeGateWidth) *
                                                allLayersReflectivities[indexArray[i] - j];
              addIndex = j;
            }
          }
          rangeGateReflectivityFactor[i] +=
            ((-rangeGatesVertical[i + 1] + allLayersHeights[indexArray[i] - addIndex]) / ssRadar->rangeGateWidth) *
            allLayersReflectivities[indexArray[i] - 1 - addIndex];
        }
      }
    }
  }
...
int readSSRadarFile (ssRadarStruct* ssRadar, char* inputFilename) {
  ...
  for (i = 2; i < rows; i++) {
    ...
    if ((ssRadar->allDistributionParameters[i - 2]->waterPhase == 1 &&
      ...
      fprintf (stderr,
               "Error in SSRadar, wrong combination of temperature and phase %d %f!\n",
  ...
  *dB     = 1;
  ...
  if (*outputFilename != *ssRadarOutputFilename) {
    if (output > -1) {
      ...
      range[numberOfRangeGates - 1] =
        (rangeGates[numberOfRangeGates - 1] + 0.5 * (rangeGates[numberOfRangeGates - 1] - rangeGates[numberOfRangeGates - 2]));
      for (i = 0; i < numberOfRangeGates; i++) {
        start[0] = i;
        status   = nc_put_vara_double (ncid, id_var[iid], start, count, &range[i]);
        if (status)
      ...
      for (i = 0; i < numberOfRangeGates; i++) {
        start[0] = i;
        status   = nc_put_vara_double (ncid, id_var[iid], start, count, &rangeGateReflectivityFactor[i]);
        if (status)
    ...
    if (output < 1) {
      ...
      for (i = 0; i < numberOfRangeGates; i++)
        fprintf (ssRadarOutputFile,
                 "%d \t %e \t %e \t %f\n",
                 i + 1,
                 rangeGates[i],
                 rangeGatesVertical[i],
                 rangeGateReflectivityFactor[i]);
...
double getWaterDensity (double temperature, int phase) {
  ...
  double waterDensityTable[12] = {999.84, 999.96, 999.7, 999.1, 998.2, 997.04, 995.64, 994.03, 992.21, 990.22, 988.04, 985.7};
  ...
  if (temperature >= 0. && phase == 1) {
    ...
    return linearInterpolation (waterDensityTable[index],
                                waterDensityTable[index + 1],
                                waterTemperatureTable[index],
                                waterTemperatureTable[index + 1],
                                temperature);
    ...
    return linearInterpolation (supercooledWaterDensityTable[index],
                                supercooledWaterDensityTable[index + 1],
                                supercooledWaterTemperatureTable[index],
                                supercooledWaterTemperatureTable[index + 1],
                                temperature);
  } else if (temperature <= 0. && phase == -1) {
    for (i = 0; i < 12; i++) {
    ...
    return linearInterpolation (iceDensityTable[index],
                                iceDensityTable[index + 1],
                                iceTemperatureTable[index],
                                iceTemperatureTable[index + 1],
                                temperature);
  } else
    return -9999.;
}
