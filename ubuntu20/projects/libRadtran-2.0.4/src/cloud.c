...
int read_cloud_fraction_file (char* filename, atm_inp_struct atm_in, atm_out_struct* atm_out, cf_out_struct* cf, int quiet);
...
static int
get_total_column_caoth (caoth_out_struct* cld, int nipa, float* ipaweight, caoth_out_struct* cldipa, cf_out_struct cf, int verbose);

int ic_geom (float wavelength, int nlyr, float* iwc, float* reff, float* zd, float* dtau, float* ssa);
...
int setup_all_caoth (input_struct input, output_struct* output) {
  ...
  if (strlen (input.filename[FN_CLOUD_FRACTION]) > 0) {
    status = read_cloud_fraction_file (input.filename[FN_CLOUD_FRACTION], input.atm, &(output->atm), &(output->cf), input.quiet);
    if (status)
  ...
  for (isp = 0; isp < input.n_caoth; isp++) {
    status = setup_caoth (input, output, input.caoth[isp], &(output->caoth[isp]), &(output->caoth_ipa[isp]));
    if (status)
  ...
  if (input.cloud_overlap == CLOUD_OVERLAP_OFF && input.rte.solver != SOLVER_TWOMAXRND && input.rte.solver != SOLVER_TWOMAXRND3C &&
...
int read_cloud_fraction_file (char* filename, atm_inp_struct atm_in, atm_out_struct* atm_out, cf_out_struct* cf, int quiet) {
  ...
  if (strlen (filename) > 0) {
    status = read_2c_file_float (filename, &(cf->zd), &(cf->cf), &(cf->nlev));
    if (status != 0) {
  ...
  set_common_z_grid (cf->zd, cf->nlev, &(atm_out->zd), &(atm_out->nlev));
...
int setup_caoth (/* input */
  ...
  case CAOTH_FROM_MOMENTS:
    ...
    if (input_caoth.properties != PROP_EXPLICIT) {
      fprintf (stderr, "Error, inconsistent properties for %s ", input_caoth.fullname);
    ...
    if (strlen (input_caoth.filename) == 0) {
      fprintf (stderr, "Error, inconsistent properties %s ", input_caoth.fullname);
    ...
    if (output->alt.altitude != 0) {
      ...
      fprintf (stderr,
               " with '%s_files' (line %d, function %s in %s, %s) \n",
               input_caoth.name1,
               __LINE__,
               __func__,
               __FILE__,
    ...
    if ((status = ASCII_checkfile (input_caoth.filename, &rows, &min_columns, &max_columns, &max_length)) != 0) {
      fprintf (stderr,
               "Error %d reading %s(line %d, function %s in %s) \n",
               status,
               input_caoth.filename,
               __LINE__,
               __func__,
    ...
    /* allocate memory */
    status = calloc_caoth_out (output_caoth, input_caoth.name, input_caoth.fullname, output->wl.nlambda_r, rows, 1, 1);
    if (status)
    ...
    if (status != 0) {
      fprintf (stderr, "Error %d reading %s optical property files '%s' ", status, input_caoth.name, input_caoth.filename);
      fprintf (stderr, "(line %d, function %s in %s)\n", __LINE__, __func__, __FILE__);
      return status;
    }
  ...
  case CAOTH_FROM_3D:
    ...
    if (input_caoth.ipa) {
      ...
      for (ipa = 0; ipa < output->nipa; ipa++) {
        ...
        status = cp_caoth_out (&((*output_caoth_ipa)[ipa]), *output_caoth, FALSE, TRUE, input.quiet);
        if (status != 0) {
          fprintf (stderr, "Error %d copying %s to dummy output ipa structure %3d ", status, input_caoth.fullname, ipa);
          fprintf (stderr, "(line %d, function %s in %s)\n", __LINE__, __func__, __FILE__);
          return status;
        }
      }
  ...
  default:
    fprintf (stderr,
             "Error, unknown %s source %d (line %d, function %s in %s)\n",
             input_caoth.fullname,
             input_caoth.source,
             __LINE__,
             __func__,
  ...
  if (input_caoth.ipa) {
    ...
    if (ipasum < 1.0 - EPSILON || ipasum > 1.0 + EPSILON) {
      fprintf (stderr, "Error, ipa weights add up to %f. But the sum should be 1.0000 ! ", ipasum);
      fprintf (stderr, "(line %d, function %s in %s)\n", __LINE__, __func__, __FILE__);
      return -1;
    }
  }
  ...
  status = get_total_column_caoth (output_caoth, output->nipa, output->ipaweight, *output_caoth_ipa, output->cf, input.verbose);
  if (status)
...
static int setup_cloudfraction (int              cloud_overlap,
  ...
  case CLOUD_OVERLAP_MAXRAND:
    ...
    for (lc = 1; lc < cf->nlev - 1; lc++) {
      if (cf->cf[lc] > cc_max)
        cc_max = cf->cf[lc];                              /* find cc_max inside one cloud */
  ...
  case CLOUD_OVERLAP_MAXRAND:
    ...
    if (done == FALSE) {
      ...
      if (!quiet)
        fprintf (stderr,
                 " ... IPA with %2d columns, %6.2f%% cloudfree and %6.2f%% overcast\n",
                 (*nipa),
                 100.0 * (*ipaweight)[iclear],
                 100.0 * (*ipaweight)[icloud]);
      ...
      for (lc = 0; lc < (*cfipa)[icloud].nlev - 1; lc++) {
        if (fabs ((*cfipa)[icloud].cf[lc] - (cf->cf[lc] / cf->cf_total)) > EPSILON) {
          fprintf (stderr,
                   "Error, different cloud_fraction for water and ice clouds (line %d, function %s in %s)\n",
                   __LINE__,
                   __func__,
                   __FILE__);
    ...
    for (ipa = 0; ipa < (*nipa); ipa++) {
      status = calloc_caoth_out (&((*cldipa)[ipa]), caoth_name, caoth_fullname, nlambda, cld.nlev, cld.optprop.nphamat, 1);
      if (status != 0) {
        fprintf (stderr,
                 "Error %d allocating memory for caoth_out_struct %d (line %d, function %s in %s)\n",
                 status,
                 ipa,
                 __LINE__,
                 __func__,
    ...
    if (status != 0) { /*  target,      source, copy_ssprop, alloc_moment  quiet  */
      fprintf (stderr,
               "Error %d copying cld to cloudy cldipa (line %d, function %s in %s)\n",
               status,
               __LINE__,
               __func__,
               __FILE__);
    ...
    case CAOTH_FROM_MOMENTS:
      fprintf (stderr,
               "Error, cloud overlap only possible with clouds from file, ECMWF, or ECHAM (line %d, function %s in %s)\n",
               __LINE__,
               __func__,
               __FILE__);
      fprintf (stderr,
               "       clouds given from %d (ipa=%d, 3Dclouds=%d, cloudlayer=%d)\n",
               cld_source,
               CAOTH_FROM_IPA_FILES,
               CAOTH_FROM_3D,
               CAOTH_FROM_MOMENTS);
      return -1;
      break;
    ...
    if (verbose) {
      ...
      fprintf (stderr, "\n");
      for (lc = 0; lc < cld.nlev; lc++) {
        fprintf (stderr, " %3d %7.3f %e |", lc, cld.zd[lc], cld.microphys.lwc[lc]);
  ...
  case CLOUD_OVERLAP_OFF:
    ...
    if ((*cloudcover) >= 0) {
      ...
      for (ipa = 0; ipa < (*nipa); ipa++) {
        ...
        if (status != 0) {
          fprintf (stderr,
                   "Error %d allocating memory for caoth_out_struct (line %d, function %s in %s)\n",
                   status,
                   __LINE__,
                   __func__,
                   __FILE__);
...
static int get_total_column_caoth (caoth_out_struct* cld,
  ...
  if (verbose)
    fprintf (stderr, " ... calculate total column ice/liquid water\n");
...
int calloc_ssprop_struct (ssprop_struct* out, int nlambda) {
  ...
  out->gg     = calloc (nlambda, sizeof (double*));
...
int calloc_caoth_out (caoth_out_struct* out, char* name, char* fullname, int nlambda, int nlev, int nphamat, int calloc_ssprop) {
  int status = 0;
...
int cp_optprop (optprop_struct* target, optprop_struct* source, int nlev, int alloc_moment, int quiet) {
      ...
      for (iv = 0; iv < source->nlam; iv++) {
        ...
        if (source->nmom[iv][lev] > 0) {
          ...
          if (target->nmom[iv][lev] != source->nmom[iv][lev]) {
            fprintf (stderr,
                     "Error, number of moments in source (%d) and target (%d) different\n",
                     source->nmom[iv][lev],
                     target->nmom[iv][lev]);
            return -1;
          }
          ...
          for (ip = 0; ip < source->nphamat; ip++) {
            if (source->ntheta[iv][lev][ip] > 0) {
              ...
              if (target->ntheta[iv][lev][ip] != source->ntheta[iv][lev][ip]) {
                fprintf (stderr,
                         "Error, number of moments in source (%d) and target (%d) different\n",
                         source->ntheta[iv][lev][ip],
                         target->ntheta[iv][lev][ip]);
                return -1;
              }
...
int cp_caoth_out (caoth_out_struct* target, caoth_out_struct source, int copy_ssprop, int alloc_moment, int quiet) {
    ...
    if (source.ssprop.alloc_moments || source.ssprop.alloc_explicit) {
      ...
      for (iv = 0; iv < source.optprop.nlam; iv++) {
        ...
        if (source.ssprop.alloc_moments && !target->ssprop.alloc_moments) {
          ...
          target->ssprop.gg[iv]     = calloc (target->ssprop.nreff[iv], sizeof (double));
          ...
          for (ir = 0; ir < target->ssprop.nreff[iv]; ir++) {
            ...
            for (ip = 0; ip < target->ssprop.nphamat; ip++)
              target->ssprop.legen[iv][ir][ip] = calloc (source.ssprop.nleg[iv][ir], sizeof (float));
          }
        }
        ...
        if (source.ssprop.theta[iv] != NULL) {
          if (source.ssprop.theta[iv][0] != NULL) {
            if (source.ssprop.alloc_explicit && !target->ssprop.alloc_explicit) {
              ...
              for (ir = 0; ir < target->ssprop.nreff[iv]; ir++)
                for (ip = 0; ip < target->ssprop.nphamat; ip++) {
                  target->ssprop.theta[iv][ir][ip] = calloc (source.ssprop.ntheta[iv][ir][ip], sizeof (float));
                  target->ssprop.mu[iv][ir][ip]    = calloc (source.ssprop.ntheta[iv][ir][ip], sizeof (double));
                  target->ssprop.phase[iv][ir][ip] = calloc (source.ssprop.ntheta[iv][ir][ip], sizeof (float));
                }
            }
          }
        }
      ...
      for (iv = 0; iv < source.optprop.nlam; iv++) {
        if (target->ssprop.alloc_moments) {
          for (ir = 0; ir < target->ssprop.nreff[iv]; ir++) {
            ...
            target->ssprop.gg[iv][ir]     = source.ssprop.gg[iv][ir];
              ...
              for (ir = 0; ir < target->ssprop.nreff[iv]; ir++) {
                ...
                for (ip = 0; ip < target->ssprop.nphamat; ip++) {
                  ...
                  for (itheta = 0; itheta < target->ssprop.ntheta[iv][ir][ip]; itheta++) {
                    target->ssprop.mu[iv][ir][ip][itheta]    = source.ssprop.mu[iv][ir][ip][itheta];
                    target->ssprop.theta[iv][ir][ip][itheta] = source.ssprop.theta[iv][ir][ip][itheta];
                    target->ssprop.phase[iv][ir][ip][itheta] = source.ssprop.phase[iv][ir][ip][itheta];
                  } /* end for itheta */
                }   /* end for ip */
              }     /* end for ir */
            }       /* end if explicit */
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d copying cloud optical properties (line %d, function %s in %s)\n",
             status,
             __LINE__,
             __func__,
             __FILE__);
...
int cp_caoth3d_out (caoth_out_struct* target, caoth3d_out_struct source, int quiet, int iipa, int jipa) {
  int lev = 0;
...
/*****************************************************/
/* copy cloud fraction from 3D caoth to array        */
/* NO memory allocation is done here!!!              */
/*****************************************************/

int cp_caoth3d_cf_out (float* target, caoth3d_out_struct source, int quiet, int iipa, int jipa) {
  /* copy microphysical properties */
  for (int lev = 0; lev < source.nlyr; lev++) {
    if (source.threed[source.nlyr - lev - 1])
      target[lev] = source.cf[source.nlyr - lev - 1][iipa][jipa];
  }
...
int interpolate_ssprop_in_reff (/* Input */
  ...
  *ext                     = interp_optprop_weight[0] * extinc[lower] + interp_optprop_weight[1] * extinc[upper];
  ...
  *alb                     = interp_optprop_weight[0] * albedo[lower] + interp_optprop_weight[1] * albedo[upper];
  ...
  if (interpolate_moments || interpolate_explicit) {
    interp_optprop_weight[0] = (1. - interp_optprop_weight_0) * albedo[lower] * extinc[lower] / (*alb * *ext);
    interp_optprop_weight[1] = (interp_optprop_weight_0)*albedo[upper] * extinc[upper] / (*alb * *ext);
  }
  ...
  if (interpolate_moments) {
    ...
    *coeffc = calloc (nphamat, sizeof (float*));
    ...
    for (ip = 0; ip < nphamat; ip++) {
      ...
      for (k = 0; k < min; k++)
        (*coeffc)[ip][k] = interp_optprop_weight[0] * legen[lower][ip][k] + interp_optprop_weight[1] * legen[upper][ip][k];
  ...
  if (interpolate_explicit) {
    ...
    *phase_new  = calloc (nphamat, sizeof (float*));
    ...
    status = sort_and_add_weighted_phase (2,
  ...
  *f = interp_optprop_weight[0] * fdelta[lower] + interp_optprop_weight[1] * fdelta[upper];
...
int read_caoth_file (int   source,
  ...
  case CAOTH_FROM_IPA_FILES:
    ...
    status = ASCII_file2float (filename, &rows, &max_columns, &min_columns, &caoth_data);
    if (status != 0) {
  ...
  if (optical) {
    status = calloc_caoth_out (caoth, caoth_name, caoth_fullname, nlambda, rows, 6, 1);
    if (status)
      return mem_err_out ("output_caoth struct ", ERROR_POSITION);
  } else {
    caoth->nlev   = rows;
  ...
  for (lev = 0; lev < caoth->nlev; lev++) {
    ...
    if (verbose)
      fprintf (stderr, " %3d %7.3f %e %e\n", lev, caoth->zd[lev], caoth->microphys.lwc[lev], caoth->microphys.effr[lev]);
  }
    ...
    if (layer) {
      ...
      for (lc = 0; lc < caoth->nlev - 1; lc++) {
        if (caoth->microphys.lwc[lc + 1] > 0 && caoth->microphys.lwc[lc] > 0) {
          caoth->microphys.lwc_layer[lc]  = 0.5 * (caoth->microphys.lwc[lc + 1] + caoth->microphys.lwc[lc]);
          caoth->microphys.effr_layer[lc] = 0.5 * (caoth->microphys.effr[lc + 1] + caoth->microphys.effr[lc]);
        } else {
    ...
    if (input_caoth.fu2yang) {
      ...
      for (lev = 0; lev < output_caoth->nlev; lev++) {
        ...
        if (output_caoth->microphys.lwc_layer[lev] > 0 && !input.quiet)
          fprintf (stderr, "     yang2fu: scaling effective radius from %f to", output_caoth->microphys.effr_layer[lev]);
  ...
  for (lc = 0; lc < cld->nlev - 1; lc++) { /* count layers, 0 ... nlev-2 */
    ...
    if (cld->microphys.lwc_layer[lc] > 0) {
      ...
      if (cld->ssprop.nreff[iv] > 1) {
        ...
        if (status != 0) {
          fprintf (stderr,
                   "Error %d interpolating Legendre coefficients to reff = %f in layer %d.\n",
                   status,
                   cld->microphys.effr_layer[lc],
                   lc);
          return status;
        }
        ...
        */
        ...
        if (cld->ssprop
              .alloc_explicit) { /* bug fix by RPB on 14.10.2010: when using simple pmom netcdf file, this is not adequate */
...
int aer_ssprop2optprop (aer_out_struct* aer,
                        int             iv,
                        int             i_aer,
                        float*          rh,
                        int             nlambda,
  ...
  for (lc = 0; lc < aer->nlev - 1; lc++) { /* count layers, 0 ... nlev-2 */
    ...
    if (aer->massdens[i_aer][lc] > 0) {
      ...
      aer->optprop_opac[i_aer].dtau[iv][lc] *= aer->massdens[i_aer][lc] * (aer->zd[lc] - aer->zd[lc + 1]);
...
int read_caoth_prop (char*             filename,
  ...
  if (interpolate) {
    ...
    if (aerosol) /* either *.mie.cdf (for spherical aerosols), or *.tmatrix.cdf for aspherical */
    ...
    for (ifi = 0; ifi < nfiles; ifi++) {
      status = read_mie_table_lambda (momfiles.gl_pathv[ifi], &(nlam[ifi]), &(wavelen[ifi]), quiet);
    ...
    if (!quiet)
      fprintf (stderr,
               " ... found %d %s optical properties files covering the wavelength range %f - %f nm with a total of %d discrete "
               "wavelengths\n",
               nfiles,
               type_string,
               tmp_lambda[0],
               tmp_lambda[tmp_nlambda - 1],
               tmp_nlambda);

    /* check if wavelength range is sufficient */
    if (tmp_lambda[0] > lambda_r[nlambda_lower] || tmp_lambda[tmp_nlambda - 1] < lambda_r[nlambda_upper]) {
      fprintf (stderr,
               "Error, wavelength range covered by %s optical properties files (%f - %f nm)\n",
               type_string,
               tmp_lambda[0],
               tmp_lambda[tmp_nlambda - 1]);
    ...
    if (!quiet)
      fprintf (stderr,
               " ... selected %s files %d - %d, wavelengths %f - %f nm\n",
               type_string,
               ifile[lambda_optprop_lower],
               ifile[lambda_optprop_upper],
               tmp_lambda[lambda_optprop_lower],
               tmp_lambda[lambda_optprop_upper]);
    ...
    for (ifi = ifile[lambda_optprop_lower]; ifi <= ifile[lambda_optprop_upper]; ifi++) {
      ...
      if (iws < 0)
        iws = 0;
      niw = (iwstart[ifi + 1] - 1 < lambda_optprop_upper ? iwstart[ifi + 1] - 1 : lambda_optprop_upper) - iws - iwstart[ifi] + 1;
      ...
      if (verbose)
        fprintf (stderr,
                 " ... read %3zd reff's from file %3d, %s, %11.4f - %11.4f nm\n",
                 tmp_ssprop.nreff[iw],
                 ifi,
                 momfiles.gl_pathv[ifi],
                 wave_arr[iw],
                 wave_arr[iw + niw - 1]);
    ...
    status =
      interpolate_ssprop_in_lambda (&tmp_ssprop, ssprop, wave_arr, nlambda_read, lambda_r, nlambda_r, nlambda_lower, nlambda_upper);
...
int apply_user_defined_properties_to_caoth (caoth_inp_struct cldin,
  ...
  if (nlambda_r < 0)
    return err_out ("nlambda_r has to be positive", nlambda_r);
  ...
  for (iv = 0; iv < nlambda_r; iv++) {
    ...
    if (cldin.modify[MODIFY_VAR_GG][MODIFY_TYPE_SET] >= -1.0) {
      for (lc = 0; lc < cldout->nlev - 1; lc++) {
        ...
        cldout->optprop.g1[iv][lc] =
          (cldin.modify[MODIFY_VAR_GG][MODIFY_TYPE_SET] - cldout->optprop.f[iv][lc]) / (1.0 - cldout->optprop.f[iv][lc]);
    ...
    if (cldin.modify[MODIFY_VAR_GG][MODIFY_TYPE_SCALE] >= 0.0) {
      for (lc = 0; lc < cldout->nlev - 1; lc++) {
        ...
        unscaled_gg = cldout->optprop.g1[iv][lc] * (1.0 - cldout->optprop.f[iv][lc]) + cldout->optprop.f[iv][lc];
        ...
        cldout->optprop.g1[iv][lc] = (unscaled_gg - cldout->optprop.f[iv][lc]) / (1.0 - cldout->optprop.f[iv][lc]);
    ...
    if (cldin.modify[MODIFY_VAR_SSA][MODIFY_TYPE_SCALE] >= 0.0)
      for (lc = 0; lc < cldout->nlev - 1; lc++) {
        ...
        unscaled_ssa = cldout->optprop.ssa[iv][lc] / (1.0 + cldout->optprop.f[iv][lc] * (cldout->optprop.ssa[iv][lc] - 1.0));

        /* Apply scaling factor */
        unscaled_ssa *= cldin.modify[MODIFY_VAR_SSA][MODIFY_TYPE_SCALE];

        /* Delta scale the result */
        cldout->optprop.ssa[iv][lc] =
          (1.0 - cldout->optprop.f[iv][lc]) * unscaled_ssa / (1.0 - unscaled_ssa * cldout->optprop.f[iv][lc]);
      ...
      for (lc = 0; lc < cldout->nlev - 1; lc++) {
        ...
        if (cldout->zd[lc] > altitude) {       /* layers with upper boundary above the surface */
          if (cldout->zd[lc + 1] > altitude) { /* layers completely above the surface          */
            ...
            sum += cldout->optprop.dtau[iv][lc] / (1.0 - unscaled_ssa * cldout->optprop.f[iv][lc]);
          } else /* upper boundary above, lower boundary below surface */
            sum += cldout->optprop.dtau[iv][lc] / (1.0 - unscaled_ssa * cldout->optprop.f[iv][lc]) * (cldout->zd[lc] - altitude) /
                   (cldout->zd[lc] - cldout->zd[lc + 1]);
        }
      }
      ...
      for (lc = 0; lc < cldout->nlev - 1; lc++) {
        ...
        if (cldout->zd[lc] > altitude) {     /* layers with upper boundary above the surface */
          if (cldout->zd[lc + 1] > altitude) /* layers completely above the surface          */
            ...
            tautot[iv] += cldout->optprop.dtau[iv][lc] / (1.0 - unscaled_ssa * cldout->optprop.f[iv][lc]);
          else /* upper boundary above, lower boundary below surface */
            tautot[iv] += cldout->optprop.dtau[iv][lc] / (1.0 - unscaled_ssa * cldout->optprop.f[iv][lc]) *
...
int wc_echam4 (float wavelength, int nlyr, float* iwc, float* reff, float* dtau, float* g, float* ssa, float* zd, int layer) {
  int    lc      = 0;
  ...
  for (lc = 0; lc < nlyr; lc++) {
    if (iwc[lc + 1] > 0) {
      ...
      if (wavelength <= 680)
        ...
        ssa[lc] = 0.9854369057 + 0.013584242533 * logreff - 0.024856960461 * logreff * logreff +
                  0.0055918314369 * logreff * logreff * logreff;

      /* asymmetry parameter */
      if (wavelength <= 680)
        g[lc] = 0.78756640717 + 0.10660598895 * logreff - 0.031012468401 * logreff * logreff;
      else
        g[lc] = 0.79208639802 - 0.044930076174 * logreff + 0.18980672305 * logreff * logreff -
...
int ic_echam4 (float wavelength, int nlyr, float* iwc, float* reff, float* dtau, float* g, float* ssa, float* zd, int layer) {
  int        lc      = 0;
  double     logreff = 0, ext = 0;
  static int first = 1;
  ...
  for (lc = 0; lc < nlyr; lc++) {
    if (iwc[lc + 1] > 0) {
      ...
      if (wavelength <= 680)
        ...
        ssa[lc] = 0.98475089485 + 0.0053152066002 * logreff - 0.0061150583857 * logreff * logreff -
                  0.0032775655896 * logreff * logreff * logreff;

      /* asymmetry parameter */
      if (wavelength <= 680)
        g[lc] =
          0.7700034985 + 0.19598466851 * logreff - 0.11836420885 * logreff * logreff + 0.025209205131 * logreff * logreff * logreff;
      else
        g[lc] = 0.83631171237 - 0.19965998649 * logreff + 0.46130320487 * logreff * logreff -
                0.29719270332 * logreff * logreff * logreff + 0.062554483594 * logreff * logreff * logreff * logreff;
...
int interpolate_ssprop_in_lambda (ssprop_struct* src,
  ...
  for (iv = 0; iv < tgt_nlambda; iv++) {
    ...
    tgt->gg[iv]     = calloc (tgt->nreff[iv], sizeof (double));
  ...
  for (ivn = nlambda_lower; ivn <= nlambda_upper; ivn++) {
    ...
    if (tgt_lambda[ivn] < src_lambda[0] - 1.E-10 || tgt_lambda[ivn] > src_lambda[src_nlambda - 1] + 1.E-10) {
      fprintf (stderr,
               "internal wavelength not within external wavelength grid %d %f [%f - %f]\n",
               ivn,
               tgt_lambda[ivn],
               src_lambda[0],
               src_lambda[src_nlambda - 1]);
      return -1;
    }
    ...
    for (ir = 0; ir < src->nreff[0]; ir++) {
      ...
      /*interpolate gg*/  //Paul Ockenfuss
      tgt->gg[ivn][ir] = interp_optprop_weight[0] * src->gg[i][ir] + interp_optprop_weight[1] * src->gg[ip1][ir];
      ...
      if (src->alloc_moments) {
        ...
        for (ip = 0; ip < src->nphamat; ip++)
          for (k = 0; k < tgt->nleg[ivn][ir]; k++) {
            ...
            tgt->legen[ivn][ir][ip][k] = interp_optprop_weight[0] * value[0] + interp_optprop_weight[1] * value[1];
          }
...
int read_isccp_reflectivity (int type, float sza, float tau, char* path, int quiet, float* ref) {
  int           i = 0;
  int           u = 0, l = 0;
...
int read_ECMWF_clouds (char*          filename,
  ...
  if (status != NC_NOERR) {
    fprintf (stderr,
             "Error '%s' opening netCDF file '%s' (line %d, function %s in %s)\n",
             nc_strerror (status),
             filename,
             __LINE__,
             __func__,
  ...
  if (ilat < 0) {
    fprintf (stderr,
             "Error -1 finding index for lat=%5.2f in %s (line %d, function %s in %s)\n",
             lat,
             filename,
             __LINE__,
             __func__,
  ...
  if (ilon < 0) {
    fprintf (stderr,
             "Error -2 finding index for lon=%5.2f in %s (line %d, function %s in %s)\n",
             lon,
             filename,
             __LINE__,
             __func__,
  ...
  if (verbose)
    fprintf (stderr,
             " *** ECMWF %s cloud data at lat = %5.2f (%5.2f), lon = %5.2f (%5.2f) \n",
             cloud,
             ECMWF_lat[ilat],
             lat,
             ECMWF_lon[ilon],
  ...
  if ((CWC = calloc (nt, sizeof (float*))) == NULL) {
    fprintf (stderr,
             "Error, allocating memory for Cloud Water Content (line %d, function %s in %s)\n",
             __LINE__,
             __func__,
             __FILE__);
    return -13;
  ...
  for (t = 0; t <= nt - 1; t++) {
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d reading temperature '%s' from netCDF file %s (line %d, function %s in %s)\n",
               status,
               T_name,
               filename,
               __LINE__,
    ...
    if (strncasecmp (cloud, "wc", 2) == 0) {
      ...
      if (status != 0) {
        fprintf (stderr,
                 "Error %d reading Cloud Liquid Water Content '%s' from netCDF file %s (line %d, function %s in %s)\n",
                 status,
                 CLWC_name,
                 filename,
                 __LINE__,
      ...
      if (status != 0) {
        fprintf (stderr,
                 "Error %d reading Cloud Ice Water Content '%s' from netCDF file %s (line %d, function %s in %s)\n",
                 status,
                 CIWC_name,
                 filename,
                 __LINE__,
    ...
    if (status != 0) {
      fprintf (stderr,
               "Error %d reading Cloud Cover '%s' from netCDF file %s (line %d, function %s in %s)\n",
               status,
               CC_name,
               filename,
               __LINE__,
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d calculating z-grid of ECMWF cloud data (line %d, function %s in %s) \n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  if (status != 0) {
    fprintf (stderr,
             "Error %d calculating zd_layer-grid of ECMWF cloud data (line %d, function %s in %s) \n",
             status,
             __LINE__,
             __func__,
             __FILE__);
  ...
  if (strncasecmp (cloud, "wc", 2) == 0) {
    ...
    status = interpolate_profile (z_reff,
                                  &(reff),
                                  nreff,
                                  zd_layer,
                                  nlay,
                                  INTERP_METHOD_LINEAR,
                                  quiet); /* !! ATTENTION POINTER ARITHMETIC !! */
    if (status != 0) {                    /* last entry is lowermost level */
      fprintf (stderr,
               "Error %d during interpolate_profile of r_eff (line %d, function %s in %s) \n",
               status,
               __LINE__,
               __func__,
               __FILE__);
    ...
    case REFF_FIXED:
      ...
      if (verbose)
        fprintf (stderr, " ... fixed r_eff_ice = %f\n", reff_fixed);
    ...
    case REFF_OU:
      if (verbose)
        fprintf (stderr, " ... parametrisation for r_eff_ice of Ou and Liou, Atm. Res. 1995, 'Ice microphysics ...'\n");
    ...
    case REFF_FILE:
      if (verbose)
        fprintf (stderr, " ... read r_eff_ice from file %s\n", file_ic_reff);

      if (nt == 1) {

        status = get_all_netCDF_indices (file_ic_reff,
                                         lat,
                                         lon,
                                         &(ncid),
                                         &(ilat2),
        ...
        if (status != 0) {
          fprintf (stderr,
                   "Error %d reading Ice Cloud effective radius '%s' from netCDF file\n   %s (line %d, function %s in %s)\n",
                   status,
                   "reff_ice",
                   file_ic_reff,
                   __LINE__,
    ...
    case PROP_FU:
      if (verbose)
        fprintf (stderr,
                 " ... check, if effective radius is inside limits of the Fu parameterisation [%f,%f] \n",
                 9.316 * fu2yang,
                 64.799 * fu2yang);
      for (lc = 0; lc < nlay; lc++) {
        ...
        if (reff[lc] > (64.799 * fu2yang)) {
          if (!quiet && CWC[0][lc] != 0.0) {
            fprintf (stderr,
                     " *** Warning, reff_ice[%3d] = %f outside Fu parameterisation [%f,%f] \n",
                     lc,
                     reff[lc],
                     9.316 * fu2yang,
                     64.799 * fu2yang);
          ...
          if (!quiet && CWC[0][lc] != 0.0)
            fprintf (stderr, " changed to reff_ice = %f IWC = %e \n", reff[lc], CWC[0][lc]);
        }
    ...
    case PROP_YANG2013:
      if (verbose)
        fprintf (stderr, " ... check, if effective radius is inside limits of the Baum parametrisation [%f,%f] \n", 5.0, 90.0);
      for (lc = 0; lc < nlay; lc++) {
        if (reff[lc] > 90.0) {
          if (!quiet && CWC[0][lc] != 0.0) {
    ...
    case PROP_BAUM_V36:
      if (verbose)
        fprintf (stderr,
                 " ... check, if effective radius is inside limits of the Baum (v3.6) parametrisation [%f,%f] \n",
                 5.0,
                 60.0);
      for (lc = 0; lc < nlay; lc++) {
        if (reff[lc] > 60.0) {
          if (!quiet && CWC[0][lc] != 0.0) {
            fprintf (stderr,
                     " *** Warning, reff_ice[%3d] = %f outside Baum (v 3.6) parametrisation [%f,%f] \n",
                     lc,
                     reff[lc],
                     5.0,
                     60.0);
  ...
  for (lc = 0; lc < nlev; lc++) {
    if (cf->cf[lc] != 0.0) {
      ...
      if (!quiet) {
        fprintf (stderr,
                 " *** Warning, cloud fraction[%3d]=%e, but %s cloud water density = %e g/m3 \n",
                 lc,
                 cf->cf[lc],
                 phase,
                 (*cld_data)[lc][1]);
  ...
  if (verbose) {
    ...
    if (cloud_overlap != CLOUD_OVERLAP_OFF) {
      fprintf (stderr,
               "#-------------------------------------------------------------------------------------------------------\n");
      fprintf (stderr,
               "# lc |    z     |    z    |  CWD_cloud |  r_eff   |   rho_air  |  CWC_cloud   |     CC    |  CWC_mean  |\n");
      fprintf (stderr,
               "#    |   [km]   |   [km]  |   [g/m3]   |  [mue]   |   [g/m3]   |   [kg/kg]    |           |   [kg/kg]  |\n");
      fprintf (stderr,
               "#-------------------------------------------------------------------------------------------------------\n");
    } else {
      fprintf (stderr, "#------------------------------------------------------------------------------\n");
    ...
    for (lc = 1; lc < nlev; lc++) {
      if (lc < nlay - 1) {                                                     /* check also CWC one level below (lc+1) */
        ...
        fprintf (stderr,
                 "%5d %9.5f %9.5f  %e %8.4f  %e  %e ",
                 lc,
                 (*cld_data)[lc][0],
                 (*cld_data)[lc - 1][0],
                 (*cld_data)[lc][1],
                 (*cld_data)[lc][2],
                 dens_air[lc - 1] * factor,
...
int read_ECHAM_clouds (char*          filename,
  ...
  if (nt == 2) {
    ...
    for (lc = 0; lc < nlev; lc++) {
      ...
      splitfactor[0][lc] = (1.0 - dt) * splitfactor[0][lc] + dt * splitfactor[1][lc];
  ...
  for (lc = 1; lc < nlev; lc++) {
    ...
    if (strncasecmp (cloud, "wc", 2) == 0) /*LWC*/
      (*cld_data)[lc][1] = (CWP[0][lc] * (1.0 - splitfactor[0][lc])) / (zd[lc - 1] - zd[lc]) / 1000.0;
    else /* IWC*/
      (*cld_data)[lc][1] = (CWP[0][lc] * splitfactor[0][lc]) / (zd[lc - 1] - zd[lc]) / 1000.0;
  }
  cf->nlev = nlev;
...
int read_raytracing_file (char* filename, crystal_prop_struct** raytracing_prop, int* n_raytracing_prop, int quiet) {
  ...
  status = ASCII_checkfile (filename, &rows, &min_columns, &max_columns, &max_length);
  ...
  /* allocate memory */
  status = ASCII_calloc_string (&string, rows, max_columns, max_length);
